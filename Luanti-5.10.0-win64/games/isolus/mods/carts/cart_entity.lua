...
function cart_entity:on_rightclick(clicker)
	...
	if self.driver and pname == self.driver then
...
local function rail_on_step(self, dtime)
	...
	if player and dir.y ~= self.old_dir.y then
		local feet = {x=0, y=-4.5, z=0}
		...
		player:set_attach(self.object, "", feet, {x=dir.y * -30, y=0, z=0})
...
core.register_craftitem("carts:cart", {
	...
	on_place = function(itemstack, placer, pointd)
		...
		if carts:is_rail(pointd.under) then
			core.add_entity(pointd.under, "carts:cart")
		elseif carts:is_rail(pointd.above) then
			core.add_entity(pointd.above, "carts:cart")
		else
			return
		end

		core.sound_play({name = "default_place_node_metal", gain = 0.5},
			{pos = pointd.above}, true)
