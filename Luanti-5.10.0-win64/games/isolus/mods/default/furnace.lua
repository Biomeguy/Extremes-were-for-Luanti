...
local function furnace_node_timer(pos, elapsed)
	...
	if items_smelt > 0 then
		...
		core.sound_play("default_cool_lava",
			{pos = pos, max_hear_distance = 16, gain = .07* math.min(items_smelt, 7) }, true)
	end
	...
	if pos.y > -50 and core.settings:get_bool("altitude_heat_humidity_check") ~= false then
for i=-1,1 do
for j=-1,1 do
for k=-1,1 do
	...
	elseif n.name == "air" then
		...
		core.get_node_timer(p):start(math.random(4, 5))
	end
end
end
end
		...
		if timer_elapsed == 0 or (timer_elapsed + 1) % 5 == 0 then
			local sound_id = core.sound_play("default_furnace_active",
				{pos = pos, max_hear_distance = 16, gain = .25})
			local hash = core.hash_node_position(pos)
...
core.register_node("default:furnace", apply_logger({
	...
	tiles = {"default_furnace_top.png", "default_furnace_bottom.png",
