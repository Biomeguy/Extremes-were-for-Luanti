...
player_api.register_model("3d_armor_character.b3d", {
	...
	animations = {
		stand = {x=0, y=79},
		lay = {x=162, y=166, eye_height = .3, override_local = true,
			collisionbox = {-.6, 0, -.6, .6, .3, .6}},
		walk = {x=168, y=187},
		mine = {x=189, y=198},
		walk_mine = {x=200, y=219},
		sit = {x=81, y=160, eye_height = .8, override_local = true,
...
core.register_globalstep(function(dtime)
	...
	for player, count in pairs(pending_players) do
		...
		if remove == false and count > armor.config.init_times then
			core.log("warning", "3d_armor: Failed to initialize player")
			remove = true
		end
		if remove == true then
			pending_players[player] = nil
...
if armor.config.fire_protect == true then
	...
	for _, row in pairs(armor.fire_nodes) do
		if core.registered_nodes[row[1]] then
			local damage = core.registered_nodes[row[1]].damage_per_second
			if not damage or damage == 0 then
				core.override_item(row[1], {damage_per_second = row[3]})
			end
		end
	end
else
