...
local night_frequent = {
	...
	{name="cricket", length=6, gain=night_frequent_volume}
}
...
local day_frequent = {
	...
	{name="crestedlark", length=7, gain=day_frequent_volume*4},
	{name="craw", length=3, gain=day_frequent_volume},
	{name="bluejay", length=18, gain=day_frequent_volume},
	{name="canadianloon", length=15, gain=day_frequent_volume*2},
...
local desert = {
	...
	{name="RattleSnake", length=4, gain=desert_volume}
}
...
local desert_frequent = {
	...
	{name="desertwind", length=9, gain=desert_frequent_volume}
}
...
local icesheet = {
	handler = {},
	frequency = icesheet_frequency,
	{name="icecrack", length=22.5, gain=icesheet_volume}
}


local music = {
	handler = {},
	frequency = 0,--music_frequency,
	is_music=true,
...
local is_daytime = function()
	return (core.get_timeofday() > 0.2 and core.get_timeofday() < 0.8)
end
...
local nodes_in_range = function(pos, search_distance, node_name)
	...
	nodes = core.find_nodes_in_area(minp, maxp, node_name)
	--core.chat_send_all("Found (" .. node_name .. ": " .. #nodes .. ")")
	return #nodes
end
...
--	core.chat_send_all("z-Found (" .. node_name .. ": " .. #nodes .. ")")
	...
	nodes = core.find_nodes_in_area(minp, maxp, node_name)	
--	core.chat_send_all("x+Found (" .. node_name .. ": " .. #nodes .. ")")
	if #nodes >= threshold then
		return true
	end
	totalnodes = totalnodes + #nodes
...
--	core.chat_send_all("x+Found (" .. node_name .. ": " .. #nodes .. ")")	
	...
	totalnodes = totalnodes + #nodes
--	core.chat_send_all("Found total(" .. totalnodes .. ")")
...
local get_immediate_nodes = function(pos)
	pos.y = pos.y-1
	node_under_feet = core.get_node(pos).name
	pos.y = pos.y-3
	node_3_under_feet = core.get_node(pos).name
	pos.y = pos.y+3
	pos.y = pos.y+2.2
	node_at_upper_body = core.get_node(pos).name
	pos.y = pos.y-1.19   
	node_at_lower_body = core.get_node(pos).name
	pos.y = pos.y+0.99  
	--core.chat_send_all("node_under_feet(" .. nodename .. ")")
end 
...
local get_ambience = function(player)
	...
	elseif node_at_upper_body == "air" then
		if string.find(node_at_lower_body, "default:water") or string.find(node_under_feet, "default:water") then
		    --core.chat_send_all("bottom counted as water")
...
--		core.chat_send_all("not moving horiz")
--	else
--		core.chat_send_all("moving horiz")
--	end	
--	core.chat_send_all("nub:" ..node_at_upper_body)
--	core.chat_send_all("nlb:" ..node_at_lower_body)
--	core.chat_send_all("nuf:" ..node_under_feet)
--	core.chat_send_all("----------")
	
	
--	if player_is_moving_horiz then
--		core.chat_send_all("playermoving")
--	end
--	if player_is_climbing then
--			core.chat_send_all("player Climbing")
--	end
--	core.chat_send_all("nub:" ..node_at_upper_body)
--	core.chat_send_all("nlb:" ..node_at_lower_body)
--	core.chat_send_all("nuf:" ..node_under_feet)
--	core.chat_send_all("n3uf:" ..node_3_under_feet)
--	
...
--	core.chat_send_all("airUnder:" ..air_under_player)
--	core.chat_send_all("ignoreUnder:" ..ignore_under_player)
--	core.chat_send_all("a+i:" ..air_plus_ignore_under)
--	core.chat_send_all("counter: (" .. counter .. "-----------------)")
	--core.chat_send_all(air_or_ignore[node_under_feet])
...
--if we are near sea level and there is lots of water around the area
	...
	--core.chat_send_all("desertcount: " .. desert_in_range .. ",".. pos.y )
	...
	ice_in_range = (nodes_in_range(pos, 6, "default:snow_block")+nodes_in_range(pos, 6, "default:cave_ice"))
	--core.chat_send_all("desertcount: " .. ice_in_range .. ",".. pos.y )
	if ice_in_range >250 then
		if music then
			return {icesheet=icesheet, music=music}
		else
			return {icesheet}
		end
	end
...
--	core.chat_send_all("Found " .. nodename .. pos.y )
...
local play_sound = function(player, list, number, is_music)
	...
	if list.handler[pname] == nil then
		local gain = 1.0
		if list[number].gain ~= nil then
			if is_music then 				
				gain = list[number].gain*MUSICVOLUME
				--core.chat_send_all("gain music: " .. gain )
			else
				gain = list[number].gain*SOUNDVOLUME 
				--core.chat_send_all("gain sound: " .. gain )
			end
		end
		local handler = core.sound_play(list[number].name, {to_player=pname, gain=gain})
		if handler ~= nil then
			list.handler[pname] = handler
			core.after(list[number].length, function(args)
				local list = args[1]
				local pname = args[2]
				if list.handler[pname] ~= nil then
					core.sound_stop(list.handler[pname])
					list.handler[pname] = nil
				end
			end, {list, pname})
		end
	end
	...
	if still_playing.cave == nil then
		...
		if list.handler[pname] ~= nil then
			if list.on_stop ~= nil then
				core.sound_play(list.on_stop, {to_player=player:get_player_name(),gain=SOUNDVOLUME})
			end
			core.sound_stop(list.handler[pname])
			list.handler[pname] = nil
		end
...
--[[	if still_playing.flowing_water == nil then
	...
	if still_playing.water_frequent == nil then
		...
		if list.handler[pname] ~= nil then
			if list.on_stop ~= nil then				
				core.sound_play(list.on_stop, {to_player=player:get_player_name(),gain=SOUNDVOLUME})
		--		core.chat_send_all("list.on_stop " .. list.on_stop  )				
				played_on_start = false
			end
			core.sound_stop(list.handler[pname])
			list.handler[pname] = nil
	...
	if still_playing.flying == nil then
		--core.chat_send_all("begin stop flying "   )
		local list = flying
		if list.handler[pname] ~= nil then
		--	core.chat_send_all("handler flying "   )
			if list.on_stop ~= nil then
			--	core.chat_send_all("onstop flying"   )
				core.sound_play(list.on_stop, {to_player=player:get_player_name(),gain=SOUNDVOLUME})
				played_on_start = false
			end
			core.sound_stop(list.handler[pname])
...
--				--	core.chat_send_all("playedOnStart "  )
--				else
--				--	core.chat_send_all("FALSEplayedOnStart "  )
--				end
				if ambience.on_start ~= nil and played_on_start == false then
					played_on_start = true
					core.sound_play(ambience.on_start, {to_player=player:get_player_name(),gain=SOUNDVOLUME})					
				end
			--	core.chat_send_all("ambience: " ..ambience )
			--	if ambience.on_start ~= nil and played_on_start_flying == false then
			--		played_on_start_flying = true
			--		core.sound_play(ambience.on_start, {to_player=player:get_player_name()})					
			--	end
...
core.register_chatcommand("svol", {
	...
	--	ambiences = get_ambience(player)
	--	stop_sound({}, player)
		core.chat_send_player(name, "Sound volume set.")
	end,		})
core.register_chatcommand("mvol", {
	...
	--	stop_sound({}, player)
	--	ambiences = get_ambience(player)	
		core.chat_send_player(name, "Music volume set.")
	end,		})	