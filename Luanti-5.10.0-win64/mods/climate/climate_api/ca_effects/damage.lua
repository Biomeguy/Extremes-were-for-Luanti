...
if not core.is_yes(core.settings:get_bool("enable_damage"))
...
local function check_hit(player, ray)
	...
	if ray.type ~= nil and ray.type ~= "light" and ray.type ~= "raycast" then
		core.log("warning", "[Climate API] Invalid damage check configuration")
		return false
	end
	...
	or not climate_mod.settings.raycast then
		return core.get_node_light(ppos, 0.5) == 15
	end
