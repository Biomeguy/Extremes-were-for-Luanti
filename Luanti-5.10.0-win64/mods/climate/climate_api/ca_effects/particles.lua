...
local function parse_config(player, particles)
	...
	if config.use_wind then
		local pos = vector.multiply(vector.add(config.minpos, config.maxpos), .5)
		...
		local vel = vector.multiply(vector.add(config.minvel, config.maxvel), .5)
...
local function handle_effect(player_data)
	for playername, data in pairs(player_data) do
		...
		for weather, value in pairs(data) do
			...
			core.add_particlespawner(config)
		end
	end
end
