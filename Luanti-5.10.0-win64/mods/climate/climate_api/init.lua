-- warn about outdated Luanti versions
assert(core.add_particlespawner, "[Climate API] This mod requires a more current version of Luanti")
...
local function get_setting_bool(name, default)
	local value = core.settings:get_bool("climate_api_" .. name)
	if type(value) == "nil" then value = default end
	return core.is_yes(value)
end

-- retrive numeric value from mod config
local function get_setting_number(name, default)
	local value = core.settings:get("climate_api_" .. name)
	if type(value) == "nil" then value = default end
...
climate_mod.i18n = core.get_translator("climate_api")

-- warn about clouds being overriden by MTG weather
if climate_mod.settings.skybox and core.get_modpath("weather") and core.settings:get_bool("enable_weather") then
    core.log("warning", "[Regional Weather] " ..
    ...
    core.settings:set_bool("enable_weather", false)
end
