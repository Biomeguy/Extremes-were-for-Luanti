...
function api.register_effect(name, handler, htype)
	...
	if htype ~= "start" and htype ~= "tick" and htype ~= "stop" then
		core.log("warning", "[Climate API] Effect " .. dump(name) .. " uses invalid callback type: " .. dump(htype))
		return
	end
...
-- Uses same config as core.register_abm() but also adds
