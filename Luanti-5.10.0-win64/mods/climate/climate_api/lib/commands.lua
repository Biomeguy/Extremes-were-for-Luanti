...
core.register_chatcommand("set_base_heat", {
	params = "<heat>",
	description = S("Override the weather algorithm's base heat"),
	privs = { weather = true },
	func = function(playername, param)
		if param == nil or param == "" then
			core.chat_send_player(playername, S("Provide a number to modify the base heat"))
			return
		end
		if param == "auto" then param = 0 end
		climate_mod.settings.heat = tonumber(param) or 0
		core.chat_send_player(playername, S("Base heat changed"))
	end
})

-- override global heat levels with given value
core.register_chatcommand("set_heat", {
	params = "<heat>",
	description = S("Override the weather algorithm's heat"),
	privs = { weather = true },
	func = function(playername, param)
		if param == nil or param == "" then
			core.chat_send_player(playername, S("Provide a number to modify the heat"))
			return
		end
		if param == "auto" then
			climate_mod.forced_enviroment.heat = nil
			core.chat_send_player(playername, S("Heat value reset"))
		else
			climate_mod.forced_enviroment.heat = tonumber(param) or 0
			core.chat_send_player(playername, S("Heat value changed"))
		end
	end
})

-- set base heat to increase or decrease global climate humidity
core.register_chatcommand("set_base_humidity", {
	params = "<humidity>",
	...
	func = function(playername, param)
		if param == "auto" then param = 0 end
		if param == nil or param == "" then
			core.chat_send_player(playername, S("Provide a number to modify the base humidity"))
			return
		end
		climate_mod.settings.humidity = tonumber(param) or 0
		core.chat_send_player(playername, S("Base humidity changed"))
	end
})

-- override global humidity with given value
core.register_chatcommand("set_humidity", {
	params = "<humidity>",
	description = S("Override the weather algorithm's humidity"),
	privs = { weather = true },
	func = function(playername, param)
		if param == nil or param == "" then
			core.chat_send_player(playername, S("Provide a number to modify the humidity"))
			return
		end
		if param == "auto" then
			climate_mod.forced_enviroment.humidity = nil
			core.chat_send_player(playername, S("Humidity value reset"))
		else
			climate_mod.forced_enviroment.humidity = tonumber(param) or 0
			core.chat_send_player(playername, S("Humidity value changed"))
		end
	end
})

-- override wind direction and speed with given values
core.register_chatcommand("set_wind", {
	params = "<wind>",
	description = S("Override the weather algorithm's windspeed"),
	privs = { weather = true },
	func = function(playername, param)
		if param == nil or param == "" then
			core.chat_send_player(playername, S("Provide a vector of two numbers to modify the wind"))
			return
		end
		local arguments = {}
		...
		if arg1 == "auto" then
			...
			core.chat_send_player(playername, S("Wind reset"))
		elseif wind_x == nil or wind_z == nil then
			core.chat_send_player(playername, S("Invalid wind configuration"))
		else
			...
			core.chat_send_player(playername, S("Wind changed"))
		end
	end
})

-- display current mod config
core.register_chatcommand("weather_settings", {
	description = S("Print the active Climate API configuration"),
	func = function(playername)
		core.chat_send_player(playername, S("Current Settings") .. "\n================")
		for setting, value in pairs(climate_mod.settings) do
			core.chat_send_player(playername, dump2(value, setting))
		end
	end
})
...
core.register_chatcommand("set_weather", {
	...
	func = function(playername, param)
		...
		if weather == nil or climate_mod.weathers[weather] == nil then
			core.chat_send_player(playername, S("Unknown weather preset"))
			return
		end
		local status
		...
		elseif arguments[2] == "auto" then
			...
			core.chat_send_player(playername, S("Invalid weather status. Set the preset to either on, off or auto."))
			return
		end
		climate_mod.forced_weather[weather] = status
		core.chat_send_player(playername, S("Weather @1 successfully set to @2", weather, arguments[2]))
	end
})

-- list all weather presets and whether they have been forced or disabled
core.register_chatcommand("weather_status", {
	description = S("Prints which weather presets are enforced or disabled"),
	func = function(playername)
		core.chat_send_player(playername, S("Current activation rules:") .. "\n================")
