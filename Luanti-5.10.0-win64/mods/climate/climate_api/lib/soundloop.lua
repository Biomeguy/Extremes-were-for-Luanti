...
soundloop.play = function(player, sound, fade)
	...
	elseif sounds[player][sound.name] ~= sound.gain then
		core.sound_stop(sounds[player][sound.name].handle)
	...
	handle = core.sound_play(sound.name, {
		to_player = player,
		loop = true,
		gain = 0
	})
	...
	core.sound_fade(handle, step, sound.gain)
	return handle
end
...
soundloop.stop = function(player, sound, fade)
	...
	core.sound_fade(handle, step, 0)
	sounds[player][sound.name].gain = 0
	core.after(fade, core.sound_stop, handle)
end
