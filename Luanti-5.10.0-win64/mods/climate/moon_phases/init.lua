local mod_climate_api = core.get_modpath("climate_api") ~= nil
local mod_skylayer = core.get_modpath("skylayer") ~= nil

local modpath = core.get_modpath("moon_phases");
...
local PHASE_LENGTH = tonumber(core.settings:get("moon_phases_cycle") or DEFAULT_LENGTH)
local TEXTURE_STYLE = core.settings:get("moon_phases_style") or DEFAULT_STYLE
...
local function set_texture(player, phase)
	...
	if mod_climate_api then
		sky.priority = 0
		climate_api.skybox.add(pname, name, sky)
	elseif mod_skylayer then
		sky.name = name
		skylayer.add_layer(pname, sky)
	else
