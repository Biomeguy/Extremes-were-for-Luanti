...
local destruction_handler = function(pos)
	pos.y = pos.y - 1
	if core.get_node(pos).name == "default:dirt_with_snow" then
		core.set_node(pos, {name = "default:dirt_with_grass"})
	end
end

for i = 1,5 do
	core.register_node(BLOCK_PREFIX .. i, {
		...
		node_box = {
			type  = "fixed",
			fixed = {-.5, -.5, -.5, .5, .2*i - .5, .5}
		},
		collision_box = {
			type  = "fixed",
			fixed = {-.5, -.5, -.5, .5, .2*i - .6, .5}
		},
		groups = {
...
climate_api.register_abm({
   ...
   action = function (pos, node, env)
		 ...
		 local base = core.get_node(vector.add(pos, {x=0, y=-1, z=0})).name
		 local is_soil = core.get_item_group(base, "soil") or 0
		 local is_stone = core.get_item_group(base, "stone") or 0
		 if not (is_soil == 0 and is_stone == 0) then
		 	core.set_node(pos, { name = BLOCK_PREFIX .. "1" })
		end
	 end
})
...
climate_api.register_abm({
   ...
   action = function (pos, node, env)
		 local value = core.get_item_group(node.name, "regional_weather_snow_cover")
		 if value == nil then value = 0 end
		 if value < 5 then
			 core.set_node(pos, { name = BLOCK_PREFIX .. (value + 1) })
		 end
	 end
})
...
climate_api.register_abm({
   ...
   action = function (pos, node, env)
			...
			elseif regional_weather.settings.puddles then
				core.set_node(pos, { name = "regional_weather:puddle" })
			else
