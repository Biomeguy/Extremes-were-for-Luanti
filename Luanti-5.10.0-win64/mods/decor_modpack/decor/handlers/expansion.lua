...
core.register_alias("decor:expansion_placeholder", "air")
...
local function is_buildable_to(pname, ...)
	for _, pos in ipairs({...}) do
		local node = core.get_node_or_nil(pos)
		local def = node and core.registered_nodes[node.name]
		if not (def and def.buildable_to) or core.is_protected(pos, pname) then
			return false
		end
	end
	return true
...
local function stack(itemstack, placer, fdir, pos, def, pos2, node1, node2, pointd)
	local pname = placer:get_player_name() or ""
	if is_buildable_to(pname, pos, pos2) then
		...
		if ctrl_node_def and ctrl_node_def.after_place_node then
			ctrl_node_def.after_place_node(pos, placer, itemstack, pointd)
		end
...
local function rightclick_pointed_thing(pos, placer, itemstack, pointd)
	local node = core.get_node_or_nil(pos)
	if not node then return false end
	local def = core.registered_nodes[node.name]
	if not def or not def.on_rightclick then return false end
	return def.on_rightclick(pos, node, placer, itemstack, pointd) or itemstack
end

-- Stack one node above another
-- leave the last argument nil if it's one 2m high node
function homedecor.stack_vertically(itemstack, placer, pointd, node1, node2)
	local rightclick_result = rightclick_pointed_thing(pointd.under, placer, itemstack, pointd)
	if rightclick_result then return rightclick_result end

	local pos, def = select_node(pointd)
	if not pos then return itemstack end

	local top_pos = { x=pos.x, y=pos.y+1, z=pos.z }

	return stack(itemstack, placer, nil, pos, def, top_pos, node1, node2, pointd)
end
...
function homedecor.stack_wing(itemstack, placer, pointd, node1, node2, node1_right, node2_right)
	local rightclick_result = rightclick_pointed_thing(pointd.under, placer, itemstack, pointd)
	if rightclick_result then return rightclick_result end
	...
	local is_right_wing = node1 == core.get_node({ x = pos.x + homedecor.fdir_to_left[fdir+1][1], y=pos.y, z = pos.z + homedecor.fdir_to_left[fdir+1][2] }).name
	if forceright or is_right_wing then
		node1, node2 = node1_right, node2_right
	end

	local top_pos = { x=pos.x, y=pos.y+1, z=pos.z }
	return stack(itemstack, placer, fdir, pos, def, top_pos, node1, node2, pointd)
end

function homedecor.stack_sideways(itemstack, placer, pointd, node1, node2, dir)
	local rightclick_result = rightclick_pointed_thing(pointd.under, placer, itemstack, pointd)
	if rightclick_result then return rightclick_result end
	...
	return stack(itemstack, placer, fdir, pos, def, pos2, node1, node2, pointd)
end

function homedecor.place_banister(itemstack, placer, pointd)
	local rightclick_result = rightclick_pointed_thing(pointd.under, placer, itemstack, pointd)
	if rightclick_result then return rightclick_result end

	local pos, _ = select_node(pointd)
	if not pos then return itemstack end
	...
	if not (adef and adef.buildable_to) then
		core.chat_send_player(pname, S("Not enough room - the upper space is occupied!" ))
		return itemstack
	end

	if core.is_protected(abovepos, pname) then
		core.chat_send_player(pname, S("Someone already owns that spot."))
		return itemstack
	end
	...
	if (left_below_node and string.find(left_below_node.name, "banister_.-_diagonal_right")
	  and below_node and is_buildable_to(pname, below_pos, below_pos))
	  or not is_buildable_to(pname, right_fwd_above_pos, right_fwd_above_pos) then
	...
	elseif (right_below_node and string.find(right_below_node.name, "banister_.-_diagonal_left")
	  and below_node and is_buildable_to(pname, below_pos, below_pos))
	  or not is_buildable_to(pname, left_fwd_above_pos, left_fwd_above_pos) then
	...
	elseif left_fwd_below_node and string.find(left_fwd_below_node.name, "decor:banister_.*_diagonal")
	  and is_buildable_to(pname, fwd_pos, fwd_pos) then
	...
	elseif left_node and string.find(left_node.name, "decor:banister_.*_horizontal") then
		fdir = left_node.param2
		new_place_name = left_node.name
	elseif right_node and string.find(right_node.name, "decor:banister_.*_horizontal") then
		fdir = right_node.param2
	...
	core.set_node(pos, {name = new_place_name, param2 = fdir})
	itemstack:take_item()
	return itemstack
end
