...
function homedecor.register(name, original_def)
	...
	if expand then
		...
		def.on_place = def.on_place or function(itemstack, placer, pointd)
			if expand.top then
				return homedecor.stack_vertically(itemstack, placer, pointd, itemstack:get_name(), expand.top)
			elseif expand.right then
				return homedecor.stack_sideways(itemstack, placer, pointd, itemstack:get_name(), expand.right, true)
			elseif expand.forward then
				return homedecor.stack_sideways(itemstack, placer, pointd, itemstack:get_name(), expand.forward, false)
			end
		end
	...
	-- register the actual core node
	core.register_node("decor:" .. name, def)
end
