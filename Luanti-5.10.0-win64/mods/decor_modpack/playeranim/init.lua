...
local ANIMATION_SPEED = tonumber(core.settings:get("playeranim.animation_speed")) or 2.4
local ANIMATION_SPEED_SNEAK = tonumber(core.settings:get("playeranim.animation_speed_sneak")) or 0.8
local BODY_ROTATION_DELAY = math.max(math.floor(tonumber(core.settings:get("playeranim.body_rotation_delay")) or 7), 1)
local BODY_X_ROTATION_SNEAK = tonumber(core.settings:get("playeranim.body_x_rotation_sneak")) or 6.0
...
local get_animation = core.global_exists("player_api")
...
local function rotate_bone(player, bone, rotation, position_optional)
	...
	local B_rota = vector_add(rotation, BONE_ROTATION[bone])

	local previous_position = players_animation_data:get_bone_position(player, bone)
	local B_pos = BONE_POSITION[bone]
	if position_optional then
		B_pos = vector_add(B_pos, position_optional)
	end

	if not previous_rotation
	or not previous_position
	or not vector_equals(B_rota, previous_rotation)
	or not vector_equals(B_pos, previous_position) then
		player:set_bone_position(bone, B_pos, B_rota)-- Can't update this because of radians vs degrees
		players_animation_data:set_bone_rotation(player, bone, B_rota)
		players_animation_data:set_bone_position(player, bone, B_pos)
	end
end
...
local function set_animation(player, animation, force_animate)
	local animation_changed = (players_animation_data:get_previous_animation(player) ~= animation)
