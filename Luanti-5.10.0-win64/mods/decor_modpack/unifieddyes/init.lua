...
function unifieddyes.recolor_on_place(pos, placer, itemstack, pointd)

	local pname = placer:get_player_name()
	local stackname = itemstack:get_name()

	if unifieddyes.last_dyed_node[pname] ~= stackname then
		if unifieddyes.last_used_dye[pname] then
			core.chat_send_player(pname, "Switched to \""..stackname.."\" while auto-coloring, color reset to neutral.")
		end
		unifieddyes.last_used_dye[pname] = nil
		unifieddyes.last_dyed_node[pname] = nil
	end

	unifieddyes.last_dyed_node[pname] = stackname

	if unifieddyes.last_used_dye[pname] then
		local lastdye = unifieddyes.last_used_dye[pname]

		local inv = placer:get_inventory()
		if (lastdye and lastdye ~= "" and inv:contains_item("main", lastdye.." 1")) or creative_mode then

			local nodedef = core.registered_nodes[stackname]
			...
			core.set_node(pos, { name = newname, param2 = oldfdir + paletteidx })
			...
			core.chat_send_player(pname, "Ran out of "..unifieddyes.last_used_dye[pname]..", resetting to neutral.")
			unifieddyes.last_used_dye[pname] = nil
		end
	end
end
...
function unifieddyes.fix_rotation(pos, placer, itemstack, pointd)
	local node = core.get_node(pos)
	...
	core.swap_node(pos, { name = node.name, param2 = fdir })
end

-- use this when you have a "wallmounted" node that should never be oriented
-- to floor or ceiling...

function unifieddyes.fix_rotation_nsew(pos, placer, itemstack, pointd)
	local node = core.get_node(pos)
	local yaw = placer:get_look_horizontal()
	local dir = core.yaw_to_dir(yaw+1.5)
	local fdir = core.dir_to_wallmounted(dir)
...
function unifieddyes.select_node(pointd)
...
function unifieddyes.on_use(itemstack, player, pointd)
	local stackname = itemstack:get_name()
	local pname = player:get_player_name()

	if pointd and pointd.type == "node" then
		if core.is_protected(unifieddyes.select_node(pointd), pname)
		  and not core.check_player_privs(pname, "protection_bypass") then
			core.chat_send_player(pname, "Sorry, someone else owns that spot.")
			return
		end
	end

	if pointd and pointd.type == "object" then
		pointd.ref:punch(player, 0, itemstack:get_tool_capabilities())
		return player:get_wielded_item() -- punch may modified the wielded item, load the new and return it
	end

	if not (pointd and pointd.type == "node") then return end  -- if "using" the dye on nothing at all (e.g. air)
	...
	if not player:get_player_control().sneak then
		local onpunch = nodedef.on_punch(pos, node, player, pointd)
		if onpunch then
			return onpunch
		end
	end
	...
	if player:get_player_control().sneak then
		if unifieddyes.last_used_dye[pname] then
			core.chat_send_player(pname, "Shift-punched a node, switching back to neutral color." )
		end
		unifieddyes.last_used_dye[pname] = nil
		return
	end
	...
	if not (nodedef and nodedef.groups and nodedef.groups.ud_param2_colorable) then
		core.chat_send_player(pname, "That node can't be colored.")
		return
	end
	...
	local pos2 = unifieddyes.select_node(pointd)
	local paletteidx, hue = unifieddyes.getpaletteidx(stackname, palette_type)

	if paletteidx then

		if unifieddyes.last_used_dye[pname] ~= stackname then
			core.chat_send_player(pname, "Color "..stackname.." selected, auto-coloring activated." )
			unifieddyes.last_used_dye[pname] = stackname
		end
		...
		if prevdye == stackname then
			...
			core.chat_send_player(pname, "That node is already "..string.sub(stackname, a + 1).."." )
			return
		...
		if newnode then -- this path is used when the calling mod want to supply a replacement node
			if palette_type == "wallmounted" then
				node.param2 = paletteidx + (core.get_node(pos).param2 % 8)
			elseif palette_type == true then  -- it's colorfacedir
				...
				node.param2 = paletteidx + (core.get_node(pos).param2 % 32)
			...
			if palette_type == "wallmounted" then
				newnode.param2 = paletteidx + (core.get_node(pos).param2 % 8)
			elseif palette_type == true then -- it's colorfacedir
				if oldhue then
					if hue ~= 0 then
				...
				elseif string.find(core.get_node(pos).name, "_grey") and hue ~= 0 then
					newnode.name = string.gsub(newnode.name, "_grey", "_"..unifieddyes.HUES[hue])
				end
				newnode.param2 = paletteidx + (core.get_node(pos).param2 % 32)
			else -- it's the 89-color palette, or the extended palette
		...
		local a,b = string.find(stackname, ":")
		if a then
			core.chat_send_player(pname, "That node can't be colored "..string.sub(stackname, a + 1).."." )
		end
	end
end
...
core.register_alias("dye:light_red",  "dye:pink")
core.register_alias("dye:medium_orange", "dye:brown")

core.register_alias("unifieddyes:black",      "dye:black")
core.register_alias("unifieddyes:dark_grey",  "dye:dark_grey")
core.register_alias("unifieddyes:light_grey", "dye:light_grey")
core.register_alias("unifieddyes:white",      "dye:white")

core.register_alias("unifieddyes:grey_0",     "dye:black")
core.register_alias("unifieddyes:grey_4",     "dye:dark_grey")
core.register_alias("unifieddyes:grey_8",     "dye:grey")
core.register_alias("unifieddyes:grey_11",    "dye:light_grey")
core.register_alias("unifieddyes:grey_15",    "dye:white")

core.register_alias("unifieddyes:white_paint", "dye:white")
core.register_alias("unifieddyes:titanium_dioxide", "dye:white")
core.register_alias("unifieddyes:lightgrey_paint", "dye:light_grey")
core.register_alias("unifieddyes:grey_paint", "dye:grey")
core.register_alias("unifieddyes:darkgrey_paint", "dye:dark_grey")
core.register_alias("unifieddyes:carbon_black", "dye:black")
...
core.register_alias("unifieddyes:aqua", "unifieddyes:spring")
core.register_alias("unifieddyes:skyblue", "unifieddyes:azure")
core.register_alias("unifieddyes:redviolet", "unifieddyes:rose")
