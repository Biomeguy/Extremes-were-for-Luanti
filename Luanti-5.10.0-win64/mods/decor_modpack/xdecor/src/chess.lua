-- Init stuff
if core.get_modpath("realchess") ~= nil then
	-- If the 'realchess' mod was found, don't use any of this mod's Chess code
	core.log("action", "[xdecor] 'realchess' mod detected. Disabling X-Decor-libre's Chess module in favor of realchess")
	return
end
...
-- Load chess bot code
local chessbot = dofile(core.get_modpath(core.get_current_modname()).."/src/chessbot.lua")
...
local chat_prefix = core.colorize("#FFFF00", "["..S("Chess").."] ")
local chat_prefix_debug = core.colorize("#FFFF00", "["..S("Chess Debug").."] ")
...
-- is not sent.
-- If you only pass pname and message, the message will always
-- be sent (if the player exists).
--
-- Parameters:
-- * pname: Name of player to send message to (can be a bot name)
...
local function send_message(pname, message, playerColor, botColor, isDebug)
	local prefix
	if isDebug then
	...
	core.chat_send_player(pname, prefix .. message)
end
...
function realchess.get_king_safe_moves(theoretical_moves, board, player)
	...
	for to_idx, value in pairs(_) do
		...
		if not black_king_idx or not white_king_idx then
			core.log("error", "[xdecor] Chess: Insufficient kings on chessboard!")
			return {}, 0
		end
		local king_idx
...
local fs_init = [[
	...
	.."label[2.2,0.652;"..core.colorize("#404040", FS("Select a game mode")).."]"
	.."label[2.2,10.21;"..core.colorize("#404040", FS("Select a game mode")).."]"
...
if CHESS_DEBUG then
	verify_eaten_list = function(meta)
		...
		if whitePiecesLeft + whitePiecesEaten ~= 16 then
			core.log("error", "[xdecor] Chess: Incorrect number of white pieces in eaten list! pieces="..whitePiecesLeft.."; eaten="..whitePiecesEaten)
			eatenError = true
		elseif blackPiecesLeft + blackPiecesEaten ~= 16 then
			core.log("error", "[xdecor] Chess: Incorrect number of black pieces in eaten list! pieces="..blackPiecesLeft.."; eaten="..blackPiecesEaten)
			eatenError = true
		end
		if eatenError then
			-- halt bots
...
local function update_formspec(meta)
	...
	local turnBlack = core.colorize("#000001", playerBlackDisplay)
	local turnWhite = core.colorize("#000001", playerWhiteDisplay)

	-- several status words for the player
	-- player is in check
	local check_s   = core.colorize("#FF8000", "["..S("check").."]")
	-- player has been checkmated
	local mate_s    = core.colorize("#FF0000", "["..S("checkmate").."]")
	-- player has resigned
	local resign_s    = core.colorize("#FF0000", "["..S("resigned").."]")
	-- player has won
	local win_s     = core.colorize("#26AB2B", "["..S("winner").."]")
	-- player has lost
	local lose_s     = core.colorize("#FF0000", "["..S("loser").."]")
	-- player has a draw
	local draw_s    = core.colorize("#FF00FF", "["..S("draw").."]")
	...
	local formspec = fs ..
		"label[2.2,0.652;"  .. turnBlack .. core.formspec_escape(status_black) .. "]" ..
		blackArr ..
		"label[2.2,10.21;" .. turnWhite .. core.formspec_escape(status_white) .. "]" ..
		whiteArr ..
		moves_list ..
		promotion_formstring ..
...
local function update_game_result(meta, lastMove)
	...
	if lastMove == "white" and not blackCanMove then
		if meta:get_string("blackAttacked") == "true" then
			...
			core.log("action", "[xdecor] Chess: "..playerWhite.." won against "..playerBlack.." by checkmate")
			return
		else
			-- stalemate
			...
			core.log("action", "[xdecor] Chess: A game between "..playerWhite.." and "..playerBlack.." ended in a draw by stalemate")
			return
		end
	end
	if lastMove == "black" and not whiteCanMove then
		if meta:get_string("whiteAttacked") == "true" then
			...
			core.log("action", "[xdecor] Chess: "..playerBlack .." won against "..playerWhite.." by checkmate")
			return
		else
			-- stalemate
			meta:set_string("gameResult", "draw")
	...
	if is_dead_position(board_t) then
		...
		core.log("action", "[xdecor] Chess: A game between "..playerWhite.." and "..playerBlack.." ended in a draw by dead position")
	end
	...
	if meta:get_int("halfmoveClock") >= DRAWCLAIM_LONGGAME_FORCE then
		...
		core.log("action", "[xdecor] Chess: A game between "..playerWhite.." and "..playerBlack.." ended in a draw via the 75-move rule")
	...
	elseif meta:get_int("halfmoveClock") >= DRAWCLAIM_LONGGAME_PLAYER and drawClaim == "50_move_rule" then
		...
		core.log("action", "[xdecor] Chess: A game between "..playerWhite.." and "..playerBlack.." ended in a draw because "..claimer.." has invoked the 50-move rule")
	...
	if CHESS_DEBUG then
		...
		for b=1, #board_t do
			...
			if letter_real ~= letter_pos then
				core.log("error", "[xdecor] Chess: Position history inconsistency on board index "..b..": '"..letter_pos.."' seen but '"..letter_real.."' expected")
			end
		end
		-- Compare current player
		if (lastMove == "black" or lastMove == "") and p_player ~= "w" then
			core.log("error", "[xdecor] Chess: Position history inconsistency: Wrong player! '"..p_player.."' seen but 'w' expected")
		elseif (lastMove == "white") and p_player ~= "b" then
			core.log("error", "[xdecor] Chess: Position history inconsistency: Wrong player! '"..p_player.."' seen but 'b' expected")
		end
		...
		if d_castling ~= p_castling then
			core.log("error", "[xdecor] Chess: Position history inconsistency: Castling rights mismatch! '"..p_castling.."' seen but '"..d_castling.."' expected")
		end
		-- Compare en passant status
		local double_step = meta:get_int("prevDoublePawnStepTo")
		local d_en_passant = en_passant_to_string(double_step)
		if d_en_passant ~= p_en_passant then
			core.log("error", "[xdecor] Chess: Position history inconsistency: En passant status mismatch! '"..p_en_passant.."' seen but '"..d_en_passant.."' expected")
		end
	end
	...
	if forceRepetitionDraw then
		...
		send_message_2(playerWhite, playerBlack, msg, botColor)
		core.log("action", "[xdecor] Chess: A game between "..playerWhite.." and "..playerBlack.." ended in a draw because the same position has appeared 5 times")
	...
	elseif chosenRepetitionDraw and drawClaim == "same_position_3" then
		...
		core.log("action", "[xdecor] Chess: A game between "..playerWhite.." and "..playerBlack.." ended in a draw because "..claimer.." has invoked the threefold repetition rule")
	elseif drawClaim == "same_position_3" then
		local claimer, other
		if lastMove == "black" or lastMove == "" then
			claimer = playerWhite
...
-- * pname: Name of player to move
function realchess.move(meta, from_list, from_index, to_list, to_index, pname)
	...
	if pieceFrom:find("white") then
		if playerWhite ~= "" and playerWhite ~= pname then
			send_message(pname, S("Someone else plays white pieces!"))
			return false
		end
		...
		if lastMove == "white" then
			send_message(pname, S("It's not your turn!"))
			return
		end
	...
	elseif pieceFrom:find("black") then
		if playerBlack ~= "" and playerBlack ~= pname then
			send_message(pname, S("Someone else plays black pieces!"))
			return false
		end
		...
		if lastMove == "" then
			...
			send_message(pname, S("Black cannot move first!"))
			return false
		end
...
function realchess.fields(pos, _, fields, sender)
	...
	if (fields.single_black or fields.single_white) and mode == "single" and meta:get_string("gameResult") == "" and meta:get_string("lastMove") == "" then
		if fields.single_white then
			meta:set_string("botColor", "black")
			meta:set_string("playerWhite", pname)
			meta:set_string("playerBlack", "*"..BOT_NAME.."*")
			update_formspec(meta)
		else
			meta:set_string("botColor", "white")
			meta:set_string("playerWhite", "*"..BOT_NAME.."*")
			meta:set_string("playerBlack", pname)
			update_formspec(meta)
			local inv = meta:get_inventory()
			chessbot.move(inv, meta)
		end
	...
	-- when the game ended.
	if fields.new then
		if mode == "bot_vs_bot" or (playerWhite == pname or playerBlack == pname or playerWhite == "" or playerBlack == "") or meta:get_string("gameResult") ~= "" then
			realchess.init(pos)

		elseif lastMoveTime > 0 then
			if core.get_gametime() >= timeout_limit and
					(playerWhite ~= pname or playerBlack ~= pname) then
				realchess.init(pos)
			else
				send_message(pname,
	...
	if fields.resign and mode ~= "bot_vs_bot" then
		local lastMove = meta:get_string("lastMove")
		if (pname == playerWhite and playerWhite == "") or (pname == playerBlack and playerBlack == "") then
			-- Can't resign before the player name has been recorded
			send_message(pname, S("Resigning is not possible yet."))
			return
		end
		local winner, loser, whiteWon
		if playerWhite == playerBlack and playerWhite == pname and playerWhite ~= "" then
			if lastMove == "black" then
				winner = playerBlack
				loser = playerWhite
				whiteWon = false
		...
		elseif pname == playerWhite and playerWhite ~= "" then
			winner = playerBlack
			loser = playerWhite
			whiteWon = false
		elseif pname == playerBlack and playerBlack ~= "" then
			winner = playerWhite
			loser = playerBlack
			whiteWon = true
		end
		if winner and loser then
			...
			core.log("action", "[xdecor] Chess: "..loser.." has resigned from the game against "..winner)
			update_formspec(meta)
		else
			send_message(pname, S("You can't resign, you're not playing in this game."))
		end
		return
	end
	...
	if fields.draw_50_moves then
		...
		local claimer, other
		if (currentPlayer == "white" and playerWhite == pname) then
			claimer = playerWhite
			other = playerBlack
		elseif (currentPlayer == "black" and playerBlack == pname) then
			claimer = playerBlack
			other = playerWhite
		else
			send_message(pname, S("You can't claim a draw, it's not your turn!"))
			return
		end
	...
	for p=1, #promotions do
		...
		if fields["p_"..promo] then
			if not (pname == playerWhite or pname == playerBlack) then
				send_message(pname, S("You're only a spectator in this game of Chess."))
				return
			end
			local pcolor = promo:sub(-5)
			local activePromo = meta:get_string("promotionActive")
			if activePromo == "" then
				send_message(pname, S("This isn't the time for promotion."))
				return
			elseif activePromo ~= pcolor then
				send_message(pname, S("It's not your turn! This promotion is meant for the other player."))
				return
			end
			if pcolor == "white" and pname == playerWhite or pcolor == "black" and pname == playerBlack then
...
function realchess.can_dig(pos, player)
	...
	elseif (lastMoveTime == 0 and core.get_gametime() > timeout_limit) then
		return true
	else
		if pname == playerWhite or pname == playerBlack or botColor == "both" then
			send_message(pname,
...
function realchess.promote_pawn(meta, color, promoteTo)
	...
	if promoted then
		...
		core.log("error", "[xdecor] Chess: Could not find pawn to promote!")
	end
end
...
if ENABLE_CHESS_GAMES then
	...
	chessboarddef.allow_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, _, player)
		...
		if player and player:is_player() then
			...
			core.log("error", "[xdecor] Chess: An unknown player tried to move a piece in the chessboard inventory")
		end
		realchess.move(meta, from_list, from_index, to_list, to_index, pname)
...
core.register_node(":realchess:chessboard", chessboarddef)

local function register_piece(name, white_desc, black_desc, count)
	for _, color in pairs({"black", "white"}) do
	if not count then
		core.register_craftitem(":realchess:" .. name .. "_" .. color, {
		...
		for i = 1, count do
			core.register_craftitem(":realchess:" .. name .. "_" .. color .. "_" .. i, {
				description = (color == "black") and black_desc or white_desc,
				inventory_image = name .. "_" .. color .. ".png",
				stack_max = 1,
				groups = {not_in_creative_inventory=1}
