...
local function update_item(pos, node)
	...
	local entity = core.add_entity(pos, "xdecor:f_item")
...
function itemframe.rightclick(pos, node, clicker, itemstack)
	...
	local owner = meta:get_string("owner")
	local admin = core.check_player_privs(pname, "protection_bypass")

	if not admin and (pname ~= owner or not itemstack) then
		return itemstack
	end
...
core.register_entity("xdecor:f_item", {
	initial_properties = {
		visual = "wielditem",
	...
	on_activate = function(self, staticdata)
		...
		if core.get_node(pos).name ~= "xdecor:itemframe" then
			self.object:remove()
		end
