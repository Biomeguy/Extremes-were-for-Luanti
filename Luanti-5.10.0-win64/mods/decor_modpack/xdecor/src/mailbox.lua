...
function mailbox:formspec(pos, owner, is_owner)
	...
	return  "size[8,5]" ..
		...
		(core.colorize and
			core.colorize("#FFFF00", owner) or owner)) .. "]" ..
...
function mailbox.dig(pos, player)
	...
	return inv:is_empty("mailbox") and pname == owner
end
...
function mailbox.rightclick(pos, node, clicker, itemstack, pointd)
	local meta = core.get_meta(pos)
	local player = clicker:get_player_name()
	local owner = meta:get_string("owner")

	core.show_formspec(player, "xdecor:mailbox",
