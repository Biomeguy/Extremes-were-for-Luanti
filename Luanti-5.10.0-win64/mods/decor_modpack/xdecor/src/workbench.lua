...
function workbench:repairable(stack)
	...
	if not core.registered_tools[stack] then
		return false
	end
	-- no repair if disable_repair group
	if core.get_item_group(stack, "disable_repair") == 1 then
		return false
	end
	return true
end
...
function workbench:cuttable(itemname)
	...
	if split and split[1] and split[2] then
		if core.registered_nodes["stairs:stair_"..split[2]] ~= nil or
		core.registered_nodes["stairs:slab_"..split[2]] ~= nil then
			return true
		end
	end
...
xdecor.register("workbench", {
	...
	node_box = {
		type = "fixed",
		fixed = {
			{-.4375, -.5, -.4375, .4375, .1875, .4375},
			{.3125, -.5, .3125, .5, .1875, .5},
			{-.5, -.5, -.5, -.3125, .1875, -.3125},
			{-.5, -.5, .3125, -.3125, .1875, .5},
			{.3125, -.5, -.5, .5, .1875, -.3125},
			{-.5, .1875, -.5, .5, .5, .5}
		}
	},
...
for i = 1, #cuttable_nodes do
	...
	if item_name and d[3] then
		...
		if not core.registered_nodes["stairs:slab_" .. item_name] then
		...
		core.register_node(":" .. node .. "_" .. cutname, {
...
core.register_tool("xdecor:hammer", {
