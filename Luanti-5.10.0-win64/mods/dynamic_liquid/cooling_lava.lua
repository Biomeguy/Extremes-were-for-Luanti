...
local new_lava_cooling = core.settings:get_bool("dynamic_liquid_new_lava_cooling", true)
if not new_lava_cooling then return end

local falling_obsidian = core.settings:get_bool("dynamic_liquid_falling_obsidian", false)
...
local particles = core.settings:get_bool("enable_particles", true)
...
local cool_lava_flowing = function(pos, node)
	local cooler_adjacent = core.find_node_near(pos, 1, dynamic_cools_lava_flowing)
	...
	if cooler_adjacent ~= nil then
		...
		local nearby_source = core.find_node_near(pos, 1, "default:lava_source")
		if nearby_source then
			core.set_node(pos, {name="default:lava_source"})
			core.set_node(nearby_source, {name="air"})
			steam(nearby_source)
		else
...
for name, node_def in pairs(core.registered_nodes) do
	...
	if core.get_item_group(name, "cools_lava") > 0 and name ~= "default:water_flowing" and name ~= "default:fresh_water_flowing" then
...
local cool_lava_source = function(pos, node)
	local cooler_list = core.find_nodes_in_area(
		vector.add(pos,{x=-1, y=-1, z=-1}),
		vector.add(pos,{x=1, y=1, z=1}),
	...
	if obsidian_location ~= nil then
		core.set_node(pos, {name = "air"})
		core.set_node(obsidian_location, {name = "default:obsidian"})
		if core.spawn_falling_node and falling_obsidian then -- TODO cutting-edge dev function, so check if it exists for the time being. Remove check when 0.4.16 is released.
			core.spawn_falling_node(obsidian_location)
		end
	elseif #evaporate_list > 0 then
		...
		if loc.y <= pos.y then
			core.set_node(pos, {name = "air"})
			core.set_node(loc, {name = "default:lava_source"})
		end
	end
