...
local disable_flow_above = tonumber(core.settings:get("dynamic_liquid_disable_flow_above"))
...
local water = core.settings:get_bool("dynamic_liquid_water", true)
local river_water = core.settings:get_bool("dynamic_liquid_river_water", false)
local lava = core.settings:get_bool("dynamic_liquid_lava", true)

local water_probability = tonumber(core.settings:get("dynamic_liquid_water_flow_propability"))
if water_probability == nil then
	water_probability = 1
end

local river_water_probability = tonumber(core.settings:get("dynamic_liquid_river_water_flow_propability"))
if river_water_probability == nil then
	river_water_probability = 1
end

local lava_probability = tonumber(core.settings:get("dynamic_liquid_lava_flow_propability"))
if lava_probability == nil then
	lava_probability = 5
end

local springs = core.settings:get_bool("dynamic_liquid_springs", true)

if water then
	-- override water_source and water_flowing with liquid_renewable set to false
	local override_def = {liquid_renewable = false}
	core.override_item("default:water_source", override_def)
	core.override_item("default:water_flowing", override_def)
end
...
if river_water then	
	dynamic_liquid.liquid_abm("default:fresh_water_source", "default:fresh_water_flowing", river_water_probability)
end

-- Flow-through nodes
-----------------------------------------------------------------------------------------------------------------------

local flow_through = core.settings:get_bool("dynamic_liquid_flow_through", true)
...
if flow_through then
	...
	core.register_abm({
		...
		nodenames = {"group:flow_through", "group:leaves", "group:sapling", "group:grass", "group:savanna_grass", "group:flora", "groups:rail", "groups:flower", "groups:water_flora"},
		...
		action = function(pos)
			...
			if source_flowing_node ~= nil then
				dest_node = core.get_node(dest_pos)
	...
	local add_flow_through = function(node_name)
		...
		if node_def == nil then
			core.log("warning", "dynamic_liquid attempted to call add_flow_through on the node name "
				.. node_name .. ", which was not found in core.registered_nodes.")
			return
		end
		local new_groups = node_def.groups
		new_groups.flow_through = 1
		core.override_item(node_name,{groups = new_groups})
	end
...
local duplicate_def = function (name)
	local old_def = core.registered_nodes[name]
...
core.register_node("dynamic_liquid:clay", clay_def)

local data = {}

if springs then	
	local c_clay = core.get_content_id("default:clay")
	local c_spring_clay = core.get_content_id("dynamic_liquid:clay")
	...
	core.register_node("dynamic_liquid:spring", {
...
local mapgen_prefill = core.settings:get_bool("dynamic_liquid_mapgen_prefill", true)
...
--		count = count + 1
--		if count % 100 == 0 then
--			core.chat_send_all("dropped water " .. (start-vi)/ystride .. " at " .. core.pos_to_string(area:position(vi)))
--		end
	end
...
local displace_liquid = core.settings:get_bool("dynamic_liquid_displace_liquid", true)
if displace_liquid then
	...
	local flood_search_outlet = function(start_pos, source, flowing)
		local start_node =  core.get_node(start_pos)
		...
		local visited = {}
		visited[core.hash_node_position(start_pos)] = true
