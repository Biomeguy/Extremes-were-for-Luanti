...
mobs:register_mob("frostyqueen:frostyqueen", {
	...
	drops = {
		...
		{name = "default:crystal_ingot", chance = 1, min = 1, max =2},
	...
	do_custom = function(self, dtime)
		...
		if math.random(0, 1) == 1 then
			core.add_particle({
				pos = {x=spos.x+math.random(-1,1)*math.random()/2,y=spos.y+math.random(0,3),z=spos.z+math.random(-1,1)*math.random()/2},
...
if core.get_modpath("awards") then
		for _,players in pairs(core.get_objects_inside_radius(pos,55)) do -- CONSEGUIR RADIUS ( POSIÇÃO ,55 NODES?)
...
mobs:spawn({
	...
	nodes = {"default:crystal_spike","ethereal:frost_leaves"},
	max_light = 7,
	interval = 60,
	chance = 300000,
	max_height = 200,
...
--[[
	...
	core.add_entity ( pos , "frozenskulls:frozenskulls")
	end
})
