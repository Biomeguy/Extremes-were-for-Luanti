...
function fishing_setting.func.save()
	...
	if input then
		input:write(core.serialize(fishing_setting.settings))
		input:close()
	else
		core.log("error", "open(" .. fishing_setting.file_settings .. ", 'w') failed: " .. err)
	end
end
...
function fishing_setting.func.ignore_mod(list)
	...
	for i,v in ipairs(list) do
		if core.get_modpath(v[1]) ~= nil then
...
function fishing_setting.func.hungry_random()
	...
	core.after(math.random(1, 12)*1800, fishing_setting.func.hungry_random )
end
...
function fishing_setting.func.notify(f_name, treasure)
	...
	for _, player in ipairs(core.get_connected_players()) do
		...
		if pname == f_name then
			core.chat_send_player(pname, S("You caught the treasure, %s!"):format(treasure[4]))
		else
			core.chat_send_player(pname, title)
		end
	end
end
...
fishing_setting.func.on_show_settings = function(pname)
	...
	core.show_formspec(pname, "fishing:settings", formspec)
end
...
function fishing_setting.func.load_trophies()
	...
	if file then
		fishing_setting.trophies = core.deserialize(file:read("*all"))
		file:close()
...
function fishing_setting.func.save_trophies()
	...
	if input then
		input:write(core.serialize(fishing_setting.trophies))
		input:close()
	else
		core.log("action","Open failed (mode:w) of " .. fishing_setting.file_trophies)
	end
end

core.register_on_shutdown(function()
	core.log("action", "[fishing] Server shuts down. saving trophies table")
...
core.register_on_joinplayer(function(player)
	...
	if fishing_setting.contest["contest"] == true then
		core.chat_send_player(pname, S("A fishing contest is in progress. (remaining time %s)"):format(fishing_setting.func.timetostr(fishing_setting.contest["duration"])))
	end
end)
...
function fishing_setting.func.add_to_trophies(player, fish, desc)
	...
	if string.find(fish, "_raw") ~= nil or fishing_setting.prizes["true_fish"]["little"][fish] or fishing_setting.prizes["true_fish"]["big"][fish] then
		if string.find(fish, "_raw") ~= nil then
			...
			fishing_setting.trophies[fish][pname] = (fishing_setting.trophies[fish][pname] or 0) + 1
			if fishing_setting.trophies[fish][pname]%100 == 0 then
				core.chat_send_player(pname, S("You win a new trophy, you have caught %s " .. fish.."."):format(fishing_setting.trophies[fish][pname]))
				...
				if inv:room_for_item("main", {name=name, count=1, wear=0, metadata=""}) then
					...
					core.spawn_item(player:get_pos(), {name=name, count=1, wear=0, metadata=""})
				end
			end
		end
		if fishing_setting.contest["contest"] ~= nil and fishing_setting.contest["contest"] == true then
			...
			fishing_setting.contest["nb_fish"][pname] = (fishing_setting.contest["nb_fish"][pname] or 0) + 1
			core.chat_send_all(S("Yeah, %s caught "..desc):format(pname))
		end
	end
end
...
fishing_setting.func.on_show_admin_menu = function(pname)
	...
	core.show_formspec(pname, "fishing:admin_conf", formspec)
end
	...
	unified_inventory.register_button("menu_fishing", {
		...
		action = function(player)
			...
			if core.check_player_privs(pname, {server=true}) then
				fishing_setting.func.on_show_admin_menu(pname)
			else
				local formspec = fishing_setting.func.get_stat()
				core.show_formspec(pname, "fishing:classement", formspec)
			end
...
function fishing_setting.func.save_contest()
	...
	if input then
		input:write(core.serialize(fishing_setting.contest))
		input:close()
	else
		core.log("action","Open failed (mode:w) of " .. fishing_setting.file_contest)
	end
end
...
function fishing_setting.func.start_contest(duration)
	...
	core.chat_send_all(S("Attention, Fishing contest start (duration %s)!!!"):format(duration))
	core.sound_play("fishing_contest_start",{gain=0.8})
	fishing_setting.func.save_contest()
...
function fishing_setting.func.end_contest()
	...
	fishing_setting.func.save_contest()
	core.chat_send_all(S("End of fishing contest."))
	core.sound_play("fishing_contest_end",{gain=0.8})
...
function fishing_setting.func.save_planned()
	...
	if input then
		input:write(core.serialize(fishing_setting.planned))
		input:close()
	else
		core.log("action","Open failed (mode:w) of " .. fishing_setting.file_planned)
	end
end

core.register_chatcommand("contest_add", {
...
core.register_chatcommand("contest_del", {
...
core.register_chatcommand("contest_show", {
	params = "",
...
core.register_chatcommand("contest_start", {
...
core.register_chatcommand("contest_stop", {
	params = "",
...
function fishing_setting.func.planned_tick()
		...
		for i, plan in pairs(fishing_setting.planned) do
			...
			if (wday == 0 or wday == time.wday) then
				if time.hour == hour and time.min == min then
					core.log("action", ("Starting fishing contest at %d:%d duration %d"):format( hour, min, duration))
					fishing_setting.contest["nb_fish"] = {}
					fishing_setting.func.start_contest(duration)
					break
				end
	...
	core.after(50, fishing_setting.func.planned_tick)
end

--Menu fishing configuration
fishing_setting.func.on_show_settings_contest = function(pname)
	if not fishing_setting.tmp_setting then
	...
	core.show_formspec(pname, "fishing:contest", formspec)
end
...
function fishing_setting.func.get_hunger_info(pname)
	...
	core.show_formspec(pname,"fishing:material_info", formspec)
end

core.register_chatcommand("fishing_menu", {
	params = "",
...
core.register_chatcommand("fishing_classement", {
	params = "",
...
function fishing_setting.func.tick()
	if fishing_setting.contest["contest"] ~= nil and fishing_setting.contest["contest"] == true then
		...
		if fishing_setting.contest["duration"] < 30 and fishing_setting.contest["warning_said"] ~= true then
			core.chat_send_all(fishing_setting.func.S("WARNING, Fishing contest will finish in 30 seconds."))
			fishing_setting.contest["warning_said"] = true
		end
		if fishing_setting.contest["duration"] < 0 then
			fishing_setting.func.end_contest()
		else
			core.after(UPDATE_TIME, fishing_setting.func.tick)
		end
	end
end
