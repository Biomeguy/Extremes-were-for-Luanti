local S = core.get_translator("hbarmor")
local N = function(s) return s end

if (not armor) or (not armor.def) then
	core.log("error", "[hbarmor] Outdated 3d_armor version. Please update your version of 3d_armor!")
end
...
local set = core.settings:get_bool("hbarmor_autohide")
if set ~= nil then
	hbarmor.autohide = set
end

set = core.settings:get("hbarmor_tick")
...
local function custom_hud(player)
	...
	if core.settings:get_bool("enable_damage") then
		...
		if ret == false then
			core.log("error", "[hbarmor] Call to hbarmor.get_armor in custom_hud returned with false!")
		end
		local arm = tonumber(hbarmor.armor[pname])
		if not arm then arm = 0 end
		local hide
		if hbarmor.autohide then
			hide = must_hide(pname, arm)
		else
			hide = false
		end
...
function hbarmor.get_armor(player)
	...
	local pname = player:get_player_name()
	local def = armor.def[pname] or nil
	if def and def.state and def.count then
		hbarmor.set_armor(pname, def.state, def.count)
	else
		return false
	end
	return true
...
local function update_hud(player)
	...
	if not arm then
		arm = 0
		hbarmor.armor[pname] = 0
	end
	if hbarmor.autohide then
		-- hide armor bar completely when there is none
		if must_hide(pname, arm) then
...
core.register_on_joinplayer(function(player)
	local pname = player:get_player_name()
	custom_hud(player)
	hbarmor.player_active[pname] = true
end)

core.register_on_leaveplayer(function(player)
	local pname = player:get_player_name()
	hbarmor.player_active[pname] = false
end)
	...
	if main_timer > hbarmor.tick or timer > 4 then
		if core.settings:get_bool("enable_damage") then
			...
			for _,player in ipairs(core.get_connected_players()) do
				local pname = player:get_player_name()
				if hbarmor.player_active[pname] == true then
					local ret = hbarmor.get_armor(player)
					if ret == false then
						core.log("error", "[hbarmor] Call to hbarmor.get_armor in globalstep returned with false!")
					end
