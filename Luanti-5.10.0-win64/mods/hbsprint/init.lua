...
local dir           = core.is_yes(setting_get("sprint_forward_only", "false"))
local particles     = tonumber(setting_get("sprint_particles", "2"))
local stamina       = core.is_yes(setting_get("sprint_stamina", "true"))
...
local starve        = core.is_yes(setting_get("sprint_starve", "true"))
...
local breath        = core.is_yes(setting_get("sprint_breath", "true"))
local breath_drain  = tonumber(setting_get("sprint_breath_drain", "1"))
local autohide      = core.is_yes(setting_get("hudbars_autohide_stamina", "true"))
...
local mod_hudbars = core.get_modpath("hudbars") ~= nil
...
if starve then
  if core.get_modpath("hbhunger") then
    starve = "hbhunger"
  elseif core.get_modpath("hunger_ng") then
    starve = "hunger_ng"
  else
    starve = false
  end
...
local function is_walkable(ground)
  local ground_def = core.registered_nodes[ground.name]
...
local function create_particles(player, name, ground)
  local def = core.registered_nodes[ground.name] or {}
...
local function sprint_step(player, dtime)
  local name = player:get_player_name()
  local fast = core.get_player_privs(name).fast
