...
core.register_abm({
	nodenames = {"hopper:hopper", "hopper:hopper_side"},
	neighbors = {"default:chest", "default:chest_locked", "protector:chest", "hopper:hopper",
		"hopper:hopper_side", "default:furnace", "default:furnace_active", "wine:wine_barrel"},
	interval = 1,
	chance = 1,
	catch_up = false,
	action = function(pos, node)
		...
		if node.name == "hopper:hopper_side" then
			...
			local face = core.get_node(pos).param2
		...
		local a = core.get_node({x = pos.x, y = pos.y + 1, z = pos.z}).name
		local b = core.get_node(front).name
		...
		or b == "default:furnace_active" then
			...
			-- re-start furnace timer
			core.get_node_timer(front):start(1.)
