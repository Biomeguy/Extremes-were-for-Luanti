...
local function inspect_pos(pos)
	...
	local fields = core.serialize(table.fields)
	desc = desc .. "==== meta ====\n"
	...
	return core.formspec_escape(desc:gsub(",", "\\,"))
end

core.register_tool("inspector:inspector", {
	...
	on_use = function(itemstack, user, pointd)
		...
		elseif pointd.type == "object" then
			...
			local ref = pointd.ref
			local entity = ref:get_luaentity()
			desc = core.formspec_escape(dump(entity):gsub("\n\n", "\n"))
		end

		core.show_formspec(user:get_player_name(), "inspector:inspector", make_fs(title, desc))
	end,
})

core.register_chatcommand("inspect", {
	params = "inspect",
	...
	func = function(name, param)
		...
		core.show_formspec(name, "inspector:inspector", make_fs("Node information", desc))
		return true
	end,
})
...
local function connects(connects_to,pos,opposite)
	local node=core.get_node(pos)
	local info=core.registered_nodes[node.name]
	if info then
...
local function get_boxes(pos)
	...
	local info=core.registered_nodes[name]
...
inspector={
	...
	slab=function(itemstack,placer,pointed,_)
		return core.item_place(itemstack,placer,pointed,
...
--level + ruler
core.register_craftitem("inspector:level",{
	description="Level (left click = check rotation, right click = get point)",
	inventory_image="pos_level.png",
	wield_image="pos_level.png^[transformR270",
	liquids_pointable=true,
	--left click: get rotation data from node
	on_use=function(_,user,pointd)
		local user_name=user:get_player_name()
		if pointd.under then
			local node=core.get_node(pointd.under)
			core.chat_send_player(user_name,"Node \""..node.name.."\" at "..core.pos_to_string(pointd.under))
			local node_info=core.registered_nodes[node.name]
			if node_info then
				if node_info.paramtype2=="wallmounted" then
					core.chat_send_player(user_name,"Facing: "..(({[0]="y-","Y+","x-","X+","z-","Z+"})[node.param2]))
				elseif node_info.paramtype2=="facedir" then
					core.chat_send_player(user_name,"Axis: "..(({[0]="Y+","Z+","z-","X+","x-","y-"})[math.floor(node.param2/4)]))
					core.chat_send_player(user_name,"Rotation: "..(node.param2%4*90).."°")
				else
					core.chat_send_player(user_name,"(no rotation)")
				end
			end
		end
	end,
	--right click: measure location of clicked point
	on_place=function(_,placer,pointd)
		local user_name=placer:get_player_name()
		local normal,p = inspector.get_point(pointd.above, pointd.under, placer)
		if p then
			core.chat_send_player(user_name,"Position within node:")
			core.chat_send_player(user_name,"X: "..p.x)
			core.chat_send_player(user_name,"Y: "..p.y)
			core.chat_send_player(user_name,"Z: "..p.z)
		else
			core.chat_send_player(user_name,"Could not get point")
		end
	end,
})

dofile(core.get_modpath("inspector").."/wires.lua")
...
local function count_items()
	...
	core.chat_send_all("There are ".. number .." registered nodes, items and tools.")
end
