...
-- called on use bag
function inventorybags.on_use_bag(itemstack, user, pointd)
	return itemstack, user, pointd
end
...
local function save_bag_inv_itemstack(inv, stack)
	...
	meta:set_string("inventorybags_inv_content", core.serialize(inv_to_table(inv)))
	return stack
end

local function save_bag_inv(inv, player)
	local playerinv = core.get_inventory{type="player", name=player:get_player_name()}
...
local function open_bag(itemstack, user, width, height, sound)
	...
	if core.get_item_group(itemstack:get_name(), "bag_bag") > 0 then
	...
	if invname == "" then
		...
		if item_count > 1 then
			...
			core.after(0.01, stack_to_player_inv, newitemstack, user)
		end
	...
	local inv = core.create_detached_inventory(invname, {
		allow_put = function(inv, listname, index, stack, player)
			if allow_bag_input then
				if core.get_item_group(stack:get_name(), "bag_bag") > 0 then
					return 0
				end
			else
			...
			for i = 1, size, 1 do
				...
				if remove_stack == true then
					...
					if playerinv:room_for_item("main", stack) then
						...
						core.item_drop(save_bag_inv_itemstack(inv, stack), player, player:get_pos())
						core.close_formspec(player:get_player_name(), inv:get_location().name)
					end
				end
			end
			save_bag_inv(inv, player)
	...
	if invmetastring ~= "" then
		table_to_inv(inv, core.deserialize(invmetastring))
	...
	if sound then
		core.sound_play(sound, {gain = 0.8, object = user, max_hear_distance = 5})
	end
	core.show_formspec(playername, invname, get_formspec(invname, width, height))
	return itemstack
end
...
function inventorybags.register_bag(name, bagtable)
	core.register_craftitem(name, {
		...
		on_use = function(itemstack, user, pointd)
			return inventorybags.on_use_bag(itemstack, user, pointd)
		end,
		...
		if nisformn == 1 then
			if fields.quit then
				...
				if bagtable.sound_close then
					core.sound_play(sound, {gain = 0.8, object = player, max_hear_distance = 5})
				end
			end
		end
		return
...
core.register_craftitem("inventorybags:bag_transporting_bag", {
...
if core.settings:get_bool("inventorybags_enable_item_teleportation_bag") then
	...
	local function open_teleport_bag_inv(itemstack, placer, pointd)
		local location = core.deserialize(itemstack:get_meta():get_string("inventorybags_formspec_location"))
		if location then
			core.sound_play("inventorybags_open_teleportation_bag", {gain = 0.8, object = placer, max_hear_distance = 5})
			local node = core.get_node(location)
			-- load the area
			local vmie, vmae = VoxelManip():read_from_map(location, location)
			if not node then
				core.chat_send_player(placer:get_player_name(), "Error: area not loaded")
				return itemstack
			end
			local rkfunction = core.registered_nodes[node.name].on_rightclick
			if rkfunction then
				rkfunction(location, node, placer, itemstack, pointd)
			else
				local nodemeta = core.get_meta(location)
				...
				if formspec then
					...
					core.show_formspec(placer:get_player_name(), "inventorybags:item_teleportation_bag", formspec)
				end
			end
		end
		return itemstack
	...
	core.register_craftitem("inventorybags:item_teleportation_bag", {
		...
		on_use = function(itemstack, user, pointd)
			if user:get_player_control().sneak == true then
				if pointd and pointd.type == "node" then
					itemstack:get_meta():set_string("inventorybags_formspec_location", core.serialize(pointd.under))
					core.chat_send_player(user:get_player_name(), "Item Teleportation Bag target set to: "..
						pointd.under.x..", "..pointd.under.y..", "..pointd.under.z..
						" ("..core.get_node(pointd.under).name..")"
					)
				end
			else
				core.chat_send_player(user:get_player_name(), "Hold sneak and left click to set the target of your Item Teleportation Bag")
			end
			return itemstack
		end,

		on_secondary_use = function(itemstack, user, pointd)
			return open_teleport_bag_inv(itemstack, user, pointd)
		end,

		on_place = function(itemstack, placer, pointd)
			return open_teleport_bag_inv(itemstack, placer, pointd)
		end,
	})
	...
	core.register_on_player_receive_fields(function(player, formname, fields)
		...
		if nisformn == 1 then
			if fields.quit then
				core.sound_play("inventorybags_close_teleportation_bag", {gain = 0.8, object = player, max_hear_distance = 5})
			end
		end
		return
	end)
...
if core.get_modpath("xdecor") then
	core.register_craftitem("inventorybags:ender_bag", {
		description = "Ender Bag",
		inventory_image = "inventorybags_ender_bag.png",
		groups = {bag = 1},
		
		on_secondary_use = function(itemstack, user)
			core.sound_play("inventorybags_open_teleportation_bag", {gain = 0.8, object = user, max_hear_distance = 5})
			core.show_formspec(user:get_player_name(), "inventorybags:ender_bag",
				"size[8,9]" ..
		...
		on_place = function(itemstack, placer, pointd)
			core.sound_play("inventorybags_open_teleportation_bag", {gain = 0.8, object = placer, max_hear_distance = 5})
			core.show_formspec(placer:get_player_name(), "inventorybags:ender_bag",
				"size[8,9]" ..
				"list[current_player;enderchest;0,0.3;8,4;]" ..
				"list[current_player;main;0,4.85;8,1;]" ..
				"list[current_player;main;0,6.08;8,3;8]" ..
...
if core.get_modpath("more_chests") then
	core.register_craftitem("inventorybags:wifi_bag", {
		...
		on_secondary_use = function(itemstack, user)
			core.sound_play("inventorybags_open_teleportation_bag", {gain = 0.8, object = user, max_hear_distance = 5})
			core.show_formspec(user:get_player_name(), "inventorybags:wifi_bag",
				"size[8,9]" ..
		...
		on_place = function(itemstack, placer, pointd)
			core.sound_play("inventorybags_open_teleportation_bag", {gain = 0.8, object = placer, max_hear_distance = 5})
			core.show_formspec(placer:get_player_name(), "inventorybags:wifi_bag",
				"size[8,9]" ..
				"list[current_player;more_chests:wifi;0,0.3;8,4;]" ..
				"list[current_player;main;0,4.85;8,1;]" ..
				"list[current_player;main;0,6.08;8,3;8]" ..
...
if core.get_modpath("beds") then
	core.register_craftitem("inventorybags:sleeping_bag", {
		description = "Sleeping Bag",
		inventory_image = "inventorybags_sleeping_bag.png",
		groups = {bag = 1},
		
		on_secondary_use = function(itemstack, user)
			core.sound_play("inventorybags_sleeping", {gain = 1, object = user, max_hear_distance = 5})
			beds.on_rightclick(user:get_pos(), user)
		end,
		on_place = function(itemstack, placer, pointd)
			core.sound_play("inventorybags_sleeping", {gain = 1, object = placer, max_hear_distance = 5})
...
if not core.settings:get_bool("inventorybags_dialable_bag_of_winds") then
	...
	local function set_physics_normal_inv_check(player, extratable)
		if player:get_wielded_item():get_name() == "inventorybags:bag_of_winds_opened" then
			core.after(1, set_physics_normal_inv_check, player, extratable)
		else
			set_physics_normal(player)
			core.sound_stop(extratable.handle)
			...
			while inv:contains_item("main", "inventorybags:bag_of_winds_opened") do
				...
				core.item_drop(itemstack, player, player:get_pos())
			end
			...
			if lostbag then
				core.chat_send_player(player:get_player_name(), "You lost your Bag of Winds.")
			end
		end
	end
	...
	local function set_new_physics(player, itemstack)
		local physics = player:get_physics_override()
		local extratable = {}
		extratable.handle = core.sound_play("inventorybags_wind", {gain = 1, object = player, max_hear_distance = 5, loop = true})
	...
	core.register_craftitem("inventorybags:bag_of_winds_closed", {
	...
	core.register_craftitem("inventorybags:bag_of_winds_opened", {
		description = "Bag of Winds",
