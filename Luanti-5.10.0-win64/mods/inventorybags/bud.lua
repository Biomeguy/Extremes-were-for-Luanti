...
local function open_bud(user)
	...
	local invname = "inventorybags_"..pname.."_bud"
	local inv = core.get_inventory({type="player", name=pname})
	local setting_default = core.formspec_escape(user:get_attribute("inventorybags_setting_default")) or ""
	inv:set_size(invname.."_bag", 1)
	inv:set_size(invname.."_upgrade", 1)
	inv:set_size(invname.."_out", 1)
	core.show_formspec(pname, invname,
		"size[8,7]" ..
		default.gui_bg ..
		default.gui_bg_img ..
		default.gui_slots ..
...
core.register_craftitem("inventorybags:bud", {
...
local function set_crafting_output(inv)
	...
	inv:set_stack("output", 1, core.get_craft_result(input).item)
end

local function show_setting_creator(player, setting_type, extra_table)
	local pname = player:get_player_name()
	local invname = "inventorybags_"..pname.."_bud_setting_creator"
	local inv = core.get_inventory({type="player", name=pname})
	local setting = player:get_attribute("inventorybags_setting_default")
	local setting_default = core.formspec_escape(setting) or ""
	...
	if setting_type == "color" then
		...
		core.show_formspec(pname, invname.."_color",
			"size[8,9]" ..
			default.gui_bg ..
			default.gui_bg_img ..
			default.gui_slots ..
	...
	elseif setting_type == "filter" then
		local finv = core.create_detached_inventory(invname.."_filter", {
		...
		local setting_table = core.deserialize(setting) or {}
		...
		core.show_formspec(pname, invname.."_filter",
			"size[12,11]" ..
			default.gui_bg ..
			default.gui_bg_img ..
			default.gui_slots ..
			...
			"label[0.2,0.4;"..core.formspec_escape('comma separated lists for example: "moreblocks,default,technic"').."]"..
			"label[6.2,0.4;"..core.formspec_escape('comma separated lists for example: "tree,stone,sand,stick"').."]"..
	...
	elseif setting_type == "crafting" then
		local craftinv = core.create_detached_inventory(invname.."_crafting", {
			allow_move = function(inv, from_list, from_index, to_list, to_index, count, player)
				inv:set_stack(to_list, to_index, inv:get_stack(from_list, from_index))
				inv:set_stack(from_list, from_index, "")
		...
		core.show_formspec(pname, invname.."_crafting",
			"size[8,9]" ..
			default.gui_bg ..
			default.gui_bg_img ..
			default.gui_slots ..
...
local function save_filter_formspec(fields, setting_table, player, finv, extra_table)
	...
	player:set_attribute("inventorybags_setting_default", core.serialize(setting_table))
end
...
core.register_on_player_receive_fields(function(player, formname, fields)
	...
	local budinvname = "inventorybags_"..pname.."_bud"
	if formname == budinvname then
		...
		elseif fields.help then
			...
			core.show_formspec(pname, formname.."_help",
				"size[10,8]" ..
				default.gui_bg ..
				default.gui_bg_img ..
				default.gui_slots ..
		...
		elseif fields.upgrade then
			if inv:is_empty(budinvname.."_out") and bagstack ~= nil and upgradestack ~= nil and
					...
					core.get_item_group(bagstack:get_name(), "bag") > 0 and
					core.get_item_group(upgradestack:get_name(), "inventorybags_upgrade") > 0 then
				local meta = bagstack:get_meta()
				local bud_recipe = inventorybags.bud_recipes[upgradestack:get_name()]
				if bud_recipe and meta:get_string(bud_recipe.meta_name) ~= bud_recipe.meta_string then
					...
					core.sound_play("inventorybags_bud_upgrade", {gain = 0.8, object = player, max_hear_distance = 5})
			...
			if inv:is_empty(budinvname.."_out") and 
				...
				if cpos then
					...
					if bud_recipe and inv:room_for_item(budinvname.."_upgrade", upgrade_name) then
						...
						core.sound_play("inventorybags_bud_extract", {gain = 0.8, object = player, max_hear_distance = 5})
	...
	elseif formname == budinvname.."_help" then
		if fields.quit then
			core.after(0.1, open_bud, player)
		end
	elseif formname == budinvname.."_setting_creator_color" then
		if fields.color_list then
			local fieldstable = core.explode_textlist_event(fields.color_list)
	...
	elseif formname == budinvname.."_setting_creator_filter" then
		local finv = core.get_inventory({type="detached", name=budinvname.."_setting_creator_filter"})
		local setting_table = {}
		if fields.setting then 
			setting_table = core.deserialize(fields.setting) or {}
		end
	...
	elseif formname == budinvname.."_setting_creator_crafting" then
		local craftinv = core.get_inventory({type="detached", name=budinvname.."_setting_creator_crafting"})
		local setting_table = {}
		if fields.setting then 
			setting_table = core.deserialize(fields.setting) or {}
		end
