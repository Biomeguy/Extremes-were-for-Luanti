...
local function register_upgrade(data)
	...
	core.register_craftitem(data.name, {
...
local function match_filter(filter_table, stackname)
	...
	if type(filter_table.group_filter_blacklist) == "string" and filter_table.group_filter_blacklist ~= "" then
		for _, group in pairs(string_list_to_table(filter_table.group_filter_blacklist)) do
			if core.get_item_group(stackname, group) > 0 then
				blacklist_match = blacklist_match + 1
			end
		end
	end
...
function inventorybags.on_open_bag(bagstack, baginv, player)
	...
	if meta:get_string("inventorybags_advanced_collecting_upgrade") == "true" then
		for k = 1, size, 1 do
			...
			if astack and core.get_item_group(astack:get_name(), "bag") == 0 then
				local fadd_item = true
				local filter_table = core.deserialize(meta:get_string("inventorybags_advanced_collecting_filter"))
...
core.register_on_dignode(function(pos, oldnode, digger)
	...
	local drops = core.get_node_drops(oldnode.name, digger:get_wielded_item():get_name())
	local size = inv:get_size("main")
		...
		if core.get_item_group(stack:get_name(), "bag") > 0 then
			if meta:get_string("inventorybags_collecting_upgrade") == "true" and not done_coll then
				for drn, drop in pairs(drops) do
					if inv:contains_item("main", drop) and core.get_item_group(drop, "bag") == 0 then
						local fadd_item = true
						local filter_table = core.deserialize(meta:get_string("inventorybags_collecting_filter"))
						if filter_table then
...
function inventorybags.on_change_bag_inv(bagstack, baginv)
	...
	if meta:get_string("inventorybags_dumping_upgrade") == "true" then
		...
		local filter_table = core.deserialize(meta:get_string("inventorybags_dumping_filter"))
		if filter_table then
			for i = 1, size, 1 do
...
local function make_max_crafting(inv, recipes)
	...
	for j, recipe in pairs(recipes) do
		local craft_result = core.get_craft_result({items = recipe, method = "normal", width = 3}).item
	...
	while nrecipes > 0 do
		...
		local craft_results = core.get_craft_result({items = recipes[nrecipes], method = "normal", width = 3})
...
function inventorybags.on_change_bag_inv(bagstack, baginv)
	...
	if meta:get_string("inventorybags_autocrafting_upgrade") == "true" then
		local setting_table = core.deserialize(meta:get_string("inventorybags_autocrafting_recipes"))
...
core.register_on_placenode(function(pos, newnode, placer, oldnode, itemstack, pointd)
	...
	for i = 1, size, 1 do
		...
		local meta = bagstack:get_meta()
		if core.get_item_group(bagstack:get_name(), "bag") > 0 and
...
if core.get_modpath("tnt") and enable_tnt then
	...
	function inventorybags.on_drop_bag(itemstack, dropper, pos)
		...
		if meta:get_string("inventorybags_explosion_upgrade") == "true" then
			...
			core.after(dtime, tnt.boom, pos, def)
			core.add_particle({
				pos = pos,
				expirationtime = dtime,
				size = def.radius * 2,
				texture = core.registered_items[itemname].inventory_image..color,
			})
	...
	function inventorybags.on_use_bag(itemstack, user, pointd)
		...
		return old_on_use_bag(itemstack, user, pointd)
	end
end
