...
local function legacy_setting_getbool(name_new, name_old, default)
	local v = core.settings:get_bool(name_new)
	if v == nil then
		v = core.settings:get_bool(name_new)
	end
	if default then
		return v ~= false
	end
...
local function legacy_setting_getnumber(name_new, name_old, default)
	return tonumber(core.settings:get(name_new))
		or tonumber(core.settings:get(name_old))
		or default
end

if legacy_setting_getbool("item_drop.enable_item_pickup", "enable_item_pickup", true) then
	local pickup_gain = legacy_setting_getnumber("item_drop.pickup_sound_gain", "item_pickup_gain", 0.2)
	local pickup_particle = core.settings:get_bool("item_drop.pickup_particle", true)
	local pickup_radius = legacy_setting_getnumber("item_drop.pickup_radius", "item_pickup_radius", 0.75)
	local magnet_radius = tonumber(core.settings:get("item_drop.magnet_radius")) or -1
	local magnet_time = tonumber(core.settings:get("item_drop.magnet_time")) or 5.0
	local pickup_age = tonumber(core.settings:get("item_drop.pickup_age")) or 0.5
	local key_triggered = legacy_setting_getbool("item_drop.enable_pickup_key", "enable_item_pickup_key", true)
	local key_invert = core.settings:get_bool("item_drop.pickup_keyinvert") ~= false
	local keytype
	if key_triggered then
		keytype = core.settings:get("item_drop.pickup_keytype") or
		core.settings:get("item_pickup_keytype") or "Use"
		-- disable pickup age if picking is explicitly enabled by the player
		if not key_invert then
			pickup_age = math.min(pickup_age, 0)
		end
	end
	local mouse_pickup = core.settings:get_bool(
	...
	local function collect_item(ent, pos, player)
		...
		local entname = ItemStack(ent.itemstring):to_table().name
		if core.get_item_group(entname, "rupee") ~= 0 then
	core.sound_play("rupee", {pos = pos, gain = pickup_gain})
		else
	core.sound_play("item_drop_pickup", {pos = pos, gain = pickup_gain})
		end
		if pickup_particle then
			local item = core.registered_nodes[
	...
	local function pickupfunc(player)
		...
		or not core.get_player_privs(player:get_player_name()).interact
		...
		local objectlist = core.get_objects_inside_radius(pos,
			...
			and item_drop.can_pickup(ent, player) then
				...
				if inv:room_for_item("main", item) then
					...
					if not ent.is_magnet_item then
						...
						core.after(magnet_time, afterflight,
							object, inv, player)
					end
				end
			end
...
and not core.settings:get_bool("creative_mode") then
	...
	function core.handle_node_drops(pos, drops, player)
		...
		for i = 1,#drops do
			...
			if name == ""
			or not core.registered_items[name] then
				count = 0
			end
