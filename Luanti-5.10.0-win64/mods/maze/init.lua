core.register_chatcommand("maze", {
	...
	func = function(name, param)
		...
		if not core.registered_nodes[material_floor] then material_floor = "default:cobble" end
		if material_floor == nil then material_floor = "default:cobble" end
		if not core.registered_nodes[material_wall] then material_wall = "default:stone_brick" end
		if material_wall == nil then material_wall = "default:stone_brick" end
		if not core.registered_nodes[material_ceiling] then material_ceiling = "default:diamite" end
		if material_ceiling == nil then material_ceiling = "default:diamite" end

core.chat_send_player(name, "Try to build " .. maze_size_x .. " * " .. maze_size_y .. " * " .. maze_size_l .. " maze.  F:" .. material_floor .. " W:" .. material_wall .. " C:" .. material_ceiling)
...
-- get transform factors to place the maze in "look_dir" of player
		local player_dir = core.get_player_by_name(name):get_look_dir()
		local cosine = 1
		local sine = 0
...
-- build maze in core-world
			...
			for y = 0, maze_size_y - 1, 1 do
				...
				for x = 0, maze_size_x - 1, 1 do
					...
					if not change_level_down then 
						core.set_node(pos, {name = material_floor})
					end
					if maze[l][x][y] then 
						line = "X" .. line
						pos.y = pos.y + 1
						core.set_node(pos, {name = material_wall})
						pos.y = pos.y + 1
						core.set_node(pos, {name = material_wall})
					else
						line = " " .. line
						...
						-- if change_level_down then core.set_node(pos, {name = "default:ladder", param2 = ladder_direction}) end
						if change_level_up then core.set_node(pos, {name = "default:ladder", param2 = ladder_direction}) end
						pos.y = pos.y + 1
						core.set_node(pos, {name = "air"})
						if change_level_up then 
							core.set_node(pos, {name = "default:ladder", param2 = ladder_direction}) 
						else 
							if change_level_down then 
								core.set_node(pos, {name = "default:meselamp_torch_wall", param2 = ladder_direction})
							elseif (math.random(20) == 1) then 
								core.set_node(pos, {name = "default:meselamp_torch", param2 = 6}) 
							end
						end
					end
					pos.y = pos.y + 1
					if change_level_up then 
						...
						core.set_node(pos, {name = material_ceiling})
					end
				end
		...
		local is_air  = core.get_node_or_nil(pos)
		while is_air ~= nil and is_air.name ~= "air" do
			core.set_node(pos, {name = "air"})
			core.set_node(pos, {name = "default:ladder", param2 = ladder_direction})
			pos.y = pos.y + 1
			is_air  = core.get_node_or_nil(pos)
		end
...
-- place a closer-stone to seal the entrance and exit
		...
		core.set_node(pos, {name = "maze:closer"})
...
core.register_node("maze:closer", {
			...
			if dist<3 then -- 2.2 would be enough, just make sure
				...
				elseif dist > 1 then -- 0.71 would be enough, at least one node away
					if meta:get_string("trap") == "triggered" then
						...
						core.set_node({x = pos.x, y = pos.y + 1, z = pos.z},{name="default:stone_brick"})
						core.set_node({x = pos.x, y = pos.y + 2, z = pos.z},{name="default:stone_brick"})
					end
				end
			end
		end
