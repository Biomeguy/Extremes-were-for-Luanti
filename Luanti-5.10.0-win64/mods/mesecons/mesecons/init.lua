...
--
--
-- This mod adds mesecons[=minecraft redstone] and different receptors/effectors to luanti.
...
-- Settings
dofile(core.get_modpath("mesecons").."/settings.lua")

-- Utilities like comparing positions,
-- adding positions and rules,
-- mostly things that make the source look cleaner
dofile(core.get_modpath("mesecons").."/util.lua");

-- Presets (eg default rules)
dofile(core.get_modpath("mesecons").."/presets.lua");

-- The ActionQueue
-- Saves all the actions that have to be execute in the future
dofile(core.get_modpath("mesecons").."/actionqueue.lua");
...
dofile(core.get_modpath("mesecons").."/internal.lua");
...
mesecon.queue:add_function("receptor_off", function (pos, rules)
	...
	for _, rule in ipairs(mesecon.flattenrules(rules)) do
		...
		for _, rulename in ipairs(rulenames) do
			mesecon.vm_begin()
			mesecon.changesignal(np, core.get_node(np), rulename, mesecon.state.off, 2)
...
dofile(core.get_modpath("mesecons").."/legacy.lua");

--Services like turnoff receptor on dignode and so on
dofile(core.get_modpath("mesecons").."/services.lua");
