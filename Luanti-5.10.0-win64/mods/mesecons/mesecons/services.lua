...
function mesecon.on_blastnode(pos, intensity)
	...
	return core.get_node_drops(node.name, "")
end

core.register_on_placenode(mesecon.on_placenode)
core.register_on_dignode(mesecon.on_dignode)
...
function mesecon.do_overheat(pos)
	...
	if heat >= OVERHEAT_MAX then
		core.log("action", "Node overheats at " .. core.pos_to_string(pos))
		object_heat[id] = nil
		return true
	end
	return false
...
function mesecon.move_hot_nodes(moved_nodes)
	...
	for _, n in ipairs(moved_nodes) do
		local old_id = core.hash_node_position(n.oldpos)
		local new_id = core.hash_node_position(n.pos)
...
core.register_globalstep(global_cooldown)
