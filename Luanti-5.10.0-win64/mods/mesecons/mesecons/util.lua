function mesecon.move_node(pos, newpos)
	...
	core.set_node(newpos, node)
	core.get_meta(pos):from_table(meta)
end
...
function mesecon.register_node(name, spec_common, spec_off, spec_on)
	...
	core.register_node(name .. "_on", spec_on)
	core.register_node(name .. "_off", spec_off)
end
...
function mesecon.flipstate(pos, node)
	...
	core.swap_node(pos, {name = nodedef.__mesecon_basename .. "_" .. newstate,
		param2 = node.param2})
...
function mesecon.vm_swap_node(pos, name)
	local tbl = vm_get_or_create_entry(pos)
	local index = tbl.va:indexp(pos)
	tbl.data[index] = core.get_content_id(name)
	tbl.dirty = true
end
