...
core.register_chatcommand("tell", {
	...
	func = function(name, param)
		...
		if not core.get_player_by_name(target) then
			core.chat_send_player(name, "Invalid target: " .. target)
		end
		core.chat_send_player(target, name .. " whispers: " .. message, false)
	end
})
...
local function initialize_data(meta)
	local commands = core.formspec_escape(meta:get_string("commands"))
...
local function commandblock_action_on(pos, node)
	...
	core.swap_node(pos, {name = "mesecons_commandblock:commandblock_on"})
	...
	for _, command in pairs(commands:split("\n")) do
		...
		if not cmddef then
			core.chat_send_player(owner, "The command "..cmd.." does not exist")
			return
		end
		local has_privs, missing_privs = core.check_player_privs(owner, cmddef.privs)
		if not has_privs then
			core.chat_send_player(owner, "You don't have permission "
					.."to run "..cmd
					.." (missing privileges: "
...
local function commandblock_action_off(pos, node)
	if node.name == "mesecons_commandblock:commandblock_on" then
		core.swap_node(pos, {name = "mesecons_commandblock:commandblock_off"})
	end
end
...
core.register_node("mesecons_commandblock:commandblock_off", {
	description = "Command Block",
	tiles = {"jeija_commandblock_off.png"},
	inventory_image = core.inventorycube("jeija_commandblock_off.png"),
...
core.register_node("mesecons_commandblock:commandblock_on", {
