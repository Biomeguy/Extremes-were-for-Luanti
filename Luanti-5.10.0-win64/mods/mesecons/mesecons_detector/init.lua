...
local function object_detector_scan(pos)
	local objs = core.get_objects_inside_radius(pos, mesecon.setting("detector_radius", 6))

	-- abort if no scan results were found
	if next(objs) == nil then return false end

	local scanname = core.get_meta(pos):get_string("scanname")
	local scan_for = {}
...
core.register_node("mesecons_detector:object_detector_off", {
...
core.register_node("mesecons_detector:object_detector_on", {
...
local function node_detector_scan(pos)
	...
	local frontname = core.get_node(
		vector.subtract(pos, vector.multiply(core.facedir_to_dir(node.param2), distance + 1))
	).name
...
local function after_place_node_detector(pos, placer)
	...
	node.param2 = core.dir_to_facedir(vector.subtract(pos, placer_pos), true)
	core.set_node(pos, node)
end

core.register_node("mesecons_detector:node_detector_off", {
...
core.register_node("mesecons_detector:node_detector_on", {
...
core.register_abm({
	nodenames = {"mesecons_detector:object_detector_off", "mesecons_detector:object_detector_on",
		"mesecons_detector:node_detector_off", "mesecons_detector:node_detector_on"},
	interval = 1,
	chance = 1,
	action = function(pos, node)
		if node.name == "mesecons_detector:object_detector_off" then
			if not object_detector_scan(pos) then return end
			node.name = "mesecons_detector:object_detector_on"
			core.swap_node(pos, node)
			mesecon.receptor_on(pos, mesecon.rules.pplate)
		end
		if node.name == "mesecons_detector:object_detector_on" then
		...
		if node.name == "mesecons_detector:node_detector_off" then
			if not node_detector_scan(pos) then return end
			node.name = "mesecons_detector:node_detector_on"
			core.swap_node(pos, node)
			mesecon.receptor_on(pos)
		end
		if node.name == "mesecons_detector:node_detector_on" then
