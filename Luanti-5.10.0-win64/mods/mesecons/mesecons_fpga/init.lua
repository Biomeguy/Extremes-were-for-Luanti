...
local lcore = dofile(core.get_modpath(core.get_current_modname()) .. "/logic.lua")
dofile(core.get_modpath(core.get_current_modname()) .. "/tool.lua")(plg)
...
plg.register_nodes = function(template)
	...
	for d = 0, 1 do
		...
		core.register_node(nodename, ndef)
	end
	end
	end
	end
...
plg.update_formspec = function(pos, is)
	...
	if err == nil then
		...
		meta:set_int("valid", 0)
		meta:set_string("infotext", "FPGA")
		local fmsg = core.colorize("#ff0000", core.formspec_escape(err.msg))
...
plg.getports = function(pos) -- gets merged states of INPUT & OUTPUT
	...
	local s = core.get_meta(pos):get_string("portstates")
	if s == nil then
...
plg.setports = function(pos, A, B, C, D) -- sets states of OUTPUT
	...
	core.swap_node(pos, {name = name, param2 = core.get_node(pos).param2})
