...
local function set_gate(pos, node, state)
	local gate = core.registered_nodes[node.name]

	if mesecon.do_overheat(pos) then
		core.remove_node(pos)
		mesecon.receptor_off(pos, gate_get_output_rules(node))
		core.add_item(pos, gate.drop)
	elseif state then
		core.swap_node(pos, {name = gate.onstate, param2=node.param2})
		mesecon.receptor_on(pos, gate_get_output_rules(node))
	else
		core.swap_node(pos, {name = gate.offstate, param2=node.param2})
		mesecon.receptor_off(pos, gate_get_output_rules(node))
	end
end
...
local function register_gate(name, inputnumber, assess, recipe, description)
	...
	core.register_craft({output = basename .."_off", recipe = recipe})
end
...
register_gate("and", 2, function (val1, val2) return val1 and val2 end,
	{{"mesecons:mesecon", "", ""},
	 {"", "basic_materials:silicon", "mesecons:mesecon"},
	 {"mesecons:mesecon", "", ""}},
	"AND Gate")

register_gate("nand", 2, function (val1, val2) return not (val1 and val2) end,
	{{"mesecons:mesecon", "", ""},
	 {"", "basic_materials:silicon", "mesecons_torch:mesecon_torch_on"},
	 {"mesecons:mesecon", "", ""}},
	"NAND Gate")
