...
core.register_node(nodename, {
	...
	on_receive_fields = function(pos, formanme, fields, sender)
		...
		if core.is_protected(pos, pname) and
				not core.check_player_privs(pname, {protection_bypass=true}) then
		...
		meta:set_string("formspec", "size[9,2.5]"..
		"field[0.256,-0.2;9,2;code;Code:;"..core.formspec_escape(fields.code).."]"..
		"button[0  ,0.2;1.5,3;band;AND]"..
		"button[1.5,0.2;1.5,3;bxor;XOR]"..
		"button[3  ,0.2;1.5,3;bnot;NOT]"..
		"button[4.5,0.2;1.5,3;bnand;NAND]"..
...
if core.get_modpath("mesecons_luacontroller") then
	...
	core.register_craft({
		...
		recipe = {
			{"basic_materials:silicon", "basic_materials:silicon", "group:mesecon_conductor_craftable"},
			{"basic_materials:silicon", "basic_materials:silicon", "group:mesecon_conductor_craftable"},
...
yc.update = function(pos)
	...
	if (mesecon.do_overheat(pos)) then
		...
		core.add_item(pos, "mesecons_microcontroller:microcontroller0000")
	end
...
yc.parsecode = function(code, pos)
	...
	while true do
		...
		core.get_meta(pos):set_string("eeprom", eeprom) end
	end
...
yc.command_after = function(params, pos)
	...
	core.after(time, yc.command_after_execute, {pos = pos, code = code, afterid = afterid})
	return true
end

yc.command_after_execute = function(params)
	local meta = core.get_meta(params.pos)
