...
function mesecon.mvps_process_stack(stack)
	...
	for _, n in ipairs(stack) do
		mesecon.on_placenode(n.pos, core.get_node(n.pos))
	end
end
...
local function node_replaceable(name)
	if core.registered_nodes[name] then
		return core.registered_nodes[name].buildable_to or false
	end
...
function mesecon.mvps_get_stack(pos, dir, maximum, all_pull_sticky)
	...
	while #frontiers > 0 do
		...
		if not node_replaceable(nn.name) then
			...
			local connected = {}
			if core.registered_nodes[nn.name]
			and core.registered_nodes[nn.name].mvps_sticky then
				connected = core.registered_nodes[nn.name].mvps_sticky(np, nn)
			end
			...
			for _, r in ipairs(mesecon.rules.alldirs) do
				...
				local adjnode = core.get_node(adjpos)
				if core.registered_nodes[adjnode.name]
				and core.registered_nodes[adjnode.name].mvps_sticky then
					local sticksto = core.registered_nodes[adjnode.name]
...
function mesecon.mvps_push_or_pull(pos, stackdir, movedir, maximum, all_pull_sticky)
	...
	for _, n in ipairs(nodes) do
		n.meta = core.get_meta(n.pos):to_table()
		local node_timer = core.get_node_timer(n.pos)
		if node_timer:is_started() then
			n.node_timer = {node_timer:get_timeout(), node_timer:get_elapsed()}
		end
		core.remove_node(n.pos)
	end
	...
	for _, n in ipairs(nodes) do
		...
		core.set_node(np, n.node)
		core.get_meta(np):from_table(n.meta)
		if n.node_timer then
			core.get_node_timer(np):set(unpack(n.node_timer))
		end
	end
...
function mesecon.mvps_move_objects(pos, dir, nodestack, movefactor)
	...
	for id, obj in pairs(core.object_refs) do
...
mesecon.register_on_mvps_move(function(moved_nodes)
	for i = 1, #moved_nodes do
		...
		core.after(0, function()
			core.check_for_falling(moved_node.oldpos)
