core.register_node("mesecons_noteblock:noteblock", {
...
local node_sounds = {
	...
	["default:beech_trunk"] = "mesecons_noteblock_crash",
	["default:wood"] = "mesecons_noteblock_litecrash",
	["default:coal_block"] = "tnt_explode",
}

mesecon.noteblock_play = function(pos, param2)
	pos.y = pos.y-1
	local nodeunder = core.get_node(pos).name
	local soundname = node_sounds[nodeunder]
	if not soundname then
		soundname = soundnames[param2]
		if not soundname then
			core.log("error", "[mesecons_noteblock] No soundname found, test param2")
			return
		end
		if nodeunder == "default:steel_block" then
			soundname = soundname.. 2
		end
	end
	pos.y = pos.y+1
	core.sound_play(soundname, {pos = pos})
end
