...
local function piston_remove_pusher(pos, node, check_falling)
	...
	core.sound_play("piston_retract", {pos = pos,
		max_hear_distance = 20, gain = .3})
...
local piston_on = function(pos, node)
	...
	core.set_node(pos, {param2 = node.param2, name = pistonspec.onname})
	core.set_node(pusher_pos, {param2 = node.param2, name = pistonspec.pusher})
	core.sound_play("piston_extend", {pos = pos,
		max_hear_distance = 20, gain = .3})
...
local function piston_off(pos, node)
	local pistonspec = get_pistonspec(node.name, "onname")
	core.set_node(pos, {param2 = node.param2, name = pistonspec.offname})
...
local function piston_orientate(pos, placer)
	...
	-- core.after, because on_placenode for unoriented piston must be processed first
...
local function piston_rotate_on(pos, node, player, mode)
	...
	local pusher_node = core.get_node(pusher_pos)
	...
	local pname = player and player:is_player() and player:get_player_name() or ""
	...
	for i = 1, 5 do
		...
		local pusher_pos_after_node_name = core.get_node(pusher_pos_after).name
		local pusher_pos_after_node_def = core.registered_nodes[pusher_pos_after_node_name]
		if pusher_pos_after_node_def and pusher_pos_after_node_def.buildable_to and
				not core.is_protected(pusher_pos_after, pname) then
			ok = true
			break
		end
	end
	...
	core.remove_node(pusher_pos)
	core.set_node(pusher_pos_after, pusher_node)
	core.swap_node(pos, node)
	mesecon.execute_autoconnect_hooks_now(pos, node)
	return true
end
...
local function piston_rotate_pusher(pos, node, player, mode)
	...
	local piston_pos = vector.add(pos, core.facedir_to_dir(node.param2))
	local piston_node = core.get_node(piston_pos)
	if piston_node.name ~= pistonspec.onname then
		core.remove_node(pos) -- Make it possible to remove alone pushers.
		return false
	end
...
-- offstate
core.register_node("mesecons_pistons:piston_normal_off", {
	description = "Piston",
	tiles = {
...
-- onstate
core.register_node("mesecons_pistons:piston_normal_on", {
...
-- pusher
core.register_node("mesecons_pistons:piston_pusher_normal", {
...
-- Sticky ones
-- offstate
core.register_node("mesecons_pistons:piston_sticky_off", {
...
-- onstate
core.register_node("mesecons_pistons:piston_sticky_on", {
...
-- pusher
core.register_node("mesecons_pistons:piston_pusher_sticky", {
...
local function piston_get_stopper(node, dir, stack, stackid)
	...
	local pushernode = core.get_node(pusherpos)
...
-- load legacy code
dofile(core.get_modpath("mesecons_pistons").. DIR_DELIM .."legacy.lua")
