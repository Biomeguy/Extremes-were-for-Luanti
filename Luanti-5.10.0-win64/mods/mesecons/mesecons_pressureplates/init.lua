...
local function pp_on_timer(pos, elapsed)
	...
	local basename = core.registered_nodes[node.name].pressureplate_basename
	...
	elseif node.name == basename .. "_off" then
		for k, obj in pairs(objs) do
			...
			if objpos.y > pos.y-1 and objpos.y < pos.y then
				core.set_node(pos, {name = basename .. "_on"})
...
function mesecon.register_pressure_plate(basename, description, textures_off, textures_on, image_w, image_i, recipe, groups, sounds)
	...
	mesecon.register_node(basename, {
		...
		on_construct = function(pos)
			core.get_node_timer(pos):start(mesecon.setting("pplate_interval", 0.1))
		end,
		sounds = sounds,
	},{
