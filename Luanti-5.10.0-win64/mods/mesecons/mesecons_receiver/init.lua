...
function mesecon.receiver_get_pos_from_rcpt(pos, param2)
	...
	if param2 == nil then param2 = core.get_node(pos).param2 end
...
function mesecon.receiver_place(rcpt_pos)
	local node = core.get_node(rcpt_pos)
	local pos, rcvtype = mesecon.receiver_get_pos_from_rcpt(rcpt_pos, node.param2)
	local nn = core.get_node(pos)
	local param2 = core.dir_to_facedir(core.facedir_to_dir(node.param2))

	if string.find(nn.name, "mesecons:wire_") ~= nil then
		core.set_node(pos, {name = rcvtype, param2 = param2})
...
core.register_on_placenode(function (pos, node)
	if core.get_item_group(node.name, "mesecon_needs_receiver") == 1 then
	...
	if string.find(node.name, "mesecons:wire_") ~= nil then
		...
		while rules[i] ~= nil do
			local np = {	x = pos.x + rules[i].x,
					y = pos.y + rules[i].y,
					z = pos.z + rules[i].z}
			if core.get_item_group(core.get_node(np).name, "mesecon_needs_receiver") == 1 then
...
function mesecon.buttonlike_onrotate(pos, node)
	core.after(0, mesecon.receiver_remove, pos, node)
	core.after(0, mesecon.receiver_place, pos)
end
