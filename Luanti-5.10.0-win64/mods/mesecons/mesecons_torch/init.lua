...
local function mesecon_torch_action(pos, elapsed)
	...
	if is_powered then
		if node.name == "mesecons_torch:mesecon_torch_on" then
			core.swap_node(pos, {name = "mesecons_torch:mesecon_torch_off", param2 = node.param2})
			mesecon.receptor_off(pos, torch_get_output_rules(node))
		end
	elseif node.name == "mesecons_torch:mesecon_torch_off" then
		core.swap_node(pos, {name = "mesecons_torch:mesecon_torch_on", param2 = node.param2})
		mesecon.receptor_on(pos, torch_get_output_rules(node))
	end
	core.get_node_timer(pos):start(.2)
end

core.register_node("mesecons_torch:mesecon_torch_off", {
	drawtype = "torchlike",
...
core.register_node("mesecons_torch:mesecon_torch_on", {
	drawtype = "torchlike",
