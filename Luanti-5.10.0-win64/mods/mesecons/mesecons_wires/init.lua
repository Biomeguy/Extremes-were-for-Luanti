...
local wire_getconnect = function (from_pos, self_pos)
	...
	and core.registered_nodes[node.name].mesecons then
		-- rules of node to possibly connect to
		local rules = {}
		if (core.registered_nodes[node.name].mesecon_wire) then
...
local wire_updateconnect = function (pos)
	...
	local state_suffix = string.find(core.get_node(pos).name, "_off") and "_off" or "_on"
	core.set_node(pos, {name = "mesecons:wire_"..nodeid..state_suffix})
end
...
local update_on_place_dig = function (pos, node)
	...
	and (core.registered_nodes[nn.name].mesecon_wire) then
	...
	local rules = {}
	if core.registered_nodes[node.name]
	and core.registered_nodes[node.name].mesecon_wire then
		rules = mesecon.rules.default
	else
		rules = mesecon.get_any_rules(node)
	end
	...
	for _, r in ipairs(mesecon.flattenrules(rules)) do
		...
		if core.registered_nodes[core.get_node(np).name]
		and core.registered_nodes[core.get_node(np).name].mesecon_wire then
