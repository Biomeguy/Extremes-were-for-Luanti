...
if type(core.colorize) == "function" then
	rgblightstone.colorize = core.colorize
else
...
function rgblightstone.autofill(pos, player)
	...
	if (not meta:get_string("channel")) or meta:get_string("channel")=="" then
		...
		local meta_above = core.get_meta(pos_above)
		...
		tonumber(meta_above:get_string("addry")) then
			...
			core.chat_send_player(player:get_player_name(),rgblightstone.colorize("#55FF55","Success: ").."Auto-filled with channel "..rgblightstone.colorize("#00FFFF",channel)..", X address "..rgblightstone.colorize("#00FFFF",addrx)..", and Y address "..rgblightstone.colorize("#00FFFF",addry)..".")
			meta:set_string("infotext","")
		else
			core.chat_send_player(player:get_player_name(),rgblightstone.colorize("#FF0000","ERROR: ").."Node above is not RGB Lightstone or is not configured correctly!")
		end
	end
end

core.register_node("rgblightstone:rgblightstone", {
	...
	on_punch = function(pos, node, player, pointd)
