	...
	for i=pos.x-math.random(0, 1), pos.x+math.random(0, 1), 1 do
		for j=pos.y-1, pos.y+2, 1 do
			for k=pos.z-math.random(0, 1), pos.z+math.random(0, 1), 1 do
				...
				if core.get_item_group(n, "unbreakable") == 1 or core.is_protected(p, "") then
				else
					core.set_node({x=i, y=j, z=k}, {name="draconis:dragon_fire"})
				end
			end
		end
	end
...
mobs:register_mob("draconis:hatched_fire_wyvern", {
	...
	if not self.v2 then
		...
		self.accel = 6
		self.terrain_type = 3
		self.driver_attach_at = {x = 0, y = 1.22, z = .5}
	...
	if self.driver then
		...
		mobs.drive(self, "walk", "stand", true, dtime)
                ...
                self.driver:set_properties({visual_size = {x=.05, y=.05}})
		...
		if self.owner == "" then
			...
			and self.order == "roam" then
				...
				self.order = "sit"
                                ...
                                self:set_animation(self.animation.stand)
			else
                                self.jump = true
				self.order = "roam"
                                self.walk_chance = 15
...
core.register_node("draconis:fire_wyvern_egg", {
	...
	local pos_under= { 
   	...
   	z = pos.z,
        ...
        local nest = core.get_node_or_nil(pos_under) 
	...
	on_timer = function(pos)
		...
		mob = core.add_entity(pos, "draconis:hatched_fire_wyvern")
