...
mobs = {
	...
	version = "20240525",
	translate = S,
	invis = core.global_exists("invisibility") and invisibility or {},
	node_snow = core.registered_aliases["mapgen_snow"]
			or (use_mc2 and "mcl_core:snow") or "default:snow",
	node_dirt = core.registered_aliases["mapgen_dirt"]
...
function mob_class:line_of_sight(pos1, pos2)
	...
	while thing do -- thing.type, thing.ref
		...
		if thing.type == "node" then
			...
			local name = core.get_node(thing.under).name

			if core.registered_items[name]
			and core.registered_items[name].walkable then
				return false
			end
		end
...
function mobs:is_node_dangerous(mob_object, nodename)
	...
	and core.get_item_group(nodename, "water") ~= 0 then
		return true
	end

	if mob_object.lava_damage > 0
	and core.get_item_group(nodename, "lava") ~= 0 then
		return true
	end

	if mob_object.fire_damage > 0
	and core.get_item_group(nodename, "fire") ~= 0 then
		return true
	end

	if core.registered_nodes[nodename].damage_per_second > 0 then
		return true
	end
...
-- can mob see player
local function is_invisible(self, pname)

	if mobs.invis[pname] and not self.ignore_invisibility then
		return true
	end
end
...
function mob_class:replace(pos)
	...
	if #core.find_nodes_in_area(pos, pos, what) > 0 then
...
local function is_peaceful_player(player)
	...
	-- player priv enabled
	if pname and core.check_player_privs(pname, "peaceful_player") then
		return true
	end
end
...
function mob_class:do_states(dtime)
	...
	if is_node_dangerous(self, self.standing_in) then
		...
		local lp = core.find_node_near(s, 1, grps)

		if lp then

			if self.pause_timer <= 0 then

				lp = core.find_nodes_in_area_under_air(
...
--print("=== explosion timer stopped")
	...
	self:set_animation("punch")-- Biomeguy's
...
function mob_class:on_punch(hitter, tflp, tool_capabilities, dir, damage)
	...
	if self.protected then
		...
		if is_player(hitter) then
			...
			if pname ~= self.owner
			and core.is_protected(self.object:get_pos(), pname) then
...
core.register_entity(":" .. name, setmetatable({
	...
	drops = def.drops,
	exen_drop = def.exen_drop,
	extra_energy = def.extra_energy,
	armor = def.armor,
	arrow = def.arrow,
...
local function can_spawn(pos, name)
	...
	for z = min_z, max_z do
		...
		if core.registered_nodes[node_ok(pos2).name].walkable == true then
			return nil
		end
	end
	end
...
--print("--- no position or definition given")
	...
	or not core.registered_entities[def.name] then
...
--print ("--- spawning not registered for " .. name)
	...
	local spawn_action = function(
		...
		if map_load and not core.find_node_near(pos, 1, neighbors) then
--print("--- lbm neighbors not found")
		...
		or not core.registered_entities[name] then
...
function mobs:boom(self, pos, radius, damage_radius, texture)
	...
	if mobs_griefing
	and core.get_modpath("tnt") and tnt and tnt.boom
		...
		on_place = function(itemstack, placer, pointd)
			...
			if def and def.on_rightclick then
				...
				return def.on_rightclick(
						pointd.under, under, placer, itemstack, pointd)
			end
...
function mobs:capture_mob(
	...
	and self.owner ~= name and force_take == false then
		...
		core.chat_send_player(name, S("@1 is owner!", self.owner))
...
if core.get_modpath("extra_energy") then-- drop any excess extra energy
		local drop = self.extra_energy
		if type(drop) == "table" then
			drop = math.random(drop[1], drop[2])
		end
...
function mobs:protect(self, clicker)
	...
	or (self.protected == 2 and tool_name == "mobs:protector2") then
		core.chat_send_player(name, S("Already protected!"))
		return true
	end
...
function mobs:alias_mob(old_name, new_name)
	...
	if core.registered_entities[old_name] then
		return
	end
