...
mobs:register_mob("mobs_monster:lava_flan", {
	...
	floats = 1,
	drops = {
		{name = "default:molten_rock", chance = 1, min = 1, max = 2},
...
if core.get_modpath("nssm") and core.get_modpath("gto") then
	local item = clicker:get_wielded_item()
	local inv = clicker:get_inventory()

	if item:get_name() == "nssm:slothful_soul_fragment" and inv:contains_item("main", "default:mese_crystal") then
		if not core.is_creative_enabled(clicker:get_player_name()) then
			item:take_item()
			clicker:set_wielded_item(item)
			inv:remove_item("main", "default:mese_crystal 1")
		end
		local pos = self.object:get_pos()
		pos.y = pos.y + 0.5
		core.add_item(pos, {name = "gto:teleport_orb"})
		mobs:effect(pos, 40, "fire_basic_flame.png", 2, 3, 2, 5, 10, nil)
		core.sound_play("mobs_fireball", {pos = pos, max_hear_distance = 10, pitch = .6, gain = .7}, true)
		self.object:remove()
	end
end
	end,
...
core.register_tool(":mobs:pick_lava", {
	...
	tool_capabilities = {
		...
		damage_groups = {fleshy = 6, fire = 1, ice = 1},
		cook_dug = true,
	},
	groups = {pickaxe = 1},
	light_source = 14
