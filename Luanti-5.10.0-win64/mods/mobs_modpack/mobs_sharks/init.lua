...
-- load settings
	dofile(core.get_modpath("mobs_sharks").."/SETTINGS.txt")
...
-- large
	if ENABLE_SHARK_LARGE then
		mobs:register_mob("mobs_sharks:shark_lg", {
			...
			hp_max = 25,
			armor = 150,
			collisionbox = {-0.75, -0.5, -0.75, 0.75, 0.5, 0.75}, --{-0.75, -0.35, -0.75, 0.75, 0.5, 0.75},
			visual = "mesh",
			mesh = l_model,
			textures = l_skins,
...
-- medium
	if ENABLE_SHARK_MEDIUM then
		mobs:register_mob("mobs_sharks:shark_md", {
			...
			hp_max = 20,
			armor = 125,
			collisionbox = {-0.57, -0.38, -0.57, 0.57, 0.38, 0.57}, --{-0.57, -0.27, -0.57, 0.57, 0.38, 0.57},
			visual = "mesh",
...
-- small
	if ENABLE_SHARK_SMALL then
		mobs:register_mob("mobs_sharks:shark_sm", {
			...
			hp_max = 15,
			armor = 100,
			collisionbox = {-0.38, -0.25, -0.38, 0.38, 0.25, 0.38}, --{-0.38, -0.18, -0.38, 0.38, 0.25, 0.38},
			visual = "mesh",
