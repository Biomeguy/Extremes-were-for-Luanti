...
function circular_saw:get_output_inv(modname, material, amount, max)
	if (not max or max < 1 or max > 144) then max = 144 end
...
if t[1] == "stair" and t[2] == "_outer" or t[1] == "stair" and t[2] == "" or t[1] == "stair" and t[2] == "_inner" or t[1] == "slab" and t[2] == "" then
	nodename = "stairs:" .. t[1] .. t[2] .. "_" .. material
end
...
function circular_saw.on_construct(pos)
	...
	meta:set_string("max_offered", 144) -- How many items of this kind are offered by default?
...
core.register_node("moreblocks:circular_saw",  {
	...
	node_box = {
		...
		fixed = {
			...
			{-.4, -.5, .25, -.25, .25, .4}, -- Leg
			{.25, -.5, -.4, .4, .25, -.25}, -- Leg
			{-.5, .25, -.5, .5, .375, .5}, -- Tabletop
			{-.01, .4375, -.125, .01, .5, .125}, -- Saw blade (top)
			{-.01, .375, -.1875, .01, .4375, .1875}, -- Saw blade (bottom)
			{-.25, -.0625, -.25, .25, .25, .25}, -- Motor case
		},
	},
