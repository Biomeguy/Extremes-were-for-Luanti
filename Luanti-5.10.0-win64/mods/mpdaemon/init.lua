...
mpd.pause_between_songs=tonumber(core.settings:get("mpd_pause_between_songs")) or 30

--end config

mpd.modpath=core.get_modpath("mpdaemon")
if not mpd.modpath then
	error("mpd mod folder has to be named 'mpdaemon'!")
end
--{name, length, gain~1}
mpd.songs = {}
...
for linent in sfile:lines() do
	...
	if line~="" and string.sub(line,1,1)~="#" then
		...
		if name and timeMins and timeSecs and gain then
			...
			core.log("warning", "[mpdaemon] Misformatted song entry in songs.txt: "..line)
		end
	end
end
sfile:close()
...
if #mpd.songs==0 then
	print("[mpdaemon]no songs registered, not doing anything")
	return
end

mpd.storage = core.get_mod_storage()
...
mpd.play_song=function(id)
	...
	for _,player in ipairs(core.get_connected_players()) do
		...
		if pvolume>0 then
			local handle = core.sound_play(song.name, {to_player=pname, gain=song.gain*pvolume})
			if handle then
...
core.register_privilege("mpdaemon", "may control the music player daemon (mpdaemon) mod")

core.register_chatcommand("mpd_stop", {
	params = "",
...
core.register_chatcommand("mpd_list", {
	params = "",
	description = "List all available songs and their IDs",
	privs = {mpdaemon=true},
	func = function(name, param)
		for k,v in ipairs(mpd.songs) do
			core.chat_send_player(name, mpd.song_human_readable(k))
		end
	end,		
})
core.register_chatcommand("mpd_play", {
	params = "<id>",
...
core.register_chatcommand("mpd_what", {
	params = "",
...
core.register_chatcommand("mpd_next", {
	params = "[seconds]",
...
core.register_chatcommand("mvolume", {
