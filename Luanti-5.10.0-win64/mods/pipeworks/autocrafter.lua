local autocrafterCache = {}  -- caches some recipe data to avoid to call the slow function core.get_craft_result() every second
...
local function get_craft(pos, inventory, hash)
	...
	if not craft then
		...
		local output, decremented_input = core.get_craft_result({method = "normal", width = 3, items = recipe})
...
core.register_node("pipeworks:autocrafter", {
	...
	allow_metadata_inventory_take = function(pos, listname, index, stack, player)
		if not pipeworks.may_configure(pos, player) then
			core.log("action", string.format("%s attempted to take from autocrafter at %s", player:get_player_name(), core.pos_to_string(pos)))
			return 0
		end
		upgrade_autocrafter(pos)
		local inv = core.get_meta(pos):get_inventory()
			...
			action = function(pos,node,channel,msg)
				...
				if type(msg) == "table" then
					...
					for y=0,2,1 do
						for x=1,3,1 do
							local slot = y*3+x
							if core.registered_items[msg[y+1][x]] then
