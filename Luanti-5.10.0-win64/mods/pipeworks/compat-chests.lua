...
local function chest_lid_obstructed(pos)
	...
	if def and (def.drawtype == "airlike" or def.drawtype == "signlike" or
		def.drawtype == "torchlike" or (def.drawtype == "nodebox" and def.paramtype2 == "wallmounted")) then
		return false
	end
	return true
end
		...
		if open_chests[pn] then
			...
			if fields.quit then
				...
				core.after(0.2, function()
					core.swap_node(pos, { name = "default:" .. swap, param2 = node.param2 })

					-- Pipeworks notification
					pipeworks.after_place(pos)
				end)
				core.sound_play(sound, {gain = 0.3, pos = pos, max_hear_distance = 10})
			end

			-- Pipeworks Switch
			if pipeworks.may_configure(pos, player) and not fields.quit then
				fs_helpers.on_receive_fields(pos, fields)
				core.show_formspec(pn, "pipeworks:chest_formspec", get_chest_formspec(pos))
			end
			return true
		end
	end
...
local old_chest_def = table.copy(core.registered_items["default:chest"])
local old_chest_open_def = table.copy(core.registered_items["default:chest_open"])
local old_chest_locked_def = table.copy(core.registered_items["default:chest_locked"])
local old_chest_locked_open_def = table.copy(core.registered_items["default:chest_locked_open"])
...
override_protected = {
	...
	on_rightclick = function(pos, node, clicker, itemstack, pointd)
		...
		core.sound_play(old_chest_locked_def.sound_open, {gain = 0.3,
...
override = {
	...
	on_rightclick = function(pos, node, clicker)
		core.sound_play(old_chest_def.sound_open, {gain = 0.3, pos = pos,
...
core.override_item("default:chest", override)
core.override_item("default:chest_open", override_open)
core.override_item("default:chest_locked", override_protected)
core.override_item("default:chest_locked_open", override_protected_open)
