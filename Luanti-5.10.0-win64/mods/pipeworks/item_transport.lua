...
local enable_max_limit = core.settings:get("pipeworks_enable_items_per_tube_limit")
...
local crunch_tube = function(pos, cnode, cmeta)
	if enable_max_limit then
		...
		if itemcount > max_tube_limit then
			cmeta:set_string("the_tube_was", core.serialize(cnode))
			print("[Pipeworks] Warning - a tube at "..core.pos_to_string(pos).." broke due to too many items ("..itemcount..")")
...
local function go_next_compat(pos, cnode, cmeta, cycledir, vel, stack, owner)
	...
	if core.registered_nodes[cnode.name] and core.registered_nodes[cnode.name].tube and core.registered_nodes[cnode.name].tube.can_go then
		can_go = core.registered_nodes[cnode.name].tube.can_go(pos, cnode, vel, stack)
	else
...
luaentity.register_entity("pipeworks:tubed_item", {
	...
	on_step = function(self, dtime)
		...
		if moved and core.get_item_group(node.name, "tubedevice_receiver") == 1 then
			local leftover
			if core.registered_nodes[node.name].tube and core.registered_nodes[node.name].tube.insert_object then
				leftover = core.registered_nodes[node.name].tube.insert_object(self.start_pos, node, stack, vel, self.owner)
			else
				leftover = stack
			end
