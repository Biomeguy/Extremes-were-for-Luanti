local filename=core.get_worldpath() .. "/teleport_tubes"

local tp_tube_db = nil -- nil forces a read
local tp_tube_db_version = 2.0

local function hash(pos)
	return string.format("%.30g", core.hash_node_position(pos))
end
...
local function save_tube_db()
	...
	if file then
		...
		file:write(core.serialize(tp_tube_db))
...
local function read_tube_db()
	...
	if file ~= nil then
		...
		if file_content and file_content ~= "" then
			tp_tube_db = core.deserialize(file_content)
...
local function set_tube(pos, channel, can_receive)
	...
	if  existing ~= nil then
		local e = "error"
		core.log(e, "pipeworks teleport tube update refused due to position hash collision")
		core.log(e, "collided hash: "..hash)
		core.log(e, "tried-to-place tube: "..fmt(pos))
		core.log(e, "existing tube: "..fmt(existing))
		return
	end
...
local function read_node_with_vm(pos)
	...
	return core.get_name_from_content_id(data[area:index(pos.x, pos.y, pos.z)])
end
...
local function get_receivers(pos, channel)
	...
	for key, val in pairs(tubes) do
		...
		if (val.cr == 1 and val.channel == channel and (val.x ~= pos.x or val.y ~= pos.y or val.z ~= pos.z)) then
			local is_loaded = (core.get_node_or_nil(val) ~= nil)
			local node_name = is_loaded and core.get_node(pos).name or read_node_with_vm(val)

			if core.registered_nodes[node_name] and core.registered_nodes[node_name].is_teleport_tube then
		...
		on_receive_fields = function(pos,formname,fields,sender)
			...
			if new_channel ~= "" then
				...
				if name and mode and name ~= sender_name then
					...
					if mode == ":" then
						core.chat_send_player(sender_name, "Sorry, channel '"..new_channel.."' is reserved for exclusive use by "..name)
						return
					...
					elseif mode == ";" and (fields.cr1 or (can_receive ~= 0 and not fields.cr0)) then
						core.chat_send_player(sender_name, "Sorry, receiving from channel '"..new_channel.."' is reserved for "..name)
						return
					end
				end
			end
