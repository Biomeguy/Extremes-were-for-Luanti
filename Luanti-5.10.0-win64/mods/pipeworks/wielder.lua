...
local function set_wielder_formspec(data, meta)
	meta:set_string("formspec",
		...
		"label[1,0;"..core.formspec_escape(data.description).."]"..
		"list[current_name;"..core.formspec_escape(data.wield_inv_name)..";"..((8-data.wield_inv_width)*0.5)..",1;"..data.wield_inv_width..","..data.wield_inv_height..";]"..
...
local can_tool_dig_node = function(nodename, toolcaps, toolname)
	...
	-- brief documentation of core.get_dig_params() as it's not yet documented in lua_api.txt:
	...
	local diggable = core.get_dig_params(nodegroups, toolcaps).diggable
	if not diggable then
		...
		local hand_caps = core.registered_items[""].tool_capabilities
		diggable = core.get_dig_params(nodegroups, hand_caps)
	end
	return diggable
end
...
local function wielder_on(data, wielder_pos, wielder_node)
	...
	core.swap_node(wielder_pos, wielder_node)
	core.check_for_falling(wielder_pos)
	local wielder_meta = core.get_meta(wielder_pos)
	...
	local dir = core.facedir_to_dir(wielder_node.param2)
	...
	local virtplayer = pipeworks.create_fake_player({
		name = data.masquerade_as_owner and wielder_meta:get_string("owner")
			or ":pipeworks:" .. core.pos_to_string(wielder_pos),
	...
	local pointd = { type="node", under=under_pos, above=above_pos }
	data.act(virtplayer, pointd)
...
local function register_wielder(data)
	...
	for _, state in ipairs({ "off", "on" }) do
		...
		core.register_node(data.name_base.."_"..state, {
			...
			tube = {
				can_insert = function(pos, node, stack, tubedir)
					if not data.tube_permit_anteroposterior_insert then
						local nodedir = core.facedir_to_dir(node.param2)
			...
			after_place_node = function (pos, placer)
				...
				if placer_pos then
					...
					node.param2 = core.dir_to_facedir(dir, true)
					core.set_node(pos, node)
					core.log("action", "real (6d) facedir: " .. node.param2)
				end
...
if pipeworks.enable_node_breaker then
	...
	data = {
		...
		sneak = false,
		act = function(virtplayer, pointd)
			...
			if on_use then
				...
				wieldstack = on_use(wieldstack, virtplayer, pointd) or wieldstack
				virtplayer:set_wielded_item(wieldstack)
			else
				local under_node = core.get_node(pointd.under)
				local def = core.registered_nodes[under_node.name]
				if not def then
				...
				if can_tool_dig_node(under_node.name,
					...
					def.on_dig(pointd.under, under_node, virtplayer)
					local sound = def.sounds and def.sounds.dug
					if sound then
						core.sound_play(sound.name,
							{pos=pointd.under, gain=sound.gain})
					end
			...
			if wieldname == oldwieldstack:get_name() then
				...
				if wieldstack:get_count() == oldwieldstack:get_count() and
						wieldstack:get_meta():get_string("") == oldwieldstack:get_meta():get_string("") and
						((core.registered_items[wieldstack:get_name()] or {}).wear_represents or "mechanical_wear") == "mechanical_wear" then
	...
	core.register_lbm({
		...
		action = function(pos, node)
			...
			local invref = core.get_meta(pos):get_inventory()
...
if pipeworks.enable_deployer then
	register_wielder({
		...
		act = function(virtplayer, pointd)
			local wieldstack = virtplayer:get_wielded_item()
			virtplayer:set_wielded_item((core.registered_items[wieldstack:get_name()] or {on_place=core.item_place}).on_place(wieldstack, virtplayer, pointd) or wieldstack)
		end,
		eject_drops = false,
	})
	core.register_craft({
...
if pipeworks.enable_dispenser then
	register_wielder({
		...
		act = function(virtplayer, pointd)
			local wieldstack = virtplayer:get_wielded_item()
			virtplayer:set_wielded_item((core.registered_items[wieldstack:get_name()] or
				{on_drop=core.item_drop}).on_drop(wieldstack, virtplayer, virtplayer:get_pos()) or
				wieldstack)
		end,
		eject_drops = false,
	})
