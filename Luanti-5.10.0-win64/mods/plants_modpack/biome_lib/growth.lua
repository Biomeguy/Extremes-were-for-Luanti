...
function biome_lib:grow_plants(opts)
	...
	core.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			local n_bot = core.get_node(p_bot)
			local root_node = core.get_node({x=pos.x, y=pos.y-options.height_limit, z=pos.z})
			local walldir = nil
			  ...
			  and (not options.need_wall or (options.need_wall and walldir)) then
				if options.grow_vertically and walldir then
					if biome_lib:search_downward(pos, options.height_limit, options.ground_nodes) then
						core.swap_node(p_top, { name = options.grow_plant, param2 = walldir})
					end
...
-- spawn_tree() on generate is routed through here so that other mods can hook into it.

function biome_lib:generate_tree(pos, nodes_or_function_or_model)
	core.spawn_tree(pos, nodes_or_function_or_model)
end
