...
biome_lib.modpath = core.get_modpath("biome_lib")

biome_lib.total_no_aircheck_calls = 0

biome_lib.queue_run_ratio = tonumber(core.settings:get("biome_lib_queue_run_ratio")) or 100
...
function biome_lib:dbg(msg)
	if DEBUG then
		print("[Plantlife] "..msg)
		core.log("verbose", "[Plantlife] "..msg)
	end
end
...
local function get_biome_data(pos, perlin_fertile)
	...
	if type(core.get_biome_data) == "function" then
...
function biome_lib:is_node_loaded(node_pos)
	local n = core.get_node_or_nil(node_pos)
...
function biome_lib:register_generate_plant(biomedef, nodes_or_function_or_model)
	...
	if type(nodes_or_function_or_model) == "string"
	  ...
	  and not core.registered_nodes[nodes_or_function_or_model] then
	...
	if biomedef.check_air == false then 
		...
		if type(s) == "string" then
			if s and (string.find(s, "^group:") or core.registered_nodes[s]) then
...
local function populate_single_surface(biome, pos, perlin_fertile_area, checkair)
	...
	if not biome.depth then
		local dest_node = core.get_node(pos)
			...
			if string.find(biome_surfaces_string, "group:") then
				for j = 1, #biome.surface do
					if string.find(biome.surface[j], "^group:") 
					  and core.get_item_group(dest_node.name, biome.surface[j]) then
						surface_ok = true
						break
					end
				end
	...
	elseif not string.find(biome_surfaces_string,
			core.get_node({ x = pos.x, y = pos.y-biome.depth-1, z = pos.z }).name) then
		surface_ok = true
	end
	...
	if checkair and core.get_node(p_top).name ~= "air" then
		return
	end
...
function biome_lib:populate_surfaces(biome, nodes_or_function_or_model, snodes, checkair)
	...
	local perlin_fertile_area = core.get_perlin(biome.seed_diff, perlin_octaves, perlin_persistence, perlin_scale)
	...
	for i = 1, math.min(biome.max_count, num_in_biome_nodes) do
		...
		while tries < 2 and not spawned do
			...
			if not (biome.avoid_nodes and biome.avoid_radius
					and core.find_node_near(p_top, biome.avoid_radius
					+ math.random(-1.5,2), biome.avoid_nodes)) then
				if biome.delete_above then
					core.swap_node(p_top, biome_lib.air)
					core.swap_node({x=p_top.x, y=p_top.y+1, z=p_top.z}, biome_lib.air)
				end

				if biome.delete_above_surround then
					core.swap_node({x=p_top.x-1, y=p_top.y, z=p_top.z}, biome_lib.air)
					core.swap_node({x=p_top.x+1, y=p_top.y, z=p_top.z}, biome_lib.air)
					core.swap_node({x=p_top.x,   y=p_top.y, z=p_top.z-1}, biome_lib.air)
					core.swap_node({x=p_top.x,   y=p_top.y, z=p_top.z+1}, biome_lib.air)

					core.swap_node({x=p_top.x-1, y=p_top.y+1, z=p_top.z}, biome_lib.air)
					core.swap_node({x=p_top.x+1, y=p_top.y+1, z=p_top.z}, biome_lib.air)
					core.swap_node({x=p_top.x,   y=p_top.y+1, z=p_top.z-1}, biome_lib.air)
					core.swap_node({x=p_top.x,   y=p_top.y+1, z=p_top.z+1}, biome_lib.air)
				end

				if biome.spawn_replace_node then
					core.swap_node(pos, biome_lib.air)
				end
				...
				if objtype == "table" then
					if nodes_or_function_or_model.axiom then
						...
						core.swap_node(p_top, { name = nodes_or_function_or_model[math.random(#nodes_or_function_or_model)], param2 = fdir })
						spawned = true
					end
				elseif objtype == "string" and
				  core.registered_nodes[nodes_or_function_or_model] then
					local fdir = nil
					if biome.random_facedir then
						fdir = math.random(biome.random_facedir[1], biome.random_facedir[2])
					end
					core.swap_node(p_top, { name = nodes_or_function_or_model, param2 = fdir })
					spawned = true
...
function biome_lib:generate_block_with_air_checking()
	...
	local blockhash =	core.hash_node_position(minp)

	if not biome_lib.surface_nodes_aircheck.blockhash then

		if type(core.find_nodes_in_area_under_air) == "function" then -- use newer API call
			biome_lib.surface_nodes_aircheck.blockhash =
				core.find_nodes_in_area_under_air(minp, maxp, biome_lib.surfaceslist_aircheck)
		else
			local search_area = core.find_nodes_in_area(minp, maxp, biome_lib.surfaceslist_aircheck)
			...
			local pos = search_area[i]
				local p_top = { x=pos.x, y=pos.y+1, z=pos.z }
				if core.get_node(p_top).name == "air" then
...
function biome_lib:generate_block_no_aircheck()
	...
	if not biome_lib.surface_nodes_no_aircheck.blockhash then
		...
		biome_lib.surface_nodes_no_aircheck.blockhash =
			core.find_nodes_in_area(minp, maxp, biome_lib.surfaceslist_no_aircheck)
...
local step_duration = tonumber(core.settings:get("dedicated_server_step"))
core.register_globalstep(function(dtime)
	...
	biome_lib.globalstep_start_time = core.get_us_time()
	...
	while (#biome_lib.blocklist_aircheck > 0 or #biome_lib.blocklist_no_aircheck > 0)
	  and biome_lib.globalstep_runtime < 200000 do  -- 0.2 seconds, in uS.
		...
		biome_lib.globalstep_runtime = core.get_us_time() - biome_lib.globalstep_start_time
	end
end)
...
function biome_lib:spawn_on_surfaces(sd,sp,sr,sc,ss,sa)
	...
	core.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			local n_light = core.get_node_light(p_top)
			if n_light < biome.light_min or n_light > biome.light_max then
				return -- Too dark or too bright
			end

			if biome.avoid_nodes and biome.avoid_radius and core.find_node_near(
			...
			if biome.alt_wallnode and walldir then
				if n_top.name == "air" then
					core.swap_node(p_top, { name = biome.alt_wallnode, param2 = walldir })
				end
				return
			end

			local currentsurface = core.get_node(pos).name
			...
			elseif not biome.spawn_on_side and not biome.spawn_on_bottom and not biome.spawn_replace_node then
				if n_top.name == "air" then
					core.swap_node(p_top, { name = plant_to_spawn, param2 = fdir })
				end
			elseif biome.spawn_replace_node then
				core.swap_node(pos, { name = plant_to_spawn, param2 = fdir })

			elseif biome.spawn_on_side then
				local onside = biome_lib:find_open_side(pos)
				if onside then
					core.swap_node(onside.newpos, { name = plant_to_spawn, param2 = onside.facedir })
				end
			elseif biome.spawn_on_bottom then
				if core.get_node({x=pos.x, y=pos.y-1, z=pos.z}).name == "air" then
					core.swap_node({x=pos.x, y=pos.y-1, z=pos.z}, { name = plant_to_spawn, param2 = fdir} )
	core.get_node_timer({x=pos.x, y=pos.y-1, z=pos.z}):start(math.random(5, 10))
				end
			end
		end
	})
...
function biome_lib:replace_object(pos, replacement, grow_function, walldir, seeddiff)
	...
	elseif growtype == "function" then
		local perlin_fertile_area = core.get_perlin(seeddiff, perlin_octaves, perlin_persistence, perlin_scale)
	...
	elseif growtype == "nil" then
		core.swap_node(pos, { name = replacement, param2 = walldir})
		return
