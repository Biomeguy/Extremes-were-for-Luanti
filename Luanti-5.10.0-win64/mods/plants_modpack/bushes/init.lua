...
-- support for i18n
local S = core.get_translator("bushes")
abstract_bushes = {}

core.register_node("bushes:youngtree2_bottom", {
...
for i in pairs(BushBranchCenter) do
	...
	core.register_node("bushes:bushbranches"..Num, {
...
for i in pairs(BushLeafNode) do
	...
	core.register_node("bushes:BushLeaves"..Num, {
...
abstract_bushes.grow_bush_node = function(pos,dir, leaf_type)
	...
	if core.get_node(right_here).name == "air"  -- instead of check_air = true,
	or core.get_node(right_here).name == "default:jungle_grass" then
		core.swap_node(right_here, {name="bushes:bushbranches"..bush_branch_type , param2=dir})
						--core.chat_send_all("leaf_type: (" .. leaf_type .. ")")
		core.swap_node(above_right_here, {name="bushes:BushLeaves"..leaf_type})
		local chance_of_high_leaves = math.random(1,10)
		if chance_of_high_leaves> 5 then
			local two_above_right_here = {x=pos.x, y=pos.y+3, z=pos.z}
							--core.chat_send_all("leaf_type: (" .. leaf_type .. ")")
			core.swap_node(two_above_right_here, {name="bushes:BushLeaves"..leaf_type})
		end
	end
end

biome_lib:register_generate_plant({
    surface = {"default:dirt_with_grass", "ecology:peat_moss"},
    max_count = 15,  --10,15
    rarity = 101 - 4,  --3,4
...
abstract_bushes.grow_youngtree_node2 = function(pos, height)
	...
	or core.get_node(right_here).name == "default:jungle_grass" then
		if height == 4 then
			...
			core.swap_node(right_here, {name="bushes:youngtree2_bottom"})
			core.swap_node(above_right_here, {name="bushes:youngtree2_bottom"})
			core.swap_node(two_above_right_here, {name="bushes:bushbranches2"  , param2=2})
			core.swap_node(two_above_right_here_south, {name="bushes:bushbranches2"  , param2=0})
			core.swap_node(three_above_right_here, {name="bushes:BushLeaves1" })
			core.swap_node(three_above_right_here_south, {name="bushes:BushLeaves1" })
		end
	end
end
