...
if not crops.difficulty then
	...
	core.log("error", "[crops] "..S("Defaulting to \"normal\" difficulty settings"))
end
...
if crops.settings.hydration then
	core.log("action", "[crops] "..S("Hydration and dehydration mechanics are enabled."))
end
...
local find_plant = function(node)
	...
	core.log("error", "[crops] "..S("Unable to find plant \"@1\" in crops table", node.name))
	return nil
end
...
crops.particles = function(pos, flag)
	...
	elseif flag == 1 then
		...
		core.add_particlespawner({
			...
			minpos = { x = pos.x - .4, y = pos.y - .4, z = pos.z - .4 },
			maxpos = { x = pos.x + .4, y = pos.y - .4, z = pos.z + .4 },
			minvel = { x = -.04, y = 0, z = -.04 },
			maxvel = { x = .04, y = 0, z = .04 },
			minacc = { x = 0, y = 0, z = 0 },
			maxacc = { x = 0, y = 0, z = 0 },
			exptime = {min = 3, max = 5},
			minsize = 1,
	...
	elseif flag == 2 then
		...
		core.add_particlespawner({
			amount = 20,
			time = 30,
			minpos = { x = pos.x + .3, y = pos.y - .5, z = pos.z - .5 },
		...
		core.add_particlespawner({
			amount = 20,
			time = 30,
			minpos = { x = pos.x - .5, y = pos.y - .5, z = pos.z + .3 },
			maxpos = { x = pos.x + .5, y = pos.y + .5, z = pos.z + .5 },
			minvel = { z = -.6, y = -.1, x = -.2 },
			maxvel = { z = -.4, y = .1, x = .2 },
			minacc = { z = .4, y = 0, x = -.1 },
...
crops.die = function(pos)
	...
	core.sound_play("crops_flies", {pos=pos, gain=.8})
end
...
if crops.settings.hydration then
	core.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			local f = core.find_node_near(pos, 1, {"default:water_source", "default:water_flowing"})
			if not f == nil then
				water = math.min(100, water + 2)
			else
				f = core.find_node_near(pos, 2, {"default:water_source", "default:water_flowing"})
				if not f == nil then
					water = math.min(100, water + 1)
				end
			end

			if core.get_node_light(pos) < plant.properties.night then
			...
			if not plant.properties.doublesize == nil and plant.properties.doublesize then
				...
				local abovemeta = core.get_meta(above)
...
core.register_craftitem("crops:vegetable_stew", {
	description = S("Bowl of vegetable stew"),
	inventory_image = "crops_bowl_vegetable_stew.png",
	groups = { eatable=1 },
	on_use = core.item_eat(8, "vessels:clay_bowl"),
})
...
core.log("action", "[crops] "..S("Loaded!"))
