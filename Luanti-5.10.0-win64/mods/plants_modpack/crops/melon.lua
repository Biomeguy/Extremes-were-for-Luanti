...
core.register_node("crops:melon_seed", {
	...
	on_place = function(itemstack, placer, pointd)
		...
		crops.plant(pointd.above, {name="crops:melon_plant_1"})
...
for stage = 1, 6 do
core.register_node("crops:melon_plant_" .. stage , {
...
core.register_node("crops:melon_plant_5_attached", {
	visual = "mesh",
...
core.register_craftitem("crops:melon_slice", {
...
-- the melon "block"
--
core.register_node("crops:melon", {
	description = S("Melon"),
	tiles = {
		"crops_melon_top.png",
		...
		for face = 1, 4 do
			...
			if n.name == "crops:melon_plant_5_attached" then
				...
				if n.param2 == faces[face].o then
					core.swap_node(s, { name = "crops:melon_plant_4" })
				end
			end
		end
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		for face = 1, 4 do
			...
			if core.get_node(t).name == "crops:melon" then
				return
			end
		end
		...
		if core.registered_nodes[n.name].drawtype == "plantlike" or
		   ...
		   n.name == "air" then
			core.swap_node(pos, {name = "crops:melon_plant_5_attached", param2 = faces[r].r})
			core.set_node(t, {name = "crops:melon", param2 = faces[r].m})
			local meta = core.get_meta(pos)
			local damage = meta:get_int("crops_damage")
			local water = meta:get_int("crops_water")
			-- growing a melon costs 25 water!
			meta:set_int("crops_water", math.max(0, water - 25))
			meta = core.get_meta(t)
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		core.swap_node(pos, {name = "crops:melon_plant_4" })
	end
})

crops.melon_die = function(pos)
	core.set_node(pos, { name = "crops:melon_plant_6" })
end
