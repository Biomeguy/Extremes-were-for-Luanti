...
core.register_node("crops:potato_eyes", {
	...
	on_place = function(itemstack, placer, pointd)
		...
		crops.plant(pointd.above, {name="crops:potato_plant_1", param2 = 3})
...
for stage = 1, 5 do
core.register_node("crops:potato_plant_" .. stage , {
...
core.register_craftitem("crops:potato", {
...
core.register_node("crops:soil_with_potatoes", {
	...
	on_dig = function(pos, node, digger)
		...
		if core.get_node(above).name == "crops:potato_plant_4" then
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if not core.registered_nodes[core.get_node(below).name].groups.soil then
			return
		end
		local meta = core.get_meta(pos)
		local damage = meta:get_int("crops_damage")
		...
		n = string.gsub(n, "2", "3")
		n = string.gsub(n, "1", "2")
		core.swap_node(pos, { name = n, param2 = 3 })
	end
})
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local meta = core.get_meta(pos)
		local damage = meta:get_int("crops_damage")
		core.set_node(below, { name = "crops:soil_with_potatoes" })
		meta = core.get_meta(below)
		meta:set_int("crops_damage", damage)
	end
})

crops.potato_die = function(pos)
	core.set_node(pos, { name = "crops:potato_plant_5", param2 = 3 })
	local below = { x = pos.x, y = pos.y - 1, z = pos.z }
