...
core.register_node("crops:pumpkin_seed", {
	...
	on_place = function(itemstack, placer, pointd)
		...
		crops.plant(pointd.above, {name="crops:pumpkin_plant_1"})
...
for stage = 1, 6 do
core.register_node("crops:pumpkin_plant_" .. stage , {
...
core.register_node("crops:pumpkin_plant_5_attached", {
	visual = "mesh",
...
core.register_craftitem("crops:roasted_pumpkin", {
...
-- the pumpkin "block"
--
core.register_node("crops:pumpkin", {
	description = S("Pumpkin"),
	tiles = {"crops_pumpkin_top.png", "crops_pumpkin_bottom.png", "crops_pumpkin.png"},
		...
		for face = 1, 4 do
			...
			if n.name == "crops:pumpkin_plant_5_attached" then
				...
				if n.param2 == faces[face].o then
					core.swap_node(s, { name = "crops:pumpkin_plant_4" })
				end
			end
		end
	end
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		for face = 1, 4 do
			...
			if core.get_node(t).name == "crops:pumpkin" then
				return
			end
		end
		local r = math.random(1, 4)
		local t = { x = pos.x + faces[r].x, y = pos.y, z = pos.z + faces[r].z }
		local n = core.get_node(t)
		if n.name == "ignore" then
			return
		end

		if core.registered_nodes[core.get_node({ x = t.x, y = t.y - 1, z = t.z }).name].walkable == false then
			return
		end

		if core.registered_nodes[n.name].drawtype == "plantlike" or
		   core.registered_nodes[n.name].groups.flora == 1 or
		   n.name == "air" then
			core.set_node(t, {name = "crops:pumpkin", param2 = faces[r].m})
			...
			if ttl > 1 then
				core.swap_node(pos, {name = "crops:pumpkin_plant_5_attached", param2 = faces[r].r})
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local ttl = meta:get_int("crops_pumpkin_ttl")
		if ttl > 1 then
			core.swap_node(pos, { name = "crops:pumpkin_plant_4" })
...
crops.pumpkin_die = function(pos)
	core.set_node(pos, { name = "crops:pumpkin_plant_6" })
end
