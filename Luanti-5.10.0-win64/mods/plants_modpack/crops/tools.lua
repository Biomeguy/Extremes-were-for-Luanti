...
core.register_tool("crops:watering_can", {
	...
	on_use = function(itemstack, user, pointd)
		...
		if core.get_item_group(core.get_node(pos).name, "water") >= 3 then
			if wear ~= 1 then
				core.sound_play("crops_watercan_entering", {pos=pos, gain=0.8})
				core.after(math.random()/2, function(p)
					if math.random(2) == 1 then
						core.sound_play("crops_watercan_splash_quiet", {pos=p, gain=0.1})
					end
					if math.random(3) == 1 then
						core.after(math.random()/2, function(pp)
							core.sound_play("crops_watercan_splash_small", {pos=pp, gain=0.7})
						end, p)
					end
					if math.random(3) == 1 then
						core.after(math.random()/2, function(pp)
							core.sound_play("crops_watercan_splash_big", {pos=pp, gain=0.7})
						end, p)
					end
				end, pos)
		...
		if meta:get_int("crops_top_half") == 1 then
			meta = core.get_meta({x=pos.x, y=pos.y-1, z=pos.z})
		end
		-- using it on a plant?
		...
		core.sound_play("crops_watercan_watering", {pos=pos, gain=0.8})
...
core.register_tool("crops:hydrometer", {
