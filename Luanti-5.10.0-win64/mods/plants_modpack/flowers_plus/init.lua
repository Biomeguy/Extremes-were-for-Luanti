-- support for i18n
local S = core.get_translator("flowers_plus")
...
for i in ipairs(lilies_list) do
	...
	if lilies_list[i][1] ~= nil then
		...
		lily_groups = { snappy = 3,flammable=2,flower=1, not_in_creative_inventory=0 }
	end

	core.register_node(":flowers:waterlily"..deg1, {
		...
		on_place = function(itemstack, placer, pointd)
			...
			local nodename = "default:cobble" -- if this block appears, something went....wrong :-)
				...
				if not keys["sneak"] then
					...
					core.swap_node(place_pos, {name = "flowers:waterlily", param2 = fdir})
				end
...
for i in ipairs(algae_list) do
	...
	if algae_list[i][1] ~= nil then
		...
		algae_groups = { snappy = 3,flammable=2,flower=1, not_in_creative_inventory=0 }
	end

	core.register_node(":flowers:seaweed"..num, {
		...
		on_place = function(itemstack, placer, pointd)
			...
			if not core.is_protected(place_pos, placer:get_player_name()) then
			...
			local nodename = "default:cobble" -- :D
				...
				if not keys["sneak"] then
					...
					local fdir = core.dir_to_facedir(placer:get_look_dir())
					core.swap_node(place_pos, {name = "flowers:seaweed", param2 = fdir})
				end
...
flowers_plus.grow_waterlily = function(pos)
	...
	for i in ipairs(lilies_list) do
		...
		if chance == num then
			core.swap_node(right_here, {name="flowers:waterlily".. ext, param2=math.random(0,3)})
		end
	end
end
...
flowers_plus.grow_seaweed = function(pos)
	local right_here = {x=pos.x, y=pos.y+1, z=pos.z}
	core.swap_node(right_here, {name="flowers:seaweed_"..math.random(2,4), param2=math.random(1,3)})
end
