...
local function notify(name, retcode)
	if(retcode == false) then
		core.chat_send_player(name, "Effect application failed. Effect was NOT applied.")
	else
		core.chat_send_player(name, "Effect applied. Effect ID: "..tostring(retcode))
	end
end
...
playereffects.register_effect_type("blind", "Blind", nil, {},
	function(player)
		...
		if(hudid ~= nil) then
			...
			core.log("error", "[playereffects] [examples] The effect \"Blind\" could not be applied. The call to hud_add(...) failed.")
			return false
		end
	end,
...
playereffects.register_effect_type("fly", "Fly mode available", "playereffects_example_fly.png", {"fly"},
	...
	function(effect, player)
		local privs = core.get_player_privs(effect.pname)
		privs.fly = nil
		core.set_player_privs(effect.pname, privs)
	end,
	false, -- not hidden
...
core.register_chatcommand("null", {
	params = "",
	description = "Does nothing.",
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("null", 5, core.get_player_by_name(name))
		notify(name, ret)
	end,
})
...
core.register_chatcommand("blind", {
	...
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("blind", 5, core.get_player_by_name(name))
		notify(name, ret)
	end,
})
core.register_chatcommand("fast", {
	params = "",
	description = "Makes you fast for a short time.",
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("high_speed", 10, core.get_player_by_name(name))
		notify(name, ret)
	end,
})
core.register_chatcommand("hfast", {
	params = "",
	description = "Makes you fast for a short time (hidden effect).",
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("high_speed_hidden", 10, core.get_player_by_name(name))
		notify(name, ret)
	end,
})
core.register_chatcommand("slow", {
	params = "",
	description = "Makes you slow for a long time.",
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("low_speed", 120, core.get_player_by_name(name))
		notify(name, ret)
	end,
})
core.register_chatcommand("highjump", {
	params = "",
	description = "Makes you jump higher for a short time.",
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("highjump", 20, core.get_player_by_name(name))
		notify(name, ret)
	end,
})

core.register_chatcommand("fly", {
	params = "",
	description = "Grants you the fly privilege for a minute. You keep the effect when you die.",
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("fly", 60, core.get_player_by_name(name))
		notify(name, ret)
	end,
})
...
core.register_chatcommand("regen", {
	...
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("regen", 10, core.get_player_by_name(name))
		notify(name, ret)
	end,
})

core.register_chatcommand("slowregen", {
	params = "",
	description = "Gives you 1 half heart every 3 seconds 10 times, healing you by 5 hearts in total.",
	privs = {},
	func = function(name, param)
		local ret = playereffects.apply_effect_type("slowregen", 10, core.get_player_by_name(name))
		notify(name, ret)
	end,
})
...
core.register_chatcommand("cancelall", {
	params = "",
	...
	func = function(name, param)
		...
		core.chat_send_player(name, "All effects cancelled.")
	end,
})
...
core.register_chatcommand("stresstest", {
	params = "[<effects>]",
	...
	func = function(name, param)
		...
		core.debug("[playereffects] Stress test started for "..name.." with "..max.." effects.")
		for i=1,max do
			playereffects.apply_effect_type("stress", math.random(6,60), player)
			if(i%100==0) then
				core.debug("[playereffects] Effect "..i.." of "..max.." applied.")
				core.chat_send_player(name, "[playereffects] Effect "..i.." of "..max.." applied.")
