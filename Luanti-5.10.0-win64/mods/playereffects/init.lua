...
--[=[ Include settings ]=]
dofile(core.get_modpath("playereffects").."/settings.lua")
...
do
	local filepath = core.get_worldpath().."/playereffects.mt"
	local file = io.open(filepath, "r")
	if file then
		core.log("action", "[playereffects] playereffects.mt opened.")
		local string = file:read()
		io.close(file)
		if(string ~= nil) then
			local savetable = core.deserialize(string)
			playereffects.inactive_effects = savetable.inactive_effects
			core.debug("[playereffects] playereffects.mt successfully read.")
			core.debug("[playereffects] inactive_effects = "..dump(playereffects.inactive_effects))
			playereffects.last_effect_id = savetable.last_effect_id
			core.debug("[playereffects] last_effect_id = "..dump(playereffects.last_effect_id))
...
function playereffects.register_effect_type(effect_type_id, description, icon, groups, apply, cancel, hidden, cancel_on_death, repeat_interval)
	...
	core.log("action", "[playereffects] Effect type "..effect_type_id.." registered!")
end
...
function playereffects.apply_effect_type(effect_type_id, duration, player, repeat_interval_time_left)
	...
	for k,v in pairs(groups) do
		playereffects.cancel_effect_group(v, pname)
	end
	...
	if(playereffects.effect_types[effect_type_id].repeat_interval == nil) then
		...
		if(status == false) then
			core.log("action", "[playereffects] Attempt to apply effect type "..effect_type_id.." to player "..pname.." failed!")
			return false
		else
			metadata = status
		end
	...
	local free_hudpos
	if(playereffects.hudinfos[pname] == nil) then
		playereffects.hudinfos[pname] = {}
	end
	local hudinfos = playereffects.hudinfos[pname]
	...
	if(free_hudpos <= 20) then
		...
		playereffects.hudinfos[pname][effect_id] = hudinfo
	else
...
function playereffects.cancel_effect_type(effect_type_id, cancel_all, pname)
	local effects = playereffects.get_player_effects(pname)
...
function playereffects.cancel_effect_group(groupname, pname)
	local effects = playereffects.get_player_effects(pname)
	for e=1,#effects do
...
function playereffects.cancel_effect(effect_id)
	local effect = playereffects.effects[effect_id]
	if(effect ~= nil) then
		local player = core.get_player_by_name(effect.pname)
		local hudinfo = playereffects.hudinfos[effect.pname][effect_id]
		if(hudinfo ~= nil) then
			...
			playereffects.hudinfos[effect.pname][effect_id] = nil
		end
...
function playereffects.has_effect_type(pname, effect_type_id)
	local pe = playereffects.get_player_effects(pname)
	for i=1,#pe do
...
function playereffects.save_to_file()
	...
	for id,effecttable in pairs(playereffects.inactive_effects) do
		local pname = id
		if(inactive_effects[pname] == nil) then
			inactive_effects[pname] = {}
		end
		for i=1,#effecttable do
			table.insert(inactive_effects[pname], effecttable[i])
		end
	end
	for id,effect in pairs(playereffects.effects) do
		...
		if(inactive_effects[effect.pname] == nil) then
			inactive_effects[effect.pname] = {}
		end
		table.insert(inactive_effects[effect.pname], new_effect)
	end
...
core.register_on_leaveplayer(function(player)
	...
	if(playereffects.inactive_effects[pname] == nil) then
		playereffects.inactive_effects[pname] = {}
	end
	for e=1,#effects do
		...
		table.insert(playereffects.inactive_effects[pname], new_effect)
		playereffects.cancel_effect(effects[e].effect_id)
	end
end)

core.register_on_shutdown(function()
	core.log("action", "[playereffects] Server shuts down. Rescuing data into playereffects.mt")
...
core.register_on_joinplayer(function(player)
	...
	if(playereffects.inactive_effects[pname] ~= nil) then
		for i=1,#playereffects.inactive_effects[pname] do
			local effect = playereffects.inactive_effects[pname][i]
			playereffects.apply_effect_type(effect.effect_type_id, effect.time_left, player, effect.repeat_interval_time_left)
		end
		playereffects.inactive_effects[pname] = nil
	end
end)
...
core.register_globalstep(function(dtime)
	...
	if(playereffects.use_autosave == true and playereffects.autosave_timer >= playereffects.autosave_time) then
		playereffects.autosave_timer = 0
		core.log("action", "[playereffects] Autosaving mod data to playereffects.mt ...")
		playereffects.save_to_file()
	end
end)
	...
	if(playereffects.use_hud == true) then
		...
		if(hudinfos ~= nil) then
			for effect_id, hudinfo in pairs(hudinfos) do
				...
				playereffects.hudinfos[pname][effect_id] = nil
			end
		end
	end
end
