...
entityInv = core.create_detached_inventory(
   "prestibags:bags",
   {
...
core.register_entity(
	...
	initial_properties = {
		hp_max = BAG_MAX_HP,
		physical = false,
		collisionbox = {-.44, -.5, -.425, .44, .35, .425},
		visual = "mesh",
		visual_size = { x = 1, y = 1 },
      ...
      on_punch = function(self, hitterObj, timeSinceLastPunch, toolCaps, dir)
         local pname = hitterObj:get_player_name()
         local playerInv = hitterObj:get_inventory()
         if not pname or not playerInv then return end
      ...
      on_rightclick = function(self, player)
         local w = math.max(8, 1 + BAG_WIDTH)
         local h = math.max(4.7 + BAG_HEIGHT)
         ...
         local formspec =
            "size["..w..","..h.."]"..
            "image[0,"..yImg..";1,1;prestibags_bag_inv.png]"..
            "list[detached:prestibags:bags;"..self.id..";1,0;"..
      ...
      on_step = function(self, dt)
         ...
         if core.find_node_near(pos, 1.0, "group:puts_out_fire") then
            return
         end
         ...
         local igniterPosList =
            core.find_nodes_in_area(minPos, maxPos, "group:igniter")
         for i, igniterPos in ipairs(igniterPosList) do
            ...
            if distSq <= MAX_IGNITE_DIST^2 + EPSILON then
               ...
               local igniterLevel =
                  core.get_item_group(igniterNode.name, "igniter")
         ...
         if burnLevels >= 1.0 then
            ...
            then
               local flamePos =
                  (node.name == "air")
                     and pos
                     or core.find_node_near(pos, 1.0, "air")
               if flamePos then
                  core.add_node(flamePos,
                                        { name = "fire:basic_flame" })
               end
...
local function rezEntity(stack, pos, player)
   ...
   local obj = core.add_entity(pos, "prestibags:bag_entity")
...
-- DEBUG core.register_craftitem(
core.register_tool("prestibags:bag", {
...
core.register_craft({
	...
	recipe = {
		{"", "group:fabric_block", ""},
		{"group:fabric_block", "", "group:fabric_block"},
		{"group:fabric_block", "group:fabric_block", "group:fabric_block"}
	}
})
