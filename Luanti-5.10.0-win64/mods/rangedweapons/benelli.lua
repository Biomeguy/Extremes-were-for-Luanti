core.register_craft({
	...
	recipe = {
		{"rangedweapons:gunsteel_ingot", "default:diamond", "default:steel_block"},
		{"rangedweapons:plastic_sheet", "default:diamond", "default:steel_ingot"}
	}
})

core.register_craftitem("rangedweapons:benelli_uld", {
	stack_max= 1,
...
core.register_craftitem("rangedweapons:benelli_rld", {
	stack_max= 1,
	range = 0,
	wield_scale = {x=2.0,y=2.0,z=1.20},
...
core.register_tool("rangedweapons:benelli", {
	...
	on_use = function(itemstack, user, pointd)
		...
		if pos and dir and yaw then
			pos.y = pos.y + 1.6
			local obj = core.add_entity(pos, "rangedweapons:benellishot")
			if obj then
				core.sound_play("rangedweapons_shotgun_shot", {object=obj})
				obj:set_velocity({x=dir.x * math.random(18,22), y=dir.y * math.random(18,22), z=dir.z * math.random(18,22)})
				obj:set_acceleration({x=dir.x * math.random(-3.8,3.8), y=math.random(-3.8,3.8), z=dir.z * math.random(-3.8,3.8)})
-----------------------------------------------------
			local obj = core.add_entity(pos, "rangedweapons:benellishot")
				obj:set_velocity({x=dir.x * math.random(18,22), y=dir.y * math.random(18,22), z=dir.z * math.random(18,22)})
				obj:set_acceleration({x=dir.x * math.random(-3.8,3.8), y=math.random(-3.8,3.8), z=dir.z * math.random(-3.8,3.8)})
...
core.register_entity("rangedweapons:benellishot", rangedweapons_benellishot )
