core.register_craft({
	...
	recipe = {
		...
		{"default:beech_trunk", "default:mese_crystal", "default:beech_trunk"}
	}
})

core.register_craftitem("rangedweapons:remington_uld", {
	stack_max= 1,
...
core.register_globalstep(function(dtime, player)
	...
	for _, player in pairs(core.get_connected_players()) do
		...
		timer = 0
				core.sound_play("rangedweapons_shotgun_reload_a", {player})
	...
	if timer >= 0.8 and
		...
		timer = 0
				core.sound_play("rangedweapons_shotgun_reload_b", {player})
		end
			end
				end
				end)

core.register_craftitem("rangedweapons:remington_rld", {
	stack_max= 1,
	range = 0,
	wield_scale = {x=1.9,y=1.9,z=1.1},
...
core.register_tool("rangedweapons:remington", {
	...
	on_use = function(itemstack, user, pointd)
		...
		if pos and dir and yaw then
			pos.y = pos.y + 1.6
			local obj = core.add_entity(pos, "rangedweapons:remingtonshot")
			if obj then
				core.sound_play("rangedweapons_shotgun_shot", {object=obj})
				obj:set_velocity({x=dir.x * math.random(13,17), y=dir.y * math.random(13,17), z=dir.z * math.random(13,17)})
				obj:set_acceleration({x=dir.x * math.random(-4.0,4.0), y=math.random(-4.0,4.0), z=dir.z * math.random(-4.0,4.0)})
-----------------------------------------------------
			local obj = core.add_entity(pos, "rangedweapons:remingtonshot")
				obj:set_velocity({x=dir.x * math.random(13,17), y=dir.y * math.random(13,17), z=dir.z * math.random(13,17)})
				obj:set_acceleration({x=dir.x * math.random(-4.0,4.0), y=math.random(-4.0,4.0), z=dir.z * math.random(-4.0,4.0)})
...
core.register_entity("rangedweapons:remingtonshot", rangedweapons_remingtonshot )
