...
core.register_craftitem("rangedweapons:spas12_uld", {
	stack_max= 1,
...
core.register_craftitem("rangedweapons:spas12_rld", {
	stack_max= 1,
	range = 0,
	wield_scale = {x=2.1,y=2.1,z=1.25},
...
core.register_tool("rangedweapons:spas12", {
	...
	on_use = function(itemstack, user, pointd)
		...
		if pos and dir and yaw then
			pos.y = pos.y + 1.6
			local obj = core.add_entity(pos, "rangedweapons:spas12shot")
			if obj then
				...
				obj:set_acceleration({x=dir.x * math.random(-3.3,3.3), y=math.random(-3.3,3.3), z=dir.z * math.random(-3.3,3.3)})
-----------------------------------------------------
			local obj = core.add_entity(pos, "rangedweapons:spas12shot")
				obj:set_velocity({x=dir.x * math.random(18,22), y=dir.y * math.random(18,22), z=dir.z * math.random(18,22)})
				obj:set_acceleration({x=dir.x * math.random(-3.3,3.3), y=math.random(-3.3,3.3), z=dir.z * math.random(-3.3,3.3)})
...
core.register_entity("rangedweapons:spas12shot", rangedweapons_spas12shot )
