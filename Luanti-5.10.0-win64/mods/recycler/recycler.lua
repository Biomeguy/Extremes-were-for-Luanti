...
local no_recycle_list  = {
	...
	["dye:white"]=1,["dye:grey"]=1,["dye:dark_grey"]=1,["dye:black"]=1,["dye:violet"]=1,
	["dye:light_blue"]=1,["dye:blue"]=1,["dye:cyan"]=1,["dye:dark_green"]=1,
...
local recycler_process = function(pos) 
	...
	if fuel-1<0 then -- we need new fuel
		local fuellist = inv:get_list("fuel") 
		if not fuellist then	return end

		local fueladd, afterfuel = core.get_craft_result({method = "fuel", width = 1, items = fuellist}) 
		...
		if fueladd.time == 0 then
			...
			fueladd.time = fueladd.time*.1 -- thats 4 for coal
		end 
		if fueladd.time>0 then 
	...
	-- RECYCLING: check inserted items
	...
	reqcount = 1 -- needed count of materials for recycler to work
	...
	if not known_recipe then
		...
		local recipe = core.get_all_craft_recipes( src_item )
		...
		meta:set_string("itemlist", core.serialize(itemlist)) -- read cached itemlist
		meta:set_int("reqcount", reqcount)
	else 
		itemlist= core.deserialize(meta:get_string("itemlist")) or {}
	...
	stack= stack:take_item(reqcount)
	inv:remove_item("src", stack)

	for _,  v in pairs(itemlist) do
		if math.random(1, 4)<=3 then -- probability 3/4 = 75%	or	6)<=5 then
			if not string.find(v,"group") then -- dont add if item described with group
				local par = string.find(v,"\"") or 0
				if inv:room_for_item("dst", ItemStack(v)) then
	...
	core.sound_play("recycler", {pos=pos, gain=.5, max_hear_distance = 16})

	fuel = fuel-1 -- burn fuel on succesful operation
	meta:set_float("fuel", fuel)
	meta:set_string("infotext", "fuel status ".. fuel ..", recycling ".. meta:get_string("node"))
end
...
core.register_node("recycler:recycler", {
	...
	after_place_node = function(pos, placer)
		local meta = core.get_meta(pos)
		meta:set_string("infotext", "Recycler: put one item in it (src) and obtain 75% of raw materials (dst). To operate it insert fuel, then insert item to recycle.")
	...
	on_rightclick = function(pos, node, player, itemstack, pointd)
		...
		if core.is_protected(pos, player:get_player_name()) and not privs.privs then return end -- only owner can interact with recycler
	...
	on_receive_fields = function(pos, formname, fields, sender) 
		...
		meta:set_string("node", "") -- this will force to re-read recipe on next use
		recycler_update_meta(pos)
	end,
	...
	can_dig = function(pos)
		...
		if not inv:is_empty("fuel") or not inv:is_empty("src") or not inv:is_empty("dst") then return false end
		return true
	end
})
...
core.register_craft({
	...
	recipe = {
		{"default:diamond", "basic_materials:copper_wire", "default:wrought_iron_ingot"},
		{"default:brass_ingot", "default:mese", "basic_materials:plastic_sheet"},
		{"default:wrought_iron_ingot", "basic_materials:plastic_sheet", "dye:dark_green"}
	},
...
core.register_craft({
	...
	recipe = {
		{"default:diamond", "basic_materials:copper_wire", "default:wrought_iron_ingot"},
		{"default:brass_ingot", "basic_materials:motor", "basic_materials:plastic_sheet"},
		{"default:wrought_iron_ingot", "basic_materials:plastic_sheet", "dye:dark_green"}
	},
	replacements = {{"basic_materials:copper_wire", "basic_materials:empty_spool"}}
})