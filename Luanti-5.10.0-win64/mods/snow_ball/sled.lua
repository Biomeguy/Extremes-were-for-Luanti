...
if they flew while on the sled.
...
core.after(.2, function()
	...
	self.driver = name
	self.object:set_attach(player, "", {x=0,y=2,z=0}, {x=0,y=90,z=0})
...
local function leave_sled(self, player)
	...
	player:get_inventory():add_item("main", "snow_ball:sled")
end
...
local function sled_rightclick(self, player)
	...
	player:set_physics_override({speed = 2, jump = 0})
...
function sled:on_punch(puncher)
	...
	and puncher:is_player() then
		puncher:get_inventory():add_item("main", "snow_ball:sled")
	end
end

local driveable_nodes = {"default:snow","default:snow_block","default:ice","default:dirt_with_snow", "group:icemaker"}
local function accelerating_possible(pos)
	if is_water(pos) then
		return false
	end
	if table_find(driveable_nodes, core.get_node({x=pos.x, y=pos.y-1, z=pos.z}).name) then
		return true
	end
	return false
end
...
core.register_entity("snow_ball:sled", sled)


core.register_craftitem("snow_ball:sled", {
	...
	on_use = function(itemstack, placer)
		...
		if accelerating_possible(vector.round(pos)) then
			...
			core.add_entity(pos, "snow_ball:sled"):right_click(placer)
...
core.register_craft({
	output = "snow_ball:sled",
	recipe = {
		{"group:stick", "", "group:stick"},
		{"group:fabric", "group:wood", "group:fabric"},
		{"group:stick", "", "group:stick"}
	}
})
