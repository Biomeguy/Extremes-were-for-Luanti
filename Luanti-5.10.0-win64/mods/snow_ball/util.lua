...
local function saveConfig(path, config, doc)
	...
	if not file then
		core.log("error", "[snow] could not open config file for writing at "..path)
		return
	end
...
local modpath = core.get_modpath("snow_ball")
--[[
...
if config then
	for i,v in pairs(config) do
		if type(snow[i]) == type(v) then
			...
			core.log("error", "[snow] wrong type of setting "..i)
		end
	end
else
	saveConfig(modpath.."/config.txt", snow, doc)
...
-- load settings from core.conf

for i,v in pairs(snow) do
	if allowed_types[type(v)] then
		local v = core.settings:get("snow_"..i)
		if v ~= nil then
...
local function get_formspec()
	...
	for n = 1,n1-1 do
		...
		p = p + 0.5
		formspec = formspec.."checkbox[0,"..p..";snow_ball:"..i..";"..i..";"..tostring(v).."]"
	end
	for n = 1,n2-1 do
		local i,v = unpack(ids[2][n])
		p = p + 1.5
		formspec = formspec.."field[0.3,"..p..";2,1;snow_ball:"..i..";"..i..";"..v.."]"
	end
	p = p + 1
	formspec = "size[4,"..p..";]\n"..formspec
	return formspec
...
core.register_chatcommand("snow_show", {
	description = "Show a menu for various actions",
	privs = {server=true},
	func = function(name)
		core.chat_send_player(name, "Showing snow menu…")
		core.show_formspec(name, "snow_ball:menu", get_formspec())
	end,
})

core.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "snow_ball:menu" then
		return
	end
	for i,v in pairs(snow) do
		local t = type(v)
		if allowed_types[t] then
			local field = fields["snow_ball:"..i]
			if field then
				if t == "number" then
