...
core.register_craft({
	...
	recipe = {
		...
		{"default:steel_block", "technic:hv_cable", "default:steel_block"}}
})
...
local function set_quarry_formspec(meta)
	...
	formspec = formspec.."label[0,4;"..core.formspec_escape(
...
local function quarry_run(pos, node)
	...
	if meta:get_int("enabled") and meta:get_int("HV_EU_input") >= quarry_demand and meta:get_int("purge_on") == 0 then
		local pdir = core.facedir_to_dir(node.param2)
		if pdir.y ~= 0 then
		...
		while nd ~= diameter*diameter * (quarry_dig_above_nodes+1+quarry_max_depth) do
			...
			local can_dig = true
			if can_dig and core.is_protected and core.is_protected(digpos, owner) then
				can_dig = false
			end
			local dignode
			if can_dig then
				dignode = technic.get_or_load_node(digpos) or core.get_node(digpos)
				local dignodedef = core.registered_nodes[dignode.name] or {diggable=false}
			...
			if can_dig then
				for ay = startpos.y, digpos.y+1, -1 do
					...
					local checknode = technic.get_or_load_node(checkpos) or core.get_node(checkpos)
			...
			nd = nd + 1
			if can_dig then
				core.remove_node(digpos)
				local drops = core.get_node_drops(dignode.name, "")
...
core.register_node("technic:quarry", {
	...
	tiles = {
		"default_steel_block.png"..tube_entry,
		"default_steel_block.png"..cable_entry,
		"default_steel_block.png"..cable_entry,
		"default_steel_block.png"..cable_entry,
		"default_steel_block.png^default_tool_mesepick.png",
		"default_steel_block.png"..cable_entry
	},
	paramtype2 = "facedir",
