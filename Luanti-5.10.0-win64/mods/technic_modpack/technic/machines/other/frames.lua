...
local infinite_stacks = core.settings:get_bool("creative_mode")
	and core.get_modpath("unified_inventory") == nil
...
local function move_nodes_vect(poslist, vect, must_not_move, owner)
	if core.is_protected then
		for _, pos in ipairs(poslist) do
			local npos = vector.add(pos, vect)
			if core.is_protected(pos, owner) or core.is_protected(npos, owner) then
				return
			end
		end
	end
	...
	for _, pos in ipairs(poslist) do
		...
		if (name ~= "air" and core.registered_nodes[name].liquidtype == "none" or
	...
	for _, n in ipairs(nodelist) do
		local npos = n.pos
		core.set_node(npos, n.node)
		local meta = core.get_meta(npos)
		meta:from_table(n.meta)
		local timer = core.get_node_timer(npos)
...
for zp = 0, 1 do
	...
	core.register_node("technic:frame_"..nameext, {
		description = S("Frame"),
		...
		on_place = function(itemstack, placer, pointd)
			...
			if node.name ~= "air" then
				if is_supported_node(node.name) then
					obj = core.add_entity(pos, "technic:frame_entity")
		...
		on_rightclick = function(pos, node, placer, itemstack, pointd)
			if is_supported_node(itemstack:get_name()) then
				...
				--local pointd = { type = "node", under = pos }
				if pointd then
...
core.register_entity("technic:frame_entity", {
	initial_properties = {
		physical = true,
		collisionbox = {-.5, -.5, -.5, .5, .5, .5},
		visual = "wielditem",
		textures = {},
		visual_size = {x = .667, y = .667},
	},
	...
	dig = function(self)
		core.handle_node_drops(self.object:get_pos(), { ItemStack("technic:frame_111111") }, self.last_puncher)
		local pos = vector.round(self.object:get_pos())
	...
	on_punch = function(self, puncher, time_from_last_punch, tool_capabilities, dir)
		...
		if self.damage_object == nil then
			self.damage_object = core.add_entity(pos, "technic:damage_entity")
	...
	on_rightclick = function(self, clicker)
		...
		local pointd = { type = "node", under = pos_under, above = pos_above }
		local itemstack = clicker:get_wielded_item()
		local itemdef = core.registered_items[itemstack:get_name()]

		if itemdef ~= nil then
			itemdef.on_place(itemstack, clicker, pointd)
		end
	end,
})

local crack = "crack_anylength.png^[verticalframe:5:0"
core.register_entity("technic:damage_entity", {
	initial_properties = {
		visual = "cube",
...
mesecon.register_on_mvps_move(function(moved_nodes)
	...
	if #to_move > 0 then
		for _, t in ipairs(to_move) do
			...
			local objects = core.get_objects_inside_radius(t.oldpos, 0.1)
			for _, obj in ipairs(objects) do
...
core.register_on_dignode(function(pos, node)
	if frames_pos[pos_to_string(pos)] ~= nil then
		core.set_node(pos, { name = frames_pos[pos_to_string(pos)] })
		frames_pos[pos_to_string(pos)] = nil
...
local function connected(pos, c, adj)
	for _, vect in ipairs(adj) do
		...
		local nodename = core.get_node(pos1).name
		...
		if not pos_in_list(c, pos1) and nodename ~= "air" and
				(core.registered_nodes[nodename].frames_can_connect == nil or
				core.registered_nodes[nodename].frames_can_connect(pos1, vect)) then
			c[#c + 1] = pos1
			if core.registered_nodes[nodename].frame == 1 then
				local adj = core.registered_nodes[nodename].frame_connect_all(nodename)
...
local function get_connected_nodes(pos)
	...
	connected(pos, c, core.registered_nodes[nodename].frame_connect_all(nodename))
	return c
end
...
local function frame_motor_on(pos, node)
	...
	local nnode = core.get_node(nnodepos)
	...
	if meta:get_int("last_moved") == core.get_gametime() then
		return
	end

	local owner = meta:get_string("owner")
	if core.registered_nodes[nnode.name].frame == 1 then
		local connected_nodes = get_connected_nodes(nnodepos)
		move_nodes_vect(connected_nodes, dir, pos, owner)
	end

	core.get_meta(vector.add(pos, dir)):set_int("last_moved", core.get_gametime())
end

core.register_node("technic:frame_motor", {
...
local function get_templates(pos)
	local c = { pos }
	local connectors
	if core.get_node(pos).name == "technic:template_connector" then
		connectors = { pos }
	else
		connectors = {}
	end
...
local function save_node(pos)
	local node = core.get_node(pos)
	if node.name == "air" then
		core.set_node(pos, { name = "technic:template" })
		return
	end
...
local function expand_template(pos)
	...
	for _, vect in ipairs(c) do
		...
		if saved_node ~= nil then
			meta1:set_string("saved_node", core.serialize(saved_node))
		end
	end
end
...
local function compress_templates(pos)
	...
	for _, cn in ipairs(connectors) do
		...
		meta:set_string("connected", core.serialize(c))
		meta:set_string("connectors_connected", core.serialize(cc))
	end
		...
		if cc == "" or cc == nil then
			...
			if not table_empty(dcc) then
				...
				for sp, _ in pairs(dcc) do
					...
					local d = core.deserialize(meta:get_string("connectors_connected"))
					if d ~= nil then
						d[pos_to_string({ x = -ssp.x, y = -ssp.y, z = -ssp.z })] = nil
						meta:set_string("connectors_connected", core.serialize(d))
					end
				end
			else
				local stack_max = 99
				local num = #core.deserialize(c)
				drops = {}
...
local function template_on_destruct(pos, node)
	...
	local saved_node = meta:get_string("saved_node")
	if saved_node ~= "" then
		local nnode = core.deserialize(saved_node)
		core.after(0, restore_node, pos, nnode)
	end
end

core.register_node("technic:template", {
...
core.register_node("technic:template_disabled", {
	description = S("Template"),
...
core.register_node("technic:template_connector", {
	description = S("Template"),
...
core.register_craftitem("technic:template_replacer", {
	description = S("Template (replacing)"),
	inventory_image = "technic_template_replacer.png",
	on_place = function(itemstack, placer, pointd)
		local p = pointd.under
		if core.is_protected and core.is_protected(p, placer:get_player_name()) then
			return nil
		end
		...
		if saved_node ~= nil then
			local meta = core.get_meta(p)
			meta:set_string("saved_node", core.serialize(saved_node))
		end
		return itemstack
	end
})

core.register_tool("technic:template_tool", {
	description = S("Template Tool"),
	inventory_image = "technic_template_tool.png",
	on_use = function(itemstack, puncher, pointd)
		local pos = pointd.under
		if pos == nil or core.is_protected and core.is_protected(pos, puncher:get_player_name()) then
			return nil
		end
		local node = core.get_node(pos)
...
local function get_template_nodes(pos)
	...
	local adj = core.deserialize(connected)
	local c = {}
	for _, vect in ipairs(adj) do
		local pos1 = vector.add(pos, vect)
		local nodename = core.get_node(pos1).name
...
core.register_node("technic:template_motor", {
	description = S("Template Motor"),
	tiles = {"pipeworks_filter_top.png^[transformR90", "technic_lv_cable.png"},
	groups = { snappy = 2, choppy = 2, oddly_breakable_by_hand = 2, mesecon = 2 },
	paramtype2 = "facedir",
...
core.register_craft({
	...
	recipe = {
		{'', 'default:stick', ''},
		{'default:stick', 'default:brass_ingot', 'default:stick'},
		{'', 'default:stick', ''}
	}
})
...
core.register_craft({
	...
	recipe = {
		{'', 'default:brass_ingot', ''},
		{'default:brass_ingot', 'default:mese_crystal', 'default:brass_ingot'},
		{'', 'default:brass_ingot', ''}
	}
})
