...
core.register_craft({
	output = "technic:battery",
	recipe = {
		{"group:wood", "default:sulfur", "group:wood"},
		{"technic:lead_ingot", "group:salt_water_bucket", "technic:lead_ingot"},
		{"group:wood", "default:sulfur", "group:wood"}
	},
	replacements = {
...
core.register_tool("technic:battery", {
...
local tube = {
	insert_object = function(pos, node, stack, direction)
		print(core.pos_to_string(direction), dirtab[direction.x+2+(direction.z+2)*2], node.param2)
		if direction.y == 1
...
function technic.register_battery_box(data)
	...
	for i = 0, 8 do
		...
		core.register_node("technic:"..ltier.."_battery_box"..i, {
			description = S("%s Battery Box"):format(tier),
			tiles = {top_tex, bottom_tex, side_tex, side_tex, side_tex, front_tex},
			groups = groups,
			...
			on_construct = function(pos)
				...
				meta:set_string("channel", ltier.."_battery_box"..core.pos_to_string(pos))
			...
			on_receive_fields = function(pos, formname, fields, sender)
				...
				if fields.edit_channel then
					core.show_formspec(sender:get_player_name(),
						"technic:battery_box_edit_channel"..core.pos_to_string(pos),
...
core.register_on_player_receive_fields(
	function(player, formname, fields)
		...
		local pos = core.string_to_pos(formname:sub(33))
...
function technic.charge_tools(meta, batt_charge, charge_step)
	...
	local src_meta = core.deserialize(src_stack:get_meta():get_string("")) or {}
	...
	src_stack:get_meta():set_string("", core.serialize(src_meta))
...
function technic.discharge_tools(meta, batt_charge, charge_step, max_charge)
	...
	-- Set meta data for the tool if it didn't do it itself :-(
	local src_meta = core.deserialize(srcstack:get_meta():get_string(""))
	...
	src_meta.charge = tool_charge
	srcstack:get_meta():set_string("", core.serialize(src_meta))
