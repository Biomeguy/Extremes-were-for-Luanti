...
local function clear_networks(pos)
	...
	for _,connected_pos in pairs(positions) do
		local net = technic.cables[core.hash_node_position(connected_pos)]
		if net and technic.networks[net] then
			if dead_end and placed then
				-- Dead end placed, add it to the network
				-- Get the network
				local network_id = technic.cables[core.hash_node_position(positions[1])]
				...
				technic.cables[core.hash_node_position(pos)] = network_id
				pos.visited = 1
				if technic.is_tier_cable(name, tier) then
					table.insert(network.all_nodes,pos)
				elseif technic.machines[tier][node.name] then
					meta:set_string(tier.."_network",core.pos_to_string(sw_pos))
			...
			elseif dead_end and not placed then
				...
				technic.cables[core.hash_node_position(pos)] = nil
				...
				for _,v in pairs(technic.networks[net].all_nodes) do
					local pos1 = core.hash_node_position(v)
...
function technic.register_cable(tier, size)
	...
	core.register_node("technic:"..ltier.."_cable", {
	...
	for p, i in pairs(xyz) do
		...
		if i == 1 then
			def.on_place = function(itemstack, placer, pointd)
				local pointed_thing_diff = vector.subtract(pointd.above, pointd.under)
				local num
				local changed
				...
				if (crtl.aux1 or crtl.sneak) and not (crtl.aux1 and crtl.sneak) then
					local fine_pointed = core.pointed_thing_to_face_pos(placer, pointd)
					fine_pointed = vector.subtract(fine_pointed, pointd.above)
		...
		def.on_rotate = function(pos, node, user, mode, new_param2)
			...
			num = (num <= 6 and num) or num - 6
			core.swap_node(pos, {name = "technic:"..ltier.."_cable_plate_"..num})
		end
		core.register_node("technic:"..ltier.."_cable_plate_"..i, def)
...
core.register_on_placenode(clear_nets_if_machine)
core.register_on_dignode(clear_nets_if_machine)
