...
function technic.smelt_item(meta, result, speed)
	...
	local result
	local afterfuel
	result, afterfuel = core.get_craft_result({method = "cooking", width = 1, items = inv:get_list("src")})
...
local function inv_change(pos, player, count, from_list, to_list, stack)
	...
	if (not public or to_upgrade or from_upgrade) and core.is_protected(pos, pname) then
		core.chat_send_player(pname, S("Inventory move disallowed due to protection"))
		return 0
	end
	if to_upgrade then
