...
function technic.register_recipe(typename, data)
	core.after(0.01, register_recipe, typename, data) -- Handle aliases
end

function technic.get_recipe(typename, items)
	if typename == "cooking" then -- Already builtin in Luanti, so use that
		local result, new_input = core.get_craft_result({
			method = "cooking",
			width = 1,
			items = items})
		-- Compatibility layer
