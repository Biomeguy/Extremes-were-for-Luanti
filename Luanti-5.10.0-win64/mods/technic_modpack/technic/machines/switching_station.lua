...
core.register_node("technic:switching_station",{
	...
	tiles = {"technic_water_mill_top_active.png", "technic_water_mill_top_active.png"..cable_entry,
	...
	on_construct = function(pos)
		...
		meta:set_string("channel", "switching_station"..core.pos_to_string(pos))
...
local get_network = function(sw_pos, pos1, tier)
	local network_id = core.hash_node_position(pos1)
	...
	if cached and cached.tier == tier then
		...
		for _, pos in ipairs(cached.SP_nodes) do
			local meta = core.get_meta(pos)
			meta:set_int("active", 0)
			meta:set_string("active_pos", core.serialize(sw_pos))
		end
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if meta:get_int("active") ~= 1 then
			core.forceload_free_block(pos)
			core.forceload_free_block(pos1)
			...
			if not technic.redundant_warn[poshash] then
				...
				print("[TECHNIC] Warning: redundant switching station found near "..core.pos_to_string(pos))
			end
			return
		end
		...
		local function run_nodes(list, run_stage)
			for _, pos2 in ipairs(list) do
				...
				if node2 and node2.name then
					nodedef = core.registered_nodes[node2.name]
				end
...
core.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		local tier = technic.get_cable_tier(core.get_node(pos1).name)
