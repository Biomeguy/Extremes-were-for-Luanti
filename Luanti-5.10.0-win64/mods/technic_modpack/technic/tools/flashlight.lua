...
core.register_tool("technic:flashlight", {
...
core.register_craft({
	...
	recipe = {
		...
		{"default:stainless_steel_ingot", "technic:battery", "default:stainless_steel_ingot"},
		{"", "technic:battery", ""}
	}
})
...
local function check_for_flashlight(player)
	...
	for i = 1, 8 do
		if hotbar[i]:get_name() == "technic:flashlight" then
			local meta = core.deserialize(hotbar[i]:get_meta():get_string(""))
			if meta and meta.charge and meta.charge >= 2 then
				if not technic.creative_mode then
					...
					hotbar[i]:get_meta():set_string("", core.serialize(meta))
...
core.register_on_joinplayer(function(player)
	...
	player_positions[pname] = rounded_pos
	was_wielding[pname] = true
end)


core.register_on_leaveplayer(function(player)
	local pname = player:get_player_name()
	local pos = player_positions[pname]
	local nodename = core.get_node(pos).name
	if nodename == "technic:light" then
		core.remove_node(pos)
	end
	player_positions[pname] = nil
end)
...
core.register_globalstep(function(dtime)
	for i, player in pairs(core.get_connected_players()) do
		...
		local rounded_pos = vector.round(pos)
		rounded_pos.y = rounded_pos.y + 1
		local old_pos = player_positions[pname]
		...
		if was_wielding[pname] and not flashlight_weared then
			was_wielding[pname] = false
			local node = core.get_node_or_nil(old_pos)
			if node and node.name == "technic:light" then
				core.remove_node(old_pos)
			end
		elseif (player_moved or not was_wielding[pname]) and flashlight_weared then
