...
core.register_craft({
	...
	recipe = {
		...
		{'default:stainless_steel_ingot', 'technic:mining_drill', 'default:stainless_steel_ingot'},
...
core.register_craft({
	...
	recipe = {
		{'technic:diamond_drill_head', 'technic:diamond_drill_head', 'technic:diamond_drill_head'},
		{'default:stainless_steel_ingot', 'technic:mining_drill_mk2', 'default:stainless_steel_ingot'},
...
for i = 1, 4 do
	core.register_craft({
		...
		recipe = {
			{'technic:diamond_drill_head', 'technic:diamond_drill_head', 'technic:diamond_drill_head'},
			{'default:stainless_steel_ingot', 'technic:mining_drill_mk2_'..i, 'default:stainless_steel_ingot'},
			{'', 'technic:blue_energy_crystal', ''}
		}
	})
end
...
local function drill_dig_it0 (pos,player)
	...
	if node.name == "default:water_source" then core.remove_node(pos) return end
	if node.name == "default:water_flowing" then core.remove_node(pos) return end
...
local function drill_dig_it(pos, player, mode)
	...
	core.sound_play("mining_drill", {pos = pos, gain = 1., max_hear_distance = 10,})
end
...
local function mining_drill_mk2_setmode(user,itemstack)
	...
	if meta["mode"]==nil then
		core.chat_send_player(pname, S("Use while sneaking to change Mining Drill Mk%d modes."):format(2))
		meta["mode"]=0
		mode=0
	end
	mode=(meta["mode"])
	...
	core.chat_send_player(pname, S("Mining Drill Mk%d Mode %d"):format(2, mode)..": "..mining_drill_mode_text[mode][1])
...
local function mining_drill_mk3_setmode(user,itemstack)
	...
	if meta["mode"]==nil then
		core.chat_send_player(pname, S("Use while sneaking to change Mining Drill Mk%d modes."):format(3))
		meta["mode"]=0
		mode=0
	end
	mode=(meta["mode"])
	...
	core.chat_send_player(pname, S("Mining Drill Mk%d Mode %d"):format(3, mode)..": "..mining_drill_mode_text[mode][1])
...
local function mining_drill_mk2_handler(itemstack, user, pointd)
	...
	if pointd.type ~= "node" or not pos_is_pointable(pointd.under) or not meta.charge then
		return
	end
	local charge_to_take = cost_to_use(2, meta.mode)
	if meta.charge >= charge_to_take then
		local pos = core.get_pointed_thing_position(pointd, false)
...
local function mining_drill_mk3_handler(itemstack, user, pointd)
	local keys = user:get_player_control()
	local pname = user:get_player_name()
	local meta = core.deserialize(itemstack:get_meta():get_string(""))
	if not meta or not meta.mode or keys.sneak then
...
core.register_tool("technic:mining_drill", {
	...
	on_use = function(itemstack, user, pointd)
		if pointd.type ~= "node" or not pos_is_pointable(pointd.under) then
			return itemstack
		end
		local meta = core.deserialize(itemstack:get_meta():get_string(""))
...
core.register_tool("technic:mining_drill_mk2", {
	...
	groups = {not_repaired_by_anvil = 1},
	on_use = function(itemstack, user, pointd)
		mining_drill_mk2_handler(itemstack, user, pointd)
		return itemstack
	end,
})
...
for i = 1, 4 do
	...
	core.register_tool("technic:mining_drill_mk2_"..i, {
		description = S("Mining Drill Mk%d Mode %d"):format(2, i),
		inventory_image = "technic_mining_drill_mk2.png^technic_tool_mode"..i..".png",
		wield_image = "technic_mining_drill_mk2.png",
		wear_represents = "technic_RE_charge",
		on_refill = technic.refill_RE_charge,
		groups = {not_repaired_by_anvil = 1, not_in_creative_inventory=1},
		on_use = function(itemstack, user, pointd)
			mining_drill_mk2_handler(itemstack, user, pointd)
			return itemstack
		end,
...
core.register_tool("technic:mining_drill_mk3", {
	...
	groups = {not_repaired_by_anvil = 1},
	on_use = function(itemstack, user, pointd)
	mining_drill_mk3_handler(itemstack,user,pointd)
	return itemstack
	end,
})
...
for i=1,5,1 do
	...
	core.register_tool("technic:mining_drill_mk3_"..i, {
