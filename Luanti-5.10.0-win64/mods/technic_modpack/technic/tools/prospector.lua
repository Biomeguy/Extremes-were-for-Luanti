...
local function get_metadata(toolstack)
	local m = core.deserialize(toolstack:get_meta():get_string(""))
	if not m then m = {} end
...
core.register_tool("technic:prospector", {
	...
	on_use = function(toolstack, user, pointd)
		...
		if toolmeta.target == "" then
			core.chat_send_player(user:get_player_name(), "Right-click to set target block type")
			return
		end
		if not technic.creative_mode then
			toolmeta.charge = toolmeta.charge - charge_to_take
			toolstack:get_meta():set_string("", core.serialize(toolmeta))
			technic.set_RE_wear(toolstack, toolmeta.charge, technic.power_tools[toolstack:get_name()])
		end
		local start_pos = pointd.under
		...
		for f = 0, toolmeta.look_depth-1 do
			for r = 0, look_diameter-1 do
				for u = 0, look_diameter-1 do
					if core.get_node(vector.add(vector.add(vector.add(base_pos, vector.multiply(forward, f)), vector.multiply(right, r)), vector.multiply(up, u))).name == toolmeta.target then found = true end
				end
			end
		end
		if math.random() < 0.02 then found = not found end
		core.chat_send_player(user:get_player_name(), core.registered_nodes[toolmeta.target].description.." is "..(found and "present" or "absent").." in "..look_diameter.."x"..look_diameter.."x"..toolmeta.look_depth.." region")
		core.sound_play("technic_prospector_"..(found and "hit" or "miss"), { pos = vector.add(user:getpos(), { x = 0, y = 1, z = 0 }), gain = 1.0, max_hear_distance = 10 })
		return toolstack
	end,
	on_place = function(toolstack, user, pointd)
		if not user or not user:is_player() or user.is_fake_player then return end
		local toolmeta = get_metadata(toolstack)
		local pointed
		if pointd.type == "node" then
			local pname = core.get_node(pointd.under).name
		...
		core.show_formspec(user:get_player_name(), "technic:prospector_control",
			...
			"label[1,0;"..core.formspec_escape(toolstack:get_definition().description).."]"..
			(toolmeta.target ~= "" and
				"label[0,1.5;Current target:]"..
				"label[0,2;"..core.formspec_escape(core.registered_nodes[toolmeta.target].description).."]"..
...
core.register_craft({
	...
	recipe = {
		{"default:pick_silver", "default:mithril_block", "pipeworks:teleport_tube_1"},
		{"default:brass_ingot", "technic:control_logic_unit", "default:brass_ingot"},
