...
local function restore(pos, placer, itemstack)
	...
	data = (data ~= "" and data) or	itemstack:get_meta():get_string("")
	data = core.deserialize(data)
	if not data then
		core.remove_node(pos)
		core.log("error", placer:get_player_name().." wanted to place "..
				name.." at "..core.pos_to_string(pos)..
				", but it had no data.")
		core.log("verbose", "itemstack: "..itemstack:to_string())
		return true
	end
	core.set_node(pos, {name = data.name, param2 = node.param2})
...
for name, info in pairs(wrench.registered_nodes) do
	...
	if olddef then
		...
		core.register_node(":"..get_pickup_name(name), newdef)
	end
end

core.register_tool("wrench:wrench", {
	...
	on_place = function(itemstack, placer, pointd)
		...
		local meta = core.get_meta(pos)
		if def.owned and not core.check_player_privs(placer, "protection_bypass") then
			local owner = meta:get_string("owner")
			if owner and owner ~= pname then
				core.log("action", pname..
		...
		item_meta:set_string("data", core.serialize(metadata))
		core.remove_node(pos)
