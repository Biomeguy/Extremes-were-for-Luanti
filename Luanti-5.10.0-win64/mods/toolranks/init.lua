...
function toolranks.new_afteruse(itemstack, user, node, digparams)
	...
	local lastlevel = tonumber(itemmeta:get_string("lastlevel")) or 1 -- Level the tool had on the last dig
	...
	if(dugnodes > most_digs) then
		...
		if(most_digs_user ~= user:get_player_name()) then -- Avoid spam.
			...
			core.chat_send_all("Most used tool is now a " .. toolranks.colors.green .. itemdesc 
	...
	if(itemstack:get_wear() > 60135) then
		core.chat_send_player(user:get_player_name(), "Your tool is about to break!")
	...
	--core.chat_send_all("wear="..wear.."Original wear: "..digparams.wear.." 1+level/4="..1+level/4)
...
--emerald
	...
	core.override_item("es:axe_emerald", {
	...
	core.override_item("es:shovel_emerald", {
		...
		after_use = toolranks.new_afteruse})
--ruby
	core.override_item("es:pick_ruby", {
	...
	core.override_item("es:shovel_ruby", {
		...
		after_use = toolranks.new_afteruse})
--aikerum	
	core.override_item("es:pick_aikerum", {
	...
	core.override_item("es:axe_aikerum", {
	...
	core.override_item("es:shovel_aikerum", {
		...
		after_use = toolranks.new_afteruse})
--magentium	
	core.override_item("es:pick_magentium", {
		original_description = "Extreme Survival Magentium Pickaxe",
		description = toolranks.create_description("Extreme Survival Magentium Pickaxe", 0, 1),
		after_use = toolranks.new_afteruse})

	core.override_item("es:axe_magentium", {
		original_description = "Extreme Survival Magentium Axe",
		description = toolranks.create_description("Extreme Survival Magentium Axe", 0, 1),
		after_use = toolranks.new_afteruse})

	core.override_item("es:shovel_magentium", {
		original_description = "Extreme Survival Magentium Shovel",
		description = toolranks.create_description("Extreme Survival Magentium Shovel", 0, 1),
		after_use = toolranks.new_afteruse})
end
