...
--[[
 ...
 20.03.13 - added inventory image provided by VanessaE
          ...
          - in newer versions of Luanti, the players yaw is set so that he/she looks out of the receiving box
          - target list is now centered if there are less than 9 targets
--]]


core.register_privilege("travelnet_attach", { description = "allows to attach travelnet boxes to travelnets of other players", give_to_singleplayer = false})
core.register_privilege("travelnet_remove", { description = "allows to dig travelnet boxes which belong to nets of other players", give_to_singleplayer = false})
...
dofile(core.get_modpath("travelnet").."/config.lua") -- the normal, default travelnet
...
travelnet.save_data = function()
   ...
   local data = core.serialize( travelnet.targets )
   local path = core.get_worldpath().."/mod_travelnet.data"
...
travelnet.restore_data = function()
   ...
   if( file ) then
      ...
      travelnet.targets = core.deserialize( data )
      file:close()
   else
...
--      core.chat_send_player(puncher_name, "DEBUG DATA: owner: "..(owner_name or "?")..
--                                                  " station_name: "..(station_name or "?")..
--                                               " station_network: "..(station_network or "?")..".")
-- core.chat_send_player(puncher_name, "data: "..core.serialize(  travelnet.targets ))
      ...
      meta:set_string("formspec", 
                            "size[12,10]"..
                            "field[.3,7.6;9,.9;station_name;Name of this station:;"..(station_name or "?").."]"..
                            "field[.3,8.6;9,.9;station_network;Assign to Network:;"..(station_network or "?").."]"..
                            "field[.3,9.6;9,.9;owner;Owned by:;"..(owner_name or "?").."]"..
                            "button_exit[6.3,8.2;1.7,.7;station_set;Store]" )

      core.chat_send_player(puncher_name, "Error: Update failed! Resetting this box on the travelnet.")
      return
   end
   ...
   if( not( travelnet.targets[ owner_name ] )
     ...
     or not( travelnet.targets[ owner_name ][ station_network ][ station_name ] )) then
      ...
      core.chat_send_player(owner_name, "Station '"..station_name.."' has been reattached to the network '"..station_network.."'.")
   ...
   if( this_node and this_node.name=="locked_travelnet:travelnet" ) then
      zusatzstr = "field[.3,11;6,.7;locks_sent_lock_command;Locked travelnet. Type /help for help:;]"
      trheight = "11.5"
   end
   local formspec = "size[12,"..trheight.."]"..
                            "label[3.3,0;Travelnet-Box:]".."label[6.3,0;Punch box to update target list.]"..
                            "label[.3,.4;Name of this station:]".."label[6.3,.4;"..(station_name or "?").."]"..
                            "label[.3,.8;Assigned to Network:]" .."label[6.3,.8;"..(station_network or "?").."]"..
                            "label[.3,1.2;Owned by:]"            .."label[6.3,1.2;"..(owner_name or "?").."]"..
                            "label[3.3,1.6;Click on target to travel there:]"..
			    zusatzstr
--                            "button_exit[5.3,.3;8,.8;do_update;Punch box to update destination list. Click on target to travel there.]"..
   local x = 0
   local y = 0
   local i = 0
   ...
   -- core.chat_send_player(puncher_name, "stations: "..core.serialize( stations ))
   ...
   for index,k in ipairs( stations ) do 
      ...
      if( k ~= station_name or open_door_cmd) then 
         ...
         if( open_door_cmd ) then
            formspec = formspec .."button_exit["..(x)..","..(y+2.5)..";1,.5;open_door;<>]"..
                                  "label["..(x+.9)..","..(y+2.35)..";"..tostring( k ).."]"
         elseif( is_elevator ) then
            formspec = formspec .."button_exit["..(x)..","..(y+2.5)..";1,.5;target;"..tostring( travelnet.targets[ owner_name ][ station_network ][ k ].nr ).."]"..
                                  "label["..(x+.9)..","..(y+2.35)..";"..tostring( k ).."]"
         else
            formspec = formspec .."button_exit["..(x)..","..(y+2.5)..";4,.5;target;"..k.."]"
         end
...
--         formspec = formspec .. ']'
   ...
   core.chat_send_player(puncher_name, "The target list of this box on the travelnet has been updated.")
end
...
--      owner_name   = '*' -- the owner name is not relevant here
   ...
   if( station_name == "" or not(station_name )) then
      core.chat_send_player(player_name, "Please provide a name for this station.")
      return
   end

   if( network_name == "" or not( network_name )) then
      core.chat_send_player(player_name, "Please provide the name of the network this station ought to be connected to.")
      return
   end
   ...
   elseif( not( core.check_player_privs(player_name, {interact=true}))) then

      core.chat_send_player(player_name, "There is no player with interact privilege named '"..tostring( player_name ).."'. Aborting.")
      return

   elseif not core.check_player_privs(player_name, {travelnet_attach=true}) then

       core.chat_send_player(player_name, "You do not have the travelnet_attach priv which is required to attach your box to the network of someone else. Aborting.")
      return
   end
   ...
   for k,v in pairs( travelnet.targets[ owner_name ][ network_name ] ) do
      ...
      if( k == station_name ) then
         core.chat_send_player(player_name, "Error: A station named '"..station_name.."' already exists on this network. Please choose a diffrent name!")
         return
      end
   ...
   if( anz+1 > travelnet.MAX_STATIONS_PER_NETWORK ) then
      core.chat_send_player(player_name, "Error: Network '"..network_name.."' already contains the maximum number (="
   ...
   if( meta ) then
      ...
      core.chat_send_player(player_name, "Station '"..station_name.."' has been added to the network '"
      ...
      meta:set_string("formspec", 
                     "size[12,10]"..
                     "field[.3,.6;6,.7;station_name;Station:;"..   meta:get_string("station_name").."]"..
                     "field[.3,3.6;6,.7;station_network;Network:;"..meta:get_string("station_network").."]" )
...
travelnet.open_close_door = function( pos, player, mode )
   ...
   local door_node = core.get_node( pos2 )
   if( door_node ~= nil and door_node.name ~= 'ignore' and door_node.name ~= 'air' and core.registered_nodes[ door_node.name ] ~= nil and core.registered_nodes[ door_node.name ].on_rightclick ~= nil) then
      ...
      if( mode==2 ) then
         core.after( 1, core.registered_nodes[ door_node.name ].on_rightclick, pos2, door_node, player )
      else
         core.registered_nodes[ door_node.name ].on_rightclick(pos2, door_node, player)
      end
   end
end
...
travelnet.on_receive_fields = function(pos, formname, fields, player)
   ...
   if( not( fields.target )) then
      core.chat_send_player(name, "Please click on the target you want to travel to.")
      return
   end
   ...
   if(  not( owner_name  ) 
     ...
     or not( travelnet.targets[ owner_name ][ station_network ] )) then
      ...
      if(     owner_name
         ...
         core.chat_send_player(name, "Error: There is something wrong with the configuration of this station. "..
   ...
   if(  not( owner_name )
     ...
     or not( travelnet.targets[ owner_name ] )
     or not( travelnet.targets[ owner_name ][ station_network ] )) then
      core.chat_send_player(name, "Error: This travelnet is lacking data and/or improperly configured.")
      print( "ERROR: The travelnet at "..core.pos_to_string( pos ).." has a problem: "..
   ...
   if( not( travelnet.targets[ owner_name ][ station_network ][ fields.target ] )) then
      ...
      core.chat_send_player(name, "Station '"..( fields.target or "?").." does not exist (anymore?) on this network.")
      travelnet.update_formspec( pos, name )
      return
   end

   core.chat_send_player(name, "Initiating transfer to station '"..( fields.target or "?").."'.'")

   if( travelnet.travelnet_sound_enabled ) then
      core.sound_play("128590_7037-lq.mp3", {pos = pos, gain = 1.0, max_hear_distance = 10,})
   end
   if( travelnet.travelnet_effect_enabled ) then 
      core.add_entity( {x=pos.x,y=pos.y+.5,z=pos.z}, "travelnet:effect") -- it self-destructs after 20 turns
   end
   ...
   if( travelnet.travelnet_sound_enabled ) then
      core.sound_play("travelnet_travel.wav", {pos = target_pos, gain = 1.0, max_hear_distance = 10,})
   end
   if( travelnet.travelnet_effect_enabled ) then 
      core.add_entity( {x=target_pos.x,y=target_pos.y+.5,z=target_pos.z}, "travelnet:effect") -- it self-destructs after 20 turns
   end


   -- check if the box has at the other end has been removed.
   local node2 = core.get_node(  target_pos )
   ...
   elseif( player.set_look_horizontal ) then
      ...
      player:set_look_horizontal( math.rad( yaw )) -- this is only supported in recent versions of MT
      player:set_look_vertical( math.rad( 0 )) -- this is only supported in recent versions of MT
   end
...
travelnet.remove_box = function( pos, oldnode, oldmetadata, digger )
   ...
   if( not( oldmetadata ) or oldmetadata=="nil" or not(oldmetadata.fields)) then
      core.chat_send_player( digger:get_player_name(), "Error: Could not find information about the station that is to be removed.")
      return
   end
   ...
   if(  not( owner_name ) 
     ...
     or not( travelnet.targets[ owner_name ][ station_network ] )) then
      ...
      core.chat_send_player( digger:get_player_name(), "Error: Could not find the station that is to be removed.")
      return
   end
   ...
   -- inform the owner
   core.chat_send_player( owner_name, "Station '"..station_name.."' has been REMOVED from the network '"..station_network.."'.")
   if( digger ~= nil and owner_name ~= digger:get_player_name() ) then
      core.chat_send_player( digger:get_player_name(), "Station '"..station_name.."' has been REMOVED from the network '"..station_network.."'.")
   end
...
travelnet.can_dig = function( pos, player, description )
   ...
   if( not( meta ) or not( owner) or owner=='') then
      core.chat_send_player(name, "This "..description.." has not been configured yet. Please set it up first to claim it. Afterwards you can remove it because you are then the owner.")
      return false

   elseif( owner ~= name ) then
      core.chat_send_player(name, "This "..description.." belongs to "..tostring( meta:get_string('owner'))..". You can't remove it.")
      return false
   end
   return true
end
...
if travelnet.travelnet_effect_enabled then
core.register_entity("travelnet:effect", {
initial_properties = {
	hp_max = 1,
	physical = false,
	weight = 5,
	collisionbox = {-.4,-.5,-.4, .4,1.5,.4},
...
if travelnet.travelnet_enabled then
   dofile(core.get_modpath("travelnet").."/travelnet.lua") -- the travelnet node definition
end
if travelnet.elevator_enabled then
   dofile(core.get_modpath("travelnet").."/elevator.lua") -- allows up/down transfers only
end
if travelnet.doors_enabled then
   dofile(core.get_modpath("travelnet").."/doors.lua") -- doors that open and close automaticly when the travelnet or elevator is used
end

if travelnet.abm_enabled then
   dofile(core.get_modpath("travelnet").."/restore_network_via_abm.lua") -- restore travelnet data when players pass by broken networks
end
