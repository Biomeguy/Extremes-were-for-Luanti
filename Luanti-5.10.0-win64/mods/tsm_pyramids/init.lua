local S = core.get_translator("tsm_pyramids")
...
dofile(core.get_modpath("tsm_pyramids").."/mummy.lua")
dofile(core.get_modpath("tsm_pyramids").."/nodes.lua")
dofile(core.get_modpath("tsm_pyramids").."/room.lua")
...
local chest_stuff = {
	...
	sandstone = {
		...
		{name="spawn:anak", max = 1},
	},
}

if core.get_modpath("farming") then
	table.insert(chest_stuff.desert_sandstone, {name="farming:wheat_bread", max = 3})
	table.insert(chest_stuff.sandstone, {name="farming:wheat_bread", max = 4})
	...
	table.insert(chest_stuff.normal, {name="default:apple", max = 8})
	table.insert(chest_stuff.normal, {name="default:apple", max = 3})
end
...
function tsm_pyramids.fill_chest(pos, stype, flood_sand, treasure_chance)
	...
	if n and n.name and n.name == "default:chest" then
		...
		if math.random(1,100) <= treasure_chance then
			if core.get_modpath("treasurer") ~= nil then
...
local function add_spawner(pos, mummy_offset)
	core.set_node(pos, {name="tsm_pyramids:spawner_mummy"})
	if not core.settings:get_bool("only_peaceful_mobs") then
		for i=1,2 do
...
local function can_replace(pos)
	...
	if n and n.name and core.registered_nodes[n.name] and not core.registered_nodes[n.name].walkable then
		return true
	elseif not n then
		return true
	else
	...
	for ie=0,6,1 do
		...
		for iy=2,3,1 do
			...
			if flood_sand and sand ~= "ignore" and iy <= sand_height and ie >= 3 then
				core.set_node(way_dir, {name=sand})
			else
				core.remove_node(way_dir)
			end
			-- build decoration above entrance
			if ie == 3 and iy == 3 then
				local deco = {x=way_dir.x, y=way_dir.y+1,z=way_dir.z}
				core.set_node(deco, {name=brick})
				if rot == 0 or rot == 2 then
					core.set_node(vector.add(deco, {x=-1, y=0, z=0}), {name=brick})
					core.set_node(vector.add(deco, {x=1, y=0, z=0}), {name=brick})
				else
					core.set_node(vector.add(deco, {x=0, y=0, z=-1}), {name=brick})
					core.set_node(vector.add(deco, {x=0, y=0, z=1}), {name=brick})
				end
			end
		end
...
local function make_pyramid(pos, brick, sandstone, stone, sand)
	...
	core.bulk_set_node(set_to_stone , {name=stone})
	core.bulk_set_node(set_to_brick, {name=brick})
end
...
local function make(pos, brick, sandstone, stone, sand, ptype, room_id)
	...
	-- Done
	core.log("action", "[tsm_pyramids] Created pyramid at "..core.pos_to_string(bpos)..".")
	return ok, msg
end

local perl1 = {SEED1 = 9130, OCTA1 = 3,	PERS1 = 0.5, SCAL1 = 250} -- Values should match luanti mapgen V6 desert noise.
...
core.register_on_generated(function(minp, maxp, seed)
	...
	-- TODO: Use Luanti's pseudo-random tools
	math.randomseed(seed)

	if not perlin1 then
		perlin1 = core.get_perlin(perl1.SEED1, perl1.OCTA1, perl1.PERS1, perl1.SCAL1)
	end
	...
	if noise1 > 0.25 or noise1 < -0.26 then
		...
		if math.random(0,10) > 7 then
			core.log("verbose", "[tsm_pyramids] Pyramid not placed, bad dice roll. minp="..core.pos_to_string(minp))
			return
		end
		local sand, p2
		...
		if p2 == nil then
			core.log("verbose", "[tsm_pyramids] Pyramid not placed, no suitable surface. minp="..core.pos_to_string(minp))
			return
		end
		if p2.y < PYRA_MIN_Y then
			core.log("info", "[tsm_pyramids] Pyramid not placed, too deep. p2="..core.pos_to_string(p2))
			return
		end
		...
		for o=1, #oposses do
			...
			local n = core.get_node_or_nil(opos)
		...
		if p2.y < minp.y then
			core.log("info", "[tsm_pyramids] Pyramid not placed, sunken too much. p2="..core.pos_to_string(p2))
			return
		end

		-- Make sure the pyramid is not near a "killer" node, like water
		local middle = vector.add(p2, {x=PYRA_Wh, y=0, z=PYRA_Wh})
		if core.find_node_near(p2, 5, {"default:water_source"}) ~= nil or
				core.find_node_near(vector.add(p2, {x=PYRA_W, y=0, z=0}), 5, {"default:water_source"}) ~= nil or
				core.find_node_near(vector.add(p2, {x=0, y=0, z=PYRA_W}), 5, {"default:water_source"}) ~= nil or
				core.find_node_near(vector.add(p2, {x=PYRA_W, y=0, z=PYRA_W}), 5, {"default:water_source"}) ~= nil or

				core.find_node_near(middle, PYRA_W, {"default:dirt_with_grass"}) ~= nil or
				core.find_node_near(middle, 52, {"default:sandstone_brick", "default:desert_sandstone_brick", "default:desert_stone_brick"}) ~= nil or
				core.find_node_near(middle, PYRA_Wh + 3, {"default:cactus", "group:leaves", "group:trunk"}) ~= nil then
			core.log("info", "[tsm_pyramids] Pyramid not placed, inappropriate node nearby. p2="..core.pos_to_string(p2))
			return
		end
		...
		if sand == "default:desert_stone" then
			local nodes = core.find_nodes_in_area(vector.add(p2, {x=-1, y=-2, z=-1}), vector.add(p2, {x=PYRA_W+1, y=PYRA_Wh, z=PYRA_W+1}), {"group:sand"})
			if #nodes > 5 then
				sand = "default:desert_sand"
			end
		end
		...
		elseif sand == "default:sand" then
			-- Sandstone pyramid
			make(p2, "default:sandstone_brick", "default:sandstone", "default:sandstone", "default:sand", "sandstone")
		else
			-- Desert stone pyramid
			make(p2, "default:desert_stone_brick", "default:desert_stone_block", "default:desert_stone", "ignore", "desert_stone")
		end
	end
end)

-- Add backwards-compability for nodes from the original pyramids mod
if core.get_modpath("pyramids") == nil then
	-- Nodes
	core.register_alias("pyramids:trap", "tsm_pyramids:trap")
	core.register_alias("pyramids:trap_2", "tsm_pyramids:trap_2")
	core.register_alias("pyramids:deco_stone1", "tsm_pyramids:deco_stone1")
	core.register_alias("pyramids:deco_stone2", "tsm_pyramids:deco_stone2")
	core.register_alias("pyramids:deco_stone3", "tsm_pyramids:deco_stone3")
	core.register_alias("pyramids:spawner_mummy", "tsm_pyramids:spawner_mummy")
end

core.register_chatcommand("spawnpyramid", {
	description = S("Generate a pyramid"),
		...
		func = function(name, param)
			...
			if s == 1 then
				-- Sandstone
				ok, msg = make(pos, "default:sandstone_brick", "default:sandstone", "default:sandstone", "default:sand", "sandstone", room_id)
			elseif s == 2 then
				-- Desert sandstone
				...
				-- Desert stone
				ok, msg = make(pos, "default:desert_stone_brick", "default:desert_stone_block", "default:desert_stone", "ignore", "desert_stone", room_id)
			end
			if ok then
				return true, S("Pyramid generated at @1.", core.pos_to_string(pos))
			else
				return false, msg
			end
		end,
