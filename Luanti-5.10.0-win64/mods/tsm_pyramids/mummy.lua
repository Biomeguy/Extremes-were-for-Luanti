...
local mod_cmi = core.get_modpath("cmi") ~= nil
...
MUMMY_DEF.on_punch = function(self, puncher, time_from_last_punch, tool_capabilities, dir, damage)
	...
	if puncher ~= nil then
		core.sound_play(sound_hit, {pos = self.object:get_pos(), loop = false, max_hear_distance = 10, gain = 0.4}, true)
...
MUMMY_DEF.on_death = function(self, killer)
	core.sound_play(sound_dead, {pos = self.object:get_pos(), max_hear_distance = 10 , gain = 0.3}, true)
	-- Drop item on death
	local pos = self.object:get_pos()
	pos.y = pos.y + 1.0
	local count = math.random(0,3)
	if count > 0 then
		core.add_item(pos, mummy_drop .. " " .. count)
	end
	core.add_item(pos, "bonemeal:bone " .. math.random(7,25))
	if mod_cmi then
...
MUMMY_DEF.on_step = function(self, dtime)
	...
	-- Environment damage
	local def = core.registered_nodes[current_node.name]
	...
	-- Damage from node
	if dmg < 4 and (core.get_item_group(current_node.name, "water") ~= 0 or core.get_item_group(current_node.name, "lava") ~= 0) then
		dmg = 4
	end
	...
	if dmg > 0 then
		if self.envdmg_timer >= 1 then
			...
			if self.object:get_hp() <= 0 then
				...
				hit(self)
				self.sound_timer = 0
				core.sound_play(sound_hit, {pos = current_pos, max_hear_distance = 10, gain = 0.4}, true)
			end
		end
	 else
		self.time_passed = 0
	...
	if self.sound_timer > math.random(5,35) then
		core.sound_play(sound_normal, {pos = current_pos, max_hear_distance = 10, gain = 0.2}, true)
		self.sound_timer = 0
	end
	...
	if self.state == 1 then
		self.yawwer = true
		self.attacker = ""
		for  _,object in ipairs(core.get_objects_inside_radius(self.object:get_pos(), 4)) do
	...
	if self.state == 2 then
		...
		if self.attacker ~= "" and core.settings:get_bool("enable_damage") then
...
core.register_entity("tsm_pyramids:mummy", MUMMY_DEF)
core.register_entity("tsm_pyramids:mummy_spawner", spawner_DEF)


--spawn-egg/spawner

core.register_craftitem("tsm_pyramids:spawn_egg", {
	...
	on_place = function(itemstack, placer, pointd)
		...
		core.add_entity(pointd.above,"tsm_pyramids:mummy")
...
function tsm_pyramids.spawn_mummy_at(pos, number)
	...
	for _=1, number do
		core.add_entity(pos,"tsm_pyramids:mummy")
	end
end

core.register_node("tsm_pyramids:spawner_mummy", {
	...
	on_construct = function(pos)
		pos.y = pos.y - 0.3
		core.add_entity(pos,"tsm_pyramids:mummy_spawner")
	end,
...
function tsm_pyramids.attempt_mummy_spawn(pos, player_near_required)
	...
	local mobs = 0
	for  _,obj in ipairs(core.get_objects_inside_radius(pos, spawner_check_range)) do
	...
	if player_near or (not player_near_required) then
		if mobs < spawner_max_mobs then
			...
			local nposses = core.find_nodes_in_area(vector.subtract(pos, offset), vector.add(pos,offset), "air")
			...
			for i=1, tries do
				...
				for y=-1, -5, -1 do
					npos.y = npos.y - 1
					local below = core.get_node(npos)
					if core.registered_items[below.name].liquidtype ~= "none" then
						break
					end
				...
				local light = core.get_node_light(npos, .5)
				if not two_space then
					local above = core.get_node({x=npos.x, y=npos.y+1, z=npos.z})
