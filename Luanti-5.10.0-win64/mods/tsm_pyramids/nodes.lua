...
local desc = {
	...
	S("Desert Sandstone with Anak Engraving"), S("Desert Sandstone with Scarab Engraving"), S("Desert Sandstone with Cactus Engraving")
}
...
for i=1, #img do
	...
	core.register_node("tsm_pyramids:deco_stone"..i, {
		description = desc[i],
		_doc_items_longdesc = decodesc,
		is_ground_content = false,
		tiles = {sandstone_img, sandstone_img, sandstone_img.."^tsm_pyramids_"..img[i]..".png"},
		groups = core.registered_nodes[basenode].groups,
		sounds = core.registered_nodes[basenode].sounds,
	})
end
...
local trap_on_timer = function(pos, elapsed)
	...
	for i, obj in pairs(objs) do
		if obj:is_player() then
			if core.registered_nodes[n.name]._tsm_pyramids_crack and core.registered_nodes[n.name]._tsm_pyramids_crack < 2 then
				-- 70% chance to ignore player to make the time of falling less predictable
				if math.random(1, 10) >= 3 then
					return true
				end
				if n.name == "tsm_pyramids:trap" then
					core.set_node(pos, {name="tsm_pyramids:trap_2"})
					core.check_for_falling(pos)
				elseif n.name == "tsm_pyramids:desert_trap" then
					core.set_node(pos, {name="tsm_pyramids:desert_trap_2"})
					core.check_for_falling(pos)
				end
				return true
			end
...
local register_trap_stone = function(basename, desc_normal, desc_falling, base_tile, drop)
	core.register_node("tsm_pyramids:"..basename, {
	...
	core.register_node("tsm_pyramids:"..basename.."_2", {
