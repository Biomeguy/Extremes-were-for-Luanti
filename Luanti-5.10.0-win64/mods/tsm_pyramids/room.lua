...
code_desert_stone["S"] = "default:desert_stone_brick"
...
function tsm_pyramids.make_room(pos, stype, room_id, rotations)
	...
	if room.wall then
		for iy=0,4,1 do
			for ie=0,8,1 do
				...
				core.set_node({x=hole.x+ie, y=hole.y-iy, z=hole.z-1}, {name=nn})
				core.set_node({x=hole.x-1, y=hole.y-iy, z=hole.z+ie}, {name=nn})

				core.set_node({x=hole.x+ie, y=hole.y-iy, z=hole.z+9}, {name=nn})
				core.set_node({x=hole.x+9, y=hole.y-iy, z=hole.z+ie}, {name=nn})
			end
		end
	end
	local layout
		...
		for iy=0,4,1 do
			for ix=0,8,1 do
				for iz=0,8,1 do
					...
					core.set_node(cpos, {name=nn, param2=p2})
	...
	elseif room.style == "stacked" then
		...
		core.log("error", "Invalid pyramid room style! room type ID="..r)
	end
		...
		for ix=wmin,wmax,1 do
			for iz=wmin,wmax,1 do
				...
				if ix == wmin or ix == wmax or iz == wmin or iz == wmax then
					...
					core.set_node({x=hole.x+ix,y=hole.y-iy,z=hole.z+iz}, {name=n_str})
				else
					n_str = layout_traps[ix*9+iz+1]
					core.set_node({x=hole.x+ix,y=hole.y-iy,z=hole.z+iz}, {name=replace2(n_str, iy, depth, code_table, trap_node)})
				end
			end
		end
	end
...
function tsm_pyramids.flood_sand(pos, stype)
	...
	core.bulk_set_node(set_to_sand, {name=nn})
end
