...
core.after(0.01, function()
	local rev_aliases = {}
	for source, target in pairs(core.registered_aliases) do
	...
	for name, def in pairs(core.registered_items) do
		if (not def.groups.not_in_creative_inventory or
		   ...
		   def.description and def.description ~= "" then
			...
			for _, pname in ipairs(all_names) do
				local recipes = core.get_all_craft_recipes(pname)
				if recipes then
					for _, recipe in ipairs(recipes) do
						...
						for _,chk in pairs(recipe.items) do
							...
							if (not groupchk and not core.registered_items[chk])
							  or (groupchk and not ui.get_group_item(string.gsub(chk, "group:", "")).item)
							  or core.get_item_group(chk, "not_in_craft_guide") ~= 0 then
								unknowns = true
							end
						end
...
function ui.set_home(player, pos)
	...
	ui.home_pos[pname] = vector.round(pos)
