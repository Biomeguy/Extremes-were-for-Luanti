...
local function formspec_add_categories(player, formspec, ui_peruser)
	...
	if category_count > ui_peruser.pagecols then
		scroll_offset = ui.current_category_scroll[pname]
	end
	...
	for index, category in ipairs(ui.category_list) do
		...
		if column > 0 and column <= ui_peruser.pagecols then
			local scale = 0.8
			if ui.current_category[pname] == category.name then
				scale = 1
			end
...
local function formspec_add_item_browser(player, formspec, ui_peruser)
	...
	-- Items list
	if #ui.filtered_items_list[pname] == 0 then
	...
	local dir = ui.active_search_direction[pname]
	local list_index = ui.current_index[pname]
	local page2 = math.floor(list_index / (ui_peruser.items_per_page) + 1)
	local pagemax = math.floor(
		(#ui.filtered_items_list[pname] - 1)
			/ (ui_peruser.items_per_page) + 1)
	for y = 0, ui_peruser.pagerows - 1 do
		for x = 0, ui_peruser.pagecols - 1 do
			local name = ui.filtered_items_list[pname][list_index]
			local item = core.registered_items[name]
			if item then
				-- Clicked on current item: Flip crafting direction
				if name == ui.current_item[pname] then
					local cdir = ui.current_craft_direction[pname]
					if cdir == "recipe" then
						dir = "usage"
				...
				-- Default: use active search direction by default
					dir = ui.active_search_direction[pname]
				end
				...
				formspec[n + 1] = ("tooltip[%s;%s \\[%s\\]]"):format(
					button_name, core.formspec_escape(item.description),
					item.mod_origin or "??"
				)
				n = n + 2
...
function ui.get_formspec(player, page)
	...
	local pname = player:get_player_name()
	local ui_peruser = ui.get_per_player_formspec(pname)

	ui.current_page[pname] = page
	...
	local perplayer_formspec = ui.get_per_player_formspec(pname)
...
function ui.apply_filter(player, filter, search_dir)
	...
	if lfilter:sub(1, 6) == "group:" then
		...
		local lang = core.get_player_information(pname).lang_code
		ffilter = function(name, def)
	...
	ui.filtered_items_list[pname]={}
	local category = ui.current_category[pname] or 'all'
	if category == 'all' then
		for name, def in pairs(core.registered_items) do
			if valid_def(def)
			and ffilter(name, def) then
				table.insert(ui.filtered_items_list[pname], name)
			end
		end
	...
	table.sort(ui.filtered_items_list[pname])
	ui.filtered_items_list_size[pname] = #ui.filtered_items_list[pname]
	ui.current_index[pname] = 1
	ui.activefilter[pname] = filter
	ui.active_search_direction[pname] = search_dir
