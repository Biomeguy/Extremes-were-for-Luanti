...
local trash = core.create_detached_inventory("trash", {
	...
	on_put = function(inv, listname, index, stack, player)
		...
		core.sound_play("trash", {to_player=pname, gain = 1.0})
	end,
})
...
ui.register_button("home_gui_set", {
	...
	action = function(player)
		local pname = player:get_player_name()
		if core.check_player_privs(pname, {home=true}) then
			ui.set_home(player, player:get_pos())
			local home = ui.home_pos[pname]
			if home ~= nil then
			...
			ui.set_inventory_formspec(player, ui.current_page[pname])
		end
	end,
	...
	action = function(player)
		...
		if core.check_player_privs(pname, {home=true}) then
			if ui.go_home(player) then
				core.sound_play("teleport", {to_player = pname})
			end
		else
			core.chat_send_player(pname,
				S("You don't have the \"home\" privilege!"))
...
ui.register_button("misc_set_day", {
	...
	action = function(player)
		local pname = player:get_player_name()
		if core.check_player_privs(pname, {settime=true}) then
			core.sound_play("birds",
					{to_player=pname, gain = 1.0})
...
ui.register_button("clear_inv", {
	...
	action = function(player)
		local pname = player:get_player_name()
		if not ui.is_creative(pname) then
			core.chat_send_player(pname,
		...
		core.chat_send_player(pname, S('Inventory cleared!'))
...
ui.register_page("craft", {
	get_formspec = function(player, perplayer_formspec)
		...
		if ui.trash_enabled or ui.is_creative(pname) or core.get_player_privs(pname).give then
		...
		if ui.is_creative(pname) then
			...
			formspec[n+2] = string.format("list[detached:%srefill;main;%f,%f;1,1;]",
				F(pname), craftx - 2.5 + ui.list_img_offset, crafty + 2.5 + ui.list_img_offset)
		end
...
ui.register_page("craftguide", {
	get_formspec = function(player, perplayer_formspec)
		...
		local pname = player:get_player_name()
		local player_privs = core.get_player_privs(pname)
		...
		local item_name = ui.current_item[pname]
		...
		if core.registered_items[item_name]
				and core.registered_items[item_name].description then
			item_name_shown = S("@1 (@2)",
				core.registered_items[item_name].description, item_name)
		else
			item_name_shown = item_name
		end

		local dir = ui.current_craft_direction[pname]
		local rdir = dir == "recipe" and "usage" or "recipe"

		local crafts = ui.crafts_for[dir][item_name]
		local alternate = ui.alternate[pname]
		local alternates, craft
		if crafts and #crafts > 0 then
			alternates = #crafts
			craft = crafts[alternate]
		end
		local has_give = player_privs.give or ui.is_creative(pname)
		...
		local giveme_form = table.concat({
			...
			"button["..(give_x+1.6)..",".. (craftguidey + 2.9) .. ";0.75,0.5;craftguide_giveme_144;144]"
		})
		...
		for x = 1, display_size.width do
			...
			if item then
				...
				formspec[n] = string.format("image_button[%f,%f;%f,%f;blank.png;;]",
						xoffset - xof, craftguidey - 1.25 + yof, bsize, bsize)
			end
			n = n + 1
		end
...
local function craftguide_giveme(player, formname, fields)
	...
	local output = ui.current_item[pname]
...
local function craftguide_craft(player, formname, fields)
	...
	if amount == 0 or amount < -1 or amount > 144 then return end

	local pname = player:get_player_name()

	local output = ui.current_item[pname] or ""
	if output == "" then return end

	local crafts = ui.crafts_for[
		ui.current_craft_direction[pname]][output] or {}
