...
function vehicles.object_drive(entity, dtime, def)
	...
	local node_under = core.get_node({x=pos.x, y=pos.y+2, z=pos.z})
	local accell = 1
	...
	if shoots then
		...
		inv = core.get_inventory({type="player", name=pname});
	end
	...
	if boost and not entity.boost then
		core.after(boost_charge, function()
		entity.boost = true
		end)
	end
	
	--core.chat_send_all("decell:"..decell.." speed"..absolute_speed)
	...
	--place node
	if place_node ~= nil and node == "air" or place_node ~= nil and node == "default:snow" or place_node ~= nil and core.get_item_group(node, "flora") ~= 0 then
		if place_trigger == nil and math.random(1, place_chance) == 1 then
			core.set_node(pos, {name=place_node})
		end
		if place_trigger ~= nil and ctrl.sneak then
			local facedir = core.dir_to_facedir(dir)
			core.set_node(pos, {name=place_node, param2=facedir})
		end
	end
	...
	if destroy_node ~= nil and node == destroy_node then
		core.dig_node(pos)
--[[		local item = core.get_node_drops(destroy_node)-- Fortune effect
		if item[1] ~= nil then
		core.add_item(pos, item[1])
		end
		if item[2] ~= nil then
			core.add_item(pos, item[1])
		end]]
	...
	if ctrl.jump and brakes then
		...
		core.add_particlespawner({
			...
			time = .5,
			pos = effect_pos,
			maxvel = {x=-velo3.x, y=.4, z=-velo3.z},
			exptime = {min = .5, max = 1},
			minsize = 10,
			maxsize = 15,
	...
	if entity_yaw ~= target_yaw and not uses_arrow_keys then
		...
		--core.chat_send_all("yaw:"..entity_yaw)
		--core.chat_send_all("dirx: "..dir.x.." dirz:"..dir.z)
		if ctrl.left then
	...
	local function is_water(node)
		return node == "default:fresh_water_source" or node == "default:water_source" or node == "default:fresh_water_flowing" or node == "default:water_flowing"
	end
	entity.on_water = is_water(node)
	entity.in_water = is_water(core.get_node({x=pos.x, y=pos.y+1, z=pos.z}).name) or is_water(node_under.name)
	...
	elseif ctrl.jump and brakes and not ctrl.up then
		...
		core.add_particlespawner({
			...
			pos = effect_pos,
			minvel = {x=0, y=.1, z=0},
			maxvel = {x=-velo2.x, y=.4, z=-velo2.z},
			exptime = {min = .5, max = 1},
			minsize = 10,
			maxsize = 15,
			texture = braking_effect})
	...
	elseif ctrl.up and not shoots2 and ctrl.aux1 and entity.boost then
		entity.object:set_velocity({x=dir.x*(speed*0.2)*math.log(vtimer+0.5)+8*dir.x,y=velo.y-gravity/2,z=dir.z*(speed*0.2)*math.log(vtimer+0.5)+8*dir.z})
		if boost_effect ~= nil then
		local effect_pos = {x=pos.x-dir.x*2, y=pos.y, z=pos.z-dir.z*2}
			core.add_particlespawner({
			amount = 10,
			time = .25,
			pos = {x=effect_pos.x, y=effect_pos.y+.2, z=effect_pos.z},
			minvel = {x=-velo.x, y=-velo.y, z=-velo.z},
			...
			core.after(boost_duration, function()
			entity.boost = false
			end)
	--animation
	...
	elseif ctrl.up and not fixed then
		...
		entity.object:set_velocity({x=(dir.x*(speed*0.2)*math.log(vtimer+0.5)+4*dir.x)/(braking*(0.1)+1),y=velo.y-0.5,z=(dir.z*(speed*0.2)*math.log(vtimer+0.5)+4*dir.z)/(braking*(0.1)+1)})
		elseif not fly then
		entity.object:set_velocity({x=dir.x*(speed*0.2)*math.log(vtimer+0.5)+4*dir.x,y=0,z=dir.z*(speed*0.2)*math.log(vtimer+0.5)+4*dir.z})
		else
		entity.object:set_velocity({x=dir.x*(speed*0.2)*math.log(vtimer+0.5)+4*dir.x,y=dir.y*(speed*0.2)*math.log(vtimer+0.5)+4*dir.y+1,z=dir.z*(speed*0.2)*math.log(vtimer+0.5)+4*dir.z})
		end
	--animation
	if moving_anim ~= nil and not entity.moving and not hovering then
		entity.object:set_animation(moving_anim, anim_speed, 0)
	...
	if ctrl.sneak and shoots and entity.loaded then
			...
			entity.loaded = false
			local obj = core.add_entity({x=pos.x+0+dir.x*2,y=pos.y+shoot_y+dir.y,z=pos.z+0+dir.z*2}, arrow)
			local vec = {x=dir.x*14,y=dir.y*14+shoot_angle,z=dir.z*14}
			obj:set_yaw(yaw+extra_yaw)
			...
			core.after(reload_time, function()
			entity.loaded = true
	...
	if ctrl.aux1 and shoots2 and entity.loaded2 then
			...
			local obj = core.add_entity({x=pos.x+0+dir.x*2,y=pos.y+shoot_y2+dir.y,z=pos.z+0+dir.z*2}, arrow2)
			...
			core.after(reload_time2, function()
			entity.loaded2 = true
	...
	if jump == "hover" and ctrl.jump and not entity.jumpcharge then
		...
		entity.object:set_velocity({x=dir.x*(speed*0.2)*math.log(vtimer+0.5)+4*dir.x,y=hover_speed,z=dir.z*(speed*0.2)*math.log(vtimer+0.5)+4*dir.z})
		end
		hovering = true
	...
	if jump == "jump" and ctrl.jump and not entity.jumpcharge then
		...
		entity.object:set_velocity({x=dir.x*speed/4*math.atan(0.5*vtimer-2)+8*dir.x,y=jump_speed,z=dir.z*speed/4*math.atan(0.5*vtimer-2)+8*dir.z})
		end
		hovering = true
		if jump_anim ~= nil and entity.object:get_animation().range ~= jump_anim and hovering then
			entity.object:set_animation(jump_anim, anim_speed, 0)
...
core.register_craftitem(vehicle.."_spawner", {
	description = desc,
	...
	on_place = function(item, placer, pointd)
		...
		if pointd.type == "node" and not is_boat then
		local obj = core.add_entity(pointd.above, vehicle)
		local object = obj:get_luaentity()
		object.owner = placer
		...
		elseif pointd.type == "node" and core.get_item_group(pointd.name, "water") then
		local obj = core.add_entity(pointd.under, vehicle)
...
function vehicles.explodinate(ent, radius)
	...
	core.add_particlespawner({
		amount = 90,
		time = 4,
		minpos = {x=pos.x-.6, y=pos.y, z=pos.z-.6},
		maxpos = {x=pos.x+.6, y=pos.y+1, z=pos.z+.6},
		minvel = {x=-.1, y=3.5, z=-.1},
		maxvel = {x=.1, y=4.5, z=.1},
		minacc = {x=-1.3, y=-.7, z=-1.3},
		maxacc = {x=1.3, y=-.7, z=1.3},
		exptime = {min = 2, max = 3},
		minsize = 15,
		maxsize = 25,
...
function vehicles.on_punch(self, puncher)
	...
	return end
	local creative_mode = core.is_creative_enabled(self.driver:get_player_name())
	if self.driver == puncher and (hp == self.initial_properties.hp_max-5 or hp == self.initial_properties.hp_max or creative_mode) then
		local name = self.object:get_luaentity().name
		local pos = self.object:get_pos()
		core.add_item(pos, name.."_spawner")
		vehicles.object_detach(self, self.driver, {x=1, y=0, z=1})
		self.object:remove()
	end
end
...
function vehicles.object_no_drive(entity, dtime, def)
	...
	entity.in_water = is_water(core.get_node({x=pos.x, y=pos.y+1, z=pos.z}).name) or is_water(core.get_node({x=pos.x, y=pos.y+2, z=pos.z}).name)
