...
for i in ipairs(potion_list) do
	...
	core.register_node("warp_potions:potion_"..number, {
		...
		on_rightclick = function(pos, node, player, itemstack, pointd)
			...
			player:get_meta():set_string("warp_point_"..number, core.pos_to_string(pos))
...
local m_ip = core.get_modpath("inventory_plus")
local m_ui = core.get_modpath("unified_inventory")
local m_sfinv = core.get_modpath("sfinv")


-- warp function

local function warp(player, warp_point)
	if core.string_to_pos(player:get_meta():get_string("warp_point_"..warp_point)) == nil then
		core.chat_send_player(player:get_player_name(),"Invalid or un-set warp point.")
		return
	end

	local inv = core.get_inventory({type='detached', name=player:get_player_name().."_potion_inv"})
	if not inv:contains_item("potions", ItemStack("warp_potions:potion_"..warp_point.." 1")) then
		core.chat_send_player(player:get_player_name(),"You don't have the right potion!")
		return
	end
	...
	local pos = core.string_to_pos(player:get_meta():get_string("warp_point_"..warp_point))
	player:set_pos(pos)

	-- remove warp point if another player protected destination
	core.after(2, function(pos, player, warp_point) 
		if core.is_protected(pos, player:get_player_name()) then
			player:get_meta():set_string("warp_point_"..warp_point, "")
			core.chat_send_player(player:get_player_name(), "Area is protected: Warp point lost!")
		end
...
local function potion_inv_sort(player)
	...
	local w_inv = core.get_inventory({type="detached", name=name.."_potion_inv"})
	-- sort according to slot, returning any leftover. Stack max is 144
	for i=1,8 do
...
local function get_formspec(name, part)
	...
	b =  "list[detached:"..name.."_potion_inv;potions;1.6,1.15;1,1;0]" ..
		"list[detached:"..name.."_potion_inv;potions;4.2,1.15;1,1;2]" ..
		"list[detached:"..name.."_potion_inv;potions;6.8,1.15;1,1;4]" ..
		"list[detached:"..name.."_potion_inv;potions;9.4,1.15;1,1;6]" ..

		"list[detached:"..name.."_potion_inv;potions;1.6,3.1;1,1;1]" ..
		"list[detached:"..name.."_potion_inv;potions;4.2,3.1;1,1;3]" ..
		"list[detached:"..name.."_potion_inv;potions;6.8,3.1;1,1;5]" ..
		"list[detached:"..name.."_potion_inv;potions;9.4,3.1;1,1;7]" ..

		"button[.5,1.2;1,1;1;Use]" ..
		"button[3.1,1.2;1,1;3;Use]" ..
		"button[5.69,1.2;1,1;5;Use]" ..
		"button[8.27,1.2;1,1;7;Use]" ..

		"button[.5,3.16;1,1;2;Use]" ..
		"button[3.1,3.16;1,1;4;Use]" ..
		"button[5.69,3.16;1,1;6;Use]" ..
		"button[8.27,3.16;1,1;8;Use]"
...
core.register_on_joinplayer(function(player)
	...
	local name = player:get_player_name()
	-- create inventory
	local potion_inv = core.create_detached_inventory(name.."_potion_inv",{
		...
		on_put = function(inv, listname, index, stack, player)
			core.after(0, potion_inv_sort, player)
		end
...
if m_ui then
	...
	unified_inventory.register_page("potions", {
		get_formspec = function(player, perplayer_formspec)
			...
			local formspec = "background[.26,"..fy..";10.2,9.85;potion_inv_ui_form.png]"
			.."label[.26,.3;Warp Potions]"
