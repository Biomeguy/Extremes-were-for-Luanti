...
local item_def = core.registered_items[toolname]
if def and not def.preserve_metadata and not def.groups.seaweedy and item_def and
item_def.tool_capabilities and item_def.tool_capabilities.give_dig then
	return {nodename}
end
	...
	local got_items = {}
	local got_count = 0
local tluck = item_def and item_def.tool_capabilities and item_def.tool_capabilities.luck_drops or false
	for _, item in ipairs(drop.items) do
		local good_rarity = true
		local good_tool = true
		if item.rarity ~= nil then
if tluck then
	good_rarity = item.rarity/3 < 1 or math.random(item.rarity/3) == 1	else
			good_rarity = item.rarity < 1 or math.random(item.rarity) == 1	end
		end
...
--if def and not def.groups.resinous and nodename ~= "default:coir_block_wet" and item_def and
--item_def.tool_capabilities and item_def.tool_capabilities.cook_dug then end
...
function core.has_cook_dug(pos, drops, digger, item_def)

	local item_cookdug = item_def and item_def.tool_capabilities and
		item_def.tool_capabilities.cook_dug or false
	if not digger or item_cookdug ~= true then
		return drops
	end

	local nn = core.get_node(pos).name

	if core.get_item_group(nn, "resinous") > 0 or nn == "default:coir_block_wet" then
		return drops
	end
...
function core.do_item_eat(hp_change, replace_with_item, itemstack, user, pointed_thing)
	...
	if def and def.sound and def.sound.eat and core.settings:get_bool("enable_eating_sound") ~= false then
...
function core.node_dig(pos, node, digger)
	...
	drops = core.has_cook_dug(pos, drops, digger, wdef)
...
local default_stack_max = tonumber(core.settings:get("default_stack_max")) or 144
