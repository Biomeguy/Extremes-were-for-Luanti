...
function beds.register_bed(name, def)
	local is_primi
if name == "beds:primitive_bed" then
	is_primi = true end
	...
	drop = is_primi and "stairs:slab_wood 2",
		...
		on_place = function(itemstack, placer, pointd)
			...
			if core.is_protected(pos, pname) and
					not core.check_player_privs(pname, "protection_bypass") then
			...
			if core.is_protected(botpos, pname) and
					not core.check_player_privs(pname, "protection_bypass") then
				core.record_protection_violation(botpos, pname)
				return itemstack
			end
	...
	if is_primi then
		core.get_node_timer(pos):start(math.random(17, 24)) end
	...
	on_timer = is_primi and function(pos, elapsed)
		local meta = core.get_meta(pos)
	if meta:get_int("age_wear") < 300 then
		meta:set_int("age_wear", meta:get_int("age_wear")+1)
		core.get_node_timer(pos):start(math.random(17, 24))
	else
		if not core.dig_node(pos) then
		core.get_node_timer(pos):start(math.random(17, 24)) end
	end
		end,
	})
...
function beds.register_decor_bed(name, def)
