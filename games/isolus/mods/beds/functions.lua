...
local function lay_down(player, pos, bed_pos, state, skip)
	...
	-- lay down
		...
		for _, other_pos in pairs(beds.bed_position) do
			if vector.distance(bed_pos, other_pos) < .1 then
		...
		if vector.length(player:get_velocity()) > .05 then
...
if core.get_node(pos).name == "beds:primitive_bed_bottom" then
	local meta = core.get_meta(pos)
	meta:set_int("age_wear", meta:get_int("age_wear")+2)
end
	else
		lay_down(player, nil, nil, false)
	end
