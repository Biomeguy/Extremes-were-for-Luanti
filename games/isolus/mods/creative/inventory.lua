...
function creative.init_creative_inventory(player)
	...
	core.create_detached_inventory("creative_" .. pname, {
		...
		on_take = function(inv, listname, index, stack, player2)
			if stack and stack:get_count() > 0 then
				core.log("action", pname .. " takes " .. stack:get_name().. " from creative inventory")
			end
		end,
	}, pname)
...
function creative.update_creative_inventory(pname, tab_content)
	local inv = player_inventory[pname] or
			creative.init_creative_inventory(core.get_player_by_name(pname))
	local player_inv = core.get_inventory({type = "detached", name = "creative_" .. pname})
	...
	local lang
	local player_info = core.get_player_information(pname)
...
function creative.register_tab(name, title, items)
	sfinv.register_page("creative:" .. name, {
		...
		get = function(self, player, context)
			local pname = player:get_player_name()
			creative.update_creative_inventory(pname, items)
			...
			return sfinv.make_formspec(player, context,
				...
				"listring[detached:creative_" .. pname .. ";main]" ..
				"list[detached:creative_" .. pname .. ";main;0,0;8,4;" .. tostring(inv.start_i) .. "]" ..
