...
function default.chest.chest_lid_close(pn)
	...
	for k, v in pairs(default.chest.open_chests) do
		if vector.equals(v.pos, pos) then-- another player is also looking at the chest
			return true
		end
	end
	...
	core.after(.2, function()
		...
		if current_node.name ~= swap .."_open" then-- already replaced?
			return
		end
		core.swap_node(pos, {name = swap, param2 = node.param2})
		core.sound_play(sound, {gain = .3, pos = pos, max_hear_distance = 10}, true)
	end)
end
...
core.register_craft({
	...
	burntime = 30,
	replacements = {{"default:chest_locked", "default:padlock"}}
})

core.register_node("default:basket", {
	description = S("Basket"),
	tiles = {"earthbuild_basket_top.png", "earthbuild_basket_top.png", "earthbuild_basket.png"},
	paramtype = "light",
	paramtype2 = "facedir",
	...
	on_construct = function(pos)
		local meta = core.get_meta(pos)
		meta:set_string("infotext", S("Basket"))
		local inv = meta:get_inventory()
		inv:set_size("main", 8*4)
	end,
	...
	on_rightclick = function(pos, node, clicker)
		...
		core.sound_play("default_grass_footstep", {gain = .06, pos = pos,
				max_hear_distance = 3}, true)
		core.after(.2, core.show_formspec, cn,
			"default:basket", default.chest.get_chest_formspec(pos))
	end,
...
local p = vector.offset(pos,
	...
	on_blast = function(pos)
		local drops = {}
		default.get_inventory_drops(pos, "main", drops)
		drops[#drops+1] = "default:basket"
		core.remove_node(pos)
		return drops
	end,
	groups = {snappy = 1, oddly_breakable_by_hand = 3, flammable = 3},
...
core.register_craft({
	output = "default:basket",
	recipe = {
		{"default:woven_mat", "default:woven_mat", "default:woven_mat"},
		{"default:woven_mat", "", "default:woven_mat"},
		{"default:woven_mat", "default:woven_mat", "default:woven_mat"}
	}
})
