...
local smoke_particles = {
	...
	minacc = vector.new(-.15, -.02, -.15),
	maxacc = vector.new(.15, -.01, .15),
	exptime = {min = 4, max = 6},
	minsize = 5,
	maxsize = 5,
...
core.register_entity(":__builtin:item", {
	...
	set_item = function(self, item)
		...
		self.object:set_properties({
			...
			collisionbox = c,
			automatic_rotate = .314 / size,
			wield_item = self.itemstring,
			glow = glow,
			infotext = stack:get_description(),
		})

		if def and def.groups.flammable ~= 0 then
			self.flammable = def.groups.flammable
		end
	...
	try_merge_with = function(self, own_stack, object, entity)
		...
		or own_stack:get_free_space() == 0 then
			return false -- Cannot merge different or full stack
		end
		...
		pos.y = pos.y + ((total_count - count) / max_count) * .15
	...
	burn_up = function(self)--	disappear in a smoke puff
		local p = self.object:get_pos()
		self.object:remove()
		core.sound_play("default_item_smoke", {
			pos = p, gain = 1., max_hear_distance = 8}, true)
	...
	on_step = function(self, dtime, moveresult)
		...
		if self.timer > .25 or not self.node_inside then

		self.node_inside = core.get_node_or_nil(pos)
		self.def_inside = self.node_inside and core.registered_nodes[self.node_inside.name]

		self.node_under = core.get_node_or_nil({
			x = pos.x,
			y = pos.y + self._collisionbox[2] - .05,
			z = pos.z
		})
		self.def_under = self.node_under and core.registered_nodes[self.node_under.name]
	...
	-- instantly destroy any item dropped in lava (if enabled)
	local def = node and core.registered_nodes[node.name]
	if core.settings:get_bool("lava_destroys_item") and def and def.groups and def.groups.lava then
		self:burn_up()
		return
	end
		...
		if self.force_out then
			...
			local ok = (f.x > 0 and pos.x + c[1] > s.x + .5) or
				(f.y > 0 and pos.y + c[2] > s.y + .5) or
				(f.z > 0 and pos.z + c[3] > s.z + .5) or
				(f.x < 0 and pos.x + c[4] < s.x - .5) or
				(f.z < 0 and pos.z + c[6] < s.z - .5)
			if ok then
		...
		if def then
			...
			if slippery ~= 0 and (math.abs(vel.x) > .1 or math.abs(vel.z) > .1) then
