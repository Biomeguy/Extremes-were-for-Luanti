...
local function loadhomes()
	...
	for pos, name in input:read("*a"):gmatch("(%S+ %S+ %S+)%s([%w_-]+)[\r\n]") do
		homepos[name] = core.string_to_pos(pos)
	end
	input:close()
end
...
sethome.set = function(name, pos)
	...
	player_meta:set_string("sethome:home", core.pos_to_string(pos))
...
sethome.get = function(name)
	...
	local player_meta = player:get_meta()
	local pos = core.string_to_pos(player_meta:get_string("sethome:home"))
	if pos then
		return pos
	end
