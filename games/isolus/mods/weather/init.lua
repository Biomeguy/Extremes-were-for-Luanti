...
if core.settings:get_bool("enable_weather") == false then
	return
end
...
function weather.get(player)
	...
	local time = math.floor(core.get_gametime() - t_offset)

	nobj_density = nobj_density or core.get_perlin(np_density)
	nobj_thickness = nobj_thickness or core.get_perlin(np_thickness)
	nobj_speedx = nobj_speedx or core.get_perlin(np_speedx)
	nobj_speedz = nobj_speedz or core.get_perlin(np_speedz)
	...
	local humid = core.get_humidity(player:get_pos()) or 50
