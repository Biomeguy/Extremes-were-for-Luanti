...
core.register_chatcommand("legacy_load_areas", {
	params = S("<version>"),
	description = S("Loads, converts, and saves the areas from"
		.." a legacy save file."),
	privs = {areas=true, server=true},
	func = function(name, param)
		core.chat_send_player(name, S("Converting areas…"))
		local version = tonumber(param)
		if version == 0 then
			local err = areas:node_ownership_load()
			if err then
				core.chat_send_player(name, S("Error loading legacy file: @1", err))
				return
			end
		else
			core.chat_send_player(name, S("Invalid version number. (0 allowed)"))
			return
		end
		core.chat_send_player(name, S("Legacy file loaded."))
		...
		core.chat_send_player(name, S("Table format updated."))

		areas:save()
		core.chat_send_player(name, S("Converted areas saved. Done."))
	end
})

function areas:node_ownership_load()
	local filename = core.get_worldpath().."/owners.tbl"
	local tables, err
...
function areas.isNodeOwner(pos, name)
	if core.check_player_privs(name, areas.adminPrivs) then
		return true
	end
	for id, area in pairs(areas:getAreasAtPos(pos)) do
