...
local posmorvalarblock = {x= 777, y= -30794, z= -777}
...
function respawn_block(self)
	...
	if self.hp_record ~= self.health then
		...
		if os.time() - self.stop_timer > time_limit then
			core.chat_send_all("Time is over!")
			self.object:remove()
			core.set_node(posmorvalarblock, {name="nssm:dis_morvalar_block"})
		end
	end
end
		...
		if (os.time() - self.morvalar_timer) > 2 then
			...
			if self.attack:is_player() then
				if core.get_modpath("3d_armor") then
					...
					local pname, armor_inv = armor:get_valid_player(self.attack, "[set_player_armor]")

					if armor_inv:is_empty("armor") then

						-- punch player if not armored
						...
						if armor_num > 0 then
							...
							elseif mod_name == "shields" then
								nname = "shields_inv_".. nname ..".png"
							elseif mod_name == "nssm" then
								nname = "inv_".. nname ..".png"
							else
								nname = "morparticle.png"
							end
							...
							core.add_particlespawner({
								...
								minvel = {x=(s.x-p.x), y=(s.y-p.y), z=(s.z-p.z)},
								maxvel = {x=(s.x-p.x), y=(s.y-p.y), z=(s.z-p.z)},
							...
							armor_inv:set_stack("armor", armor_elements[steal_pos].pos, "")

							armor:set_player_armor(self.attack, self.attack)
							armor:save_armor_inventory(self.attack, self.attack)
	...
	if self.attack then
			...
			core.after(1, function(self)
				if self and self.attack:is_player() then
					...
					local player_inv = core.get_inventory({type= "player", name= pname})
					...
					if not player_inv:is_empty("main") then
						...
						for i= 1, 32 do
							...
							if core.get_item_group(items:get_name(), "eatable")==1 then
						...
						if imhungry == 0 then
							...
							core.add_particlespawner({
								...
								exptime = .5,
								size = 10,
								texture = "roasted_mantis_meat.png"})
						end
	...
	custom_attack = function(self)
		...
		if (os.time() - self.morvalar5_timer) > 2 then
			...
			core.after(2, function(self)
				...
				tnt.boom(p, {damage_radius=6,radius=5,ignore_protection=false})
		...
		if os.time() - self.morvalar2_timer > 1 then
			...
			for _,obj in ipairs(objects) do
				if obj:get_luaentity() then
					...
					if (name == "nssm:mordain" or name == "nssm:morde" or name == "nssm:morgut"
					or name == "nssm:morgre" or name == "nssm:morlu" or name == "nssm:morwa"
					or name == "nssm:morvy") then
						counter = counter + 1
					end
				end
			end
			core.chat_send_all("Ne ho contati: ".. counter)
			...
			if counter < 2 then
				...
				local per = {x= v.x+2, y= v.y, z= v.z+2}
...
mobs:register_mob("nssm:morvalar0", {
	...
	custom_attack = function(self)
		...
		if os.time() - self.morvalar1_timer > 1 then
			...
			local obj = core.add_entity(s, "nssm:kamehameha_bad")
...
core.register_entity("nssm:kamehameha_bad", {
initial_properties = {
	physical = false,
	visual = "sprite",
	...
	on_step = function(self, dtime)
		...
		for _,obj in ipairs(objects) do
			if obj:is_player() then
				core.chat_send_all("Dentro il raggio grande")
		...
		for _,obj in ipairs(objects) do
			if obj:is_player() then
				tnt.boom(pos, {damage_radius= 6, radius= 5, ignore_protection=false})

				self.object:remove()
				core.chat_send_all("Dentro il raggio piccolo")
			end
